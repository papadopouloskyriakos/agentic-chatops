...
// The nodes findVmNode searches. Discovered from the LIVE cluster (/nodes) rather than the static
// PVE_NODES env list, because that list goes stale the moment a node is added: gitlabrunner02 (vmid
// VMID_REDACTED) lives on nlpve04, which PVE_NODES ("...pve01,pve02,pve03") never named, so every
// pve_start / pve_guest_status auto-detect returned "not found" for a guest pve_list_vms could show
// when handed the node explicitly. Enumerating the cluster fixes that class permanently. PVE_NODES
// stays a fallback for when /nodes is briefly unreachable, so resolution degrades rather than dies.
// exported for the self-test (index.test.js) via the pveApi injection below.
async function resolverNodes(api = pveApi) {
  try {
    const nodes = await api("/nodes");
    const online = (nodes || [])
      .filter((n) => n.status === undefined || n.status === "online")
      .map((n) => n.node)
      .filter(Boolean);
    if (online.length) return online;
  } catch {
    // /nodes unreachable — fall back to the static list rather than resolve nothing.
  }
  return PVE_NODES;
}

// Helper: find which PVE host owns a VMID
async function findVmNode(vmid, api = pveApi) {
  for (const node of await resolverNodes(api)) {
    try {
      // Try LXC first
      const lxcs = await api(`/nodes/${node}/lxc`, { node });
      if (lxcs.some((c) => String(c.vmid) === String(vmid))) {
        return { node, type: "lxc" };
      }
      // Try QEMU
      const vms = await api(`/nodes/${node}/qemu`, { node });
...
// Test seam (no effect on the running server): let the self-test drive findVmNode/resolverNodes with a
// fake API. Guarded so a require() never starts the stdio server.
export { findVmNode, resolverNodes };
...
// ---------------------------------------------------------------------------
// Start — only when RUN directly, never on import (the self-test imports the resolver).
// ---------------------------------------------------------------------------
