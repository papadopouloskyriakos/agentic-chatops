...
```
                    NL-FRR01 (10.0.X.X) ──── NL-FRR02 (10.0.X.X)
                     ...
                     │                          │
    NL ASA (10.0.X.X) ─── iBGP ─── VPS-NO (10.255.3.11) ─── VPS-CH (10.255.2.11)
                                                 │                   │
...
- NL-FRR01 has `VPS_TRANSIT_FOR_ASA` outbound to NL ASA — sets GR next-hops to VPS IP (10.255.200.3) for VPS transit
...
|--------|--------|--------|------|--------|------|
| Tunnel1 | vti-gr | outside_xs4all | 203.0.113.X | 10.255.200.0/31 | xs4all backup |
| Tunnel2 | vti-no | outside_xs4all | 185.125.171.172 | 10.255.200.2/31 | xs4all backup |
| Tunnel3 | vti-ch | outside_xs4all | 185.44.82.32 | 10.255.200.4/31 | xs4all backup |
| Tunnel4 | vti-gr-f | outside_freedom | 203.0.113.X | 10.255.200.10/31 | **Freedom primary** |
| Tunnel5 | vti-no-f | outside_freedom | 185.125.171.172 | 10.255.200.12/31 | **Freedom primary** |
| Tunnel6 | vti-ch-f | outside_freedom | 185.44.82.32 | 10.255.200.14/31 | **Freedom primary** |

**GR ASA (4 tunnels):**
| Tunnel | Nameif | Peer | VTI IP | Role |
|--------|--------|------|--------|------|
| Tunnel1 | vti-nl | 145.53.163.13 (NL xs4all) | 10.255.200.1/31 | xs4all backup |
| Tunnel2 | vti-no | 185.125.171.172 | 10.255.200.6/31 | active |
| Tunnel3 | vti-ch | 185.44.82.32 | 10.255.200.8/31 | active |
| Tunnel4 | vti-nl-f | 203.0.113.X (NL Freedom) | 10.255.200.11/31 | **Freedom primary** |
...
```
route vti-gr-f 10.0.X.X 255.255.255.0 10.255.200.11 1
route vti-gr 10.0.X.X 255.255.255.0 10.255.200.1 10
... (9 GR subnets × 2 routes each = 18)
```

**GR ASA (16 static routes):**
```
route vti-nl-f 10.0.X.X 255.255.255.0 10.255.200.10 1
route vti-nl 10.0.X.X 255.255.255.0 10.255.200.0 10
...
route outside_freedom 203.0.113.X 255.255.255.255 185.93.175.233 1 track 1
route outside_xs4all 203.0.113.X 255.255.255.255 195.190.228.131 10
route outside_freedom 185.125.171.172 255.255.255.255 185.93.175.233 1 track 1
route outside_xs4all 185.125.171.172 255.255.255.255 195.190.228.131 10
route outside_freedom 185.44.82.32 255.255.255.255 185.93.175.233 1 track 1
route outside_xs4all 185.44.82.32 255.255.255.255 195.190.228.131 10
```
...
- NO VPS: `ssh -i ~/.ssh/one_key operator@185.125.171.172` (sudo password: `REDACTED_PASSWORD`)
- CH VPS: `ssh -i ~/.ssh/one_key operator@185.44.82.32` (sudo password: `REDACTED_PASSWORD`)
...
- Next-hop via Freedom VTI IPs (10.255.200.10-15) → LP 200
- Next-hop via xs4all VTI IPs (10.255.200.0-5) → LP 100
...
2. **Track BFD state in static routes** — `route vti-gr-f 10.0.X.X 255.255.255.0 10.255.200.11 1 track <bfd-track-id>`. When BFD detects Freedom tunnel is dead, the tracked route is removed and xs4all backup activates.
...
| NO VPS (185.125.171.172) | Update FRR + swanctl-loader routes |
| CH VPS (185.44.82.32) | Same |
