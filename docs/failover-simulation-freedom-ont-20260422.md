...
**Scope:** Predict the exact cascade when the operator admin-shuts the nl-sw01 port that feeds the Freedom ISP Genexis XGS-PON ONT, holds it for ~10 min, and then `no shut`s it.
...
```
   Tenants A/B/C/D, K8s, DMZ, servers, mgmt
      ...
      (VLAN 6)        (Po1.2, other vlans)
            ↓
      nl-sw01 Gi1/0/36   <─ target of `shut`
...
- Failure domain is strictly **VLAN 6 at the nl-sw01 edge** — the 4-port LACP between nl-fw01 and nl-sw01 stays up, every other VLAN keeps working.

## 2. NL nl-fw01 Freedom-dependent state (live)
...
- nl-fw01 ACL/NAT: K8s + mgmt + servers + dmz subnets already have identity NAT + after-auto PAT on `outside_budget` so they keep their connectivity through the rtr01 edge during failover.
...
- **nl-fw01 side**: `outside_freedom` (Po1.6) STAYS line-protocol-up because the nl-sw01↔ASA Po1 LACP trunk is unaffected. Only VLAN-6 egress has nowhere to terminate.
- Syslog on nl-fw01: no immediate "interface down" message.
- Syslog on nl-sw01: `%LINK-5-CHANGED: Interface Gi1/0/36, changed state to administratively down` + `%LINEPROTO-5-UPDOWN … to down`. LibreNMS polls nl-sw01 — interface status changes get picked up on next poll (≤5 min).

### Phase 2 — SLA + track flip (T+3 s to T+9 s)

- nl-fw01 SLA 1 probe to 198.51.100.X via outside_freedom at T+3 s — **first timeout** (ICMP echo returns no reply within 1000 ms).
...
- Freedom-sourced IKE keepalives from nl-fw01 start timing out.
...
- nl-fw01 ↔ GR ASA BGP peer `10.255.200.X` → **hold-down 30 s** elapses → session torn down at roughly **T+30-35 s**. During this 30 s window, nl-fw01 may keep stale best-paths pointing at `10.255.200.X` (Freedom VTI NO side) as next-hop; those become unreachable on the first packet because `198.51.100.X` is unreachable, but the BGP table still thinks the path is valid until hold-down fires.
- After BGP reconverges, nl-fw01's inter-site routes all flip to Budget-NH via rtr01 (the only path left for GR/NO/CH remote prefixes) — this is what the `FREEDOM_FRR_IN` route-map drops out of (its LP-200 VPS-loopback preference only applies while the FRR01 peer still reflects Freedom-NH).

### Phase 4 — Tenant impact (T+5 s onwards)

- Room A/B/C/D egress traffic routes to `outside_budget` (default route), nl-fw01 applies `after-auto source dynamic any interface` PAT **only** for the zones listed (iot, cctv, guest, servers01/02, mgmt, k8s, nfs, dmz_servers02/03). **Tenant rooms are NOT in that list.**
- Result: tenant packets hit nl-fw01, find no matching NAT rule for `(inside_room_*, outside_budget)`, and get dropped by NAT-RPF or egress untranslated and die at rtr01/Budget ISP.
...
- nl-fw01 BGP: 3 established (`10.0.X.X rtr01`, `10.0.X.X FRR01`, `10.0.X.X FRR02`). `10.255.200.X` peer down.
...
- **Genexis ONT cold-boot** typically 30-90 s. PADI requests from nl-fw01 get no response until ONT has trained its PON side with the OLT.

### Phase B — PPPoE handshake (T=10:01 to T=10:03)

- nl-fw01 PPPoE `id=1` sends PADI (broadcast on VLAN 6).
...
- nl-fw01 ↔ GR ASA BGP on `10.255.200.X` re-forms → session up in ~10-15 s after VTI.
...
- Best-path computation: `FREEDOM_IN` route-map applies LP 200 on nl-fw01's peer `10.255.200.X` → all GR-originated routes now prefer GR ASA Freedom VTI over rtr01 Budget. Equivalent for NL-FRR01's Freedom-NH (`FREEDOM_FRR_IN` sets LP 200 for VPS loopbacks).
- RIB flip: rtr01/Budget paths demote to backup, Freedom paths become best.
- nl-fw01 `show conn` has stale Budget-direction flows for established TCP; `timeout floating-conn 0:00:30` lets them re-home to the Freedom egress interface within 30 s (per `feedback_asa_clear_conn_after_vti.md`), no manual `clear conn` needed.
...
3. Record baselines: `show route`, `show bgp summary`, `show crypto ikev2 sa` on nl-fw01 and rtr01 before `shut`.
4. After `no shut`, confirm recovery with same `show` commands + `ping -I vti-no-f 10.255.200.X` from nl-fw01.
...
- **Tenant-room PAT on `outside_budget`**: add `nat (inside_room_a/b/c/d, outside_budget) after-auto source dynamic any interface` on nl-fw01 + ensure rtr01 Dialer1 has SRC-PAT for `10.0.X.X/30` to its Budget public IP. Closes the 2026-04-08 dual-WAN parity gap for tenant rooms.
...
### 11.1 — Tenant PAT for `outside_budget` (nl-fw01) with per-tenant source-IP preservation

Problem identified in §4: tenant rooms have `outside_freedom` object-NAT + `outside_lte` after-auto PAT, but **no NAT path to `outside_budget`**. When Freedom fails, tenant packets hit nl-fw01's backup default route (`outside_budget` metric 5), find no matching NAT rule, and get dropped.
...
! Preserves tenant IPs across the nl-fw01 → rtr01 transit so rtr01 QoS
...
Rollback: `no nat (inside_room_X,outside_budget) 1 source …` per line on nl-fw01, `no permit ip 192.168.17X.0 …` per ACL entry on rtr01.

Alternative (rejected): `after-auto source dynamic any interface` PAT on nl-fw01. Simpler (one line per room), but collapses all tenant source IPs to `10.0.X.X` at the transit — kills per-tenant QoS visibility on rtr01, making §11.2 impossible. Only adopt this path if the operator decides per-tenant rate limiting isn't required.
...
- **Refactor `scripts/freedom-qos-toggle.sh` to monitor-only**:
  - Keep: Freedom state detection (UP/DOWN via `outside_freedom` IP assignment), SLA 1 RTT query, nl-sw01 Gi1/0/36 CRC sample (every 7th run), Twilio SMS on state change, Prometheus textfile metrics (`freedom_pppoe_up`, `freedom_bng_rtt_ms`, `freedom_ont_port_errors`), suppression gates, state-file machinery.
  - Remove: `apply_qos()` + `remove_qos()` functions (referenced the defunct `XS4ALL-ROOM-*-PM` policy-maps on nl-fw01).
...
- Replace `apply_qos()`: instead of SSH→nl-fw01 for `service-policy XS4ALL-ROOM-B-PM …`, SSH→rtr01 using `scripts/lib/ios_ssh.ssh_rtr01_config` with:
  ```
  interface Dialer1
...
- `dual_wan_vpn_parity.md` memory: update the "QoS lives on nl-fw01" bullet to reflect the new rtr01 location.
...
Alert: `TenantQoSMisconfigured` — fires if `show policy-map | include TENANT_` on rtr01 returns 0 matches (catches the same silent-regression class as the one we just found on nl-fw01). Rule lives in `prometheus/alert-rules/tenant-qos.yml`, scrapes the textfile metric.
...
- nl-fw01 matches the new Section-1 identity NAT rules → tenant source IPs preserved across the `10.0.X.X/30` transit.
...
- As Freedom PPPoE re-establishes and track 1 flips Up, nl-fw01 default route reverts to outside_freedom. Tenant traffic goes back to the object-NAT path (nl-fw01 direct to Freedom). rtr01 is no longer in path → rtr01 HQoS becomes inert (shapes zero traffic).
...
Every control-plane claim sourced from a live `show` command (nl-fw01 routes, tracks, SLAs, VTI tunnels, BGP neighbors; rtr01 Dialer1, BGP, Tunnel1-3; VPS FRR + xfrm; nl-sw01 Gi1/0/36 description + trunk). Timing estimates (SLA probe 3-6 s, BGP hold-down 30 s, BFD sub-second) derived from live config values, not assumptions. Uncertainty remains on ONT cold-boot (hardware behavior) and exact LibreNMS scrape cadence (not inspected).
...
During the extended Freedom-shut window (the operator held `shut` longer to validate Budget-only steady-state), Budget silently died at **13:55:06 UTC**. Root cause: nl-fw01 threat-detection `scanning-threat` classified `10.0.X.X` (rtr01 budget transit IP) as an attacker — burst rate hit the configured max of `10 pps`. nl-fw01 installed a shun. All traffic from rtr01 was dropped until `clear shun` fired ~19 min later.
...
~19 min of Budget-path death during a window where Freedom was also intentionally down. Site was entirely reliant on NL-local services; cross-site, cluster mesh, and any outbound traffic via the Budget path were dropped on nl-fw01 ingress.
...
| T (UTC)  | Δ (s) | Event |
|----------|-------|-------|
| 17:12:58 |    0  | Operator issued first `no shutdown` on nl-sw01 Gi1/0/36. Port came admin-up but stayed `notconnect` — ONT did NOT resume PON sync. |
...
| 17:18:04 |  +306 | nl-fw01 `outside_freedom` IP **203.0.113.X** assigned — PPPoE LCP+IPCP complete. |
...
**Runbook implication.** Any Freedom maintenance that keeps nl-sw01
