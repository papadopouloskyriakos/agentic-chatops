...
3. Run each mutation against the goldset (using Ollama on nl-gpu01 for cheap evaluation, or batch Claude API calls)
...
**Status (updated 2026-03-25):** Full metamorphic autonomy remains research-level. But a **lite implementation** is now active via `scripts/metamorphic-monitor.sh` (cron every 6h):

1. **Auto-variant promotion** — When A/B data reaches 25+ sessions per variant with >0.05 confidence delta, proposes promoting the winner. Posts to `#alerts`.
2. **Cost-adaptive plan mode** — When a category's avg cost exceeds $3/session (n≥3), flags it for auto-plan-mode routing. Currently propose-only.
3. **Self-healing prompt rollback** — When a variant's 7d confidence drops >0.15 vs prior 7d, proposes routing 100% traffic to the other variant.
4. **Topology change signals** — Monitors escalation rate, cross-tier disagreement rate, and low-confidence session ratio. Alerts when thresholds suggest architectural changes.

Prometheus metrics: `metamorphic.prom`. Goldset validation T9 tracks readiness.
...
| B: Automated Prompt Engineering | High | Medium | P4 — prerequisites DONE (2026-03-25): cost/confidence/variant/category now tracked; A/B react_v1/v2 active; goldset harness built. Blocked on 200+ labeled sessions. |
| D: Factored Cognition | High | Medium | P4 — plan mode for bursts/dev (2026-03-25). Monitoring in place: goldset-validate.sh T8 tracks % sessions >10 turns or >$3. Triggers at 20%. Blocked on labeled data. |
| E: Metamorphic | Research | N/A | P5 — lite version DONE (2026-03-25): 3 self-modification monitors + topology signals. Cron every 6h. Auto-triggers at thresholds. Full autonomy future. |
