...
- Pings 10.0.181.X (NL nl-sw01) 50x → clears gr-fw01 SAs on failure
...
interface Tunnel1
 nameif vti-gr
 ip address 10.255.200.X 255.255.255.254
...
interface Tunnel2
 ...
 ip address 10.255.200.X 255.255.255.254
 tunnel source interface outside_freedom
 tunnel destination 198.51.100.X
 tunnel mode ipsec ipv4
 tunnel protection ipsec profile VTI-PROFILE
...
SSH: `ssh -i ~/.ssh/one_key operator@198.51.100.X`
...
connections {
    nl {
        local_addrs = 198.51.100.X
        remote_addrs = 203.0.113.X
        local { auth = psk id = 198.51.100.X }
    ...
    ch {
        local_addrs = 198.51.100.X
        remote_addrs = 198.51.100.X
        local { auth = psk id = 198.51.100.X }
        remote { auth = psk id = 198.51.100.X }
        version = 2
        dpd_delay = 30s
        children {
            ch {
...
secrets {
    ...
    ike-ch { id = 198.51.100.X secret = "PSK_HERE" }
}
```
...
sudo ip addr add 10.255.200.X/31 dev xfrm-nl
sudo ip addr add 10.255.200.X/31 dev xfrm-gr
sudo ip addr add 10.255.X.X/31 dev xfrm-ch
...
**Repeat for CH VPS (chzrh01vps01, 198.51.100.X) with adjusted IPs.**
