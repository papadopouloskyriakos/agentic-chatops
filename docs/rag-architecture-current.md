...
Auto-generated by `scripts/refresh-rag-architecture.py` on 2026-04-18 12:55 UTC.
Row counts and weights are introspected from the live system; prose is templated.

## Headline numbers (live)

- **Hard retrieval judge hit@5**: n/a (weekly cron) 
- **Hard KG traversal judge coverage@5**: n/a
- **RAGAS golden-set** (n=124): faithfulness 1.00, precision 0.97, recall 0.99, answer_relevance 0.64
- **End-to-end latency**: p50 5.53s, p95 6.82s (5-min rolling probe)
- **Total embedded vectors**: 1930 across embedded tables

### Corpus by table — exact row counts

**Embedded tables and row counts:** incident_knowledge (51), wiki_articles (972), session_transcripts (837), chaos_experiments (70). Graph: graph_entities (360) and graph_relationships (193).
...
3. **RAG Fusion rewrite** — `llama3.2:1b` temp=0.0, 3 perspective variants.
...
8. **Multi-chunk synthesis** — triggers when cross-encoder max < 0.6; 2-prompt ensemble via `qwen2.5:7b` num_ctx=8192.
