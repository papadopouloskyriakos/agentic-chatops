...
| 11 | n8n Code nodes run in task runner process — `require('fs')` can't read nl-claude01 files. Use SSH nodes for file I/O. | ENOENT on remote paths |
...
| 13 | Never increase LXC RAM without checking nl-pve03 total committed RAM (nl-claude01 steady-state: ~2.5GB, limit: 8GB) | 32GB caused host OOM crash |
