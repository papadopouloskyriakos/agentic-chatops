...
**Symptom:** HAProxy on NO VPS (185.125.171.172) reports `nl-matrix` backend DOWN. TCP 443 to 10.0.X.X (Matrix server on NL DMZ) times out from VPS.

**What works:** ICMP ping works (34ms). Other DMZ services on same subnet work (.10:9443 has 23KB flowing, .12:443 has 5.7KB).

**What we know:**
- VPS source IP is 10.255.200.3 (XFRM interface IP on xfrm-nl)
- NL ASA `show conn` shows the TCP connection: `TCP vti-no 10.255.200.3:42348 dmz_servers02 10.0.X.X:443, flags SaAB` — SYN forwarded, awaiting SYN-ACK
...
1. SSH to the Matrix server (10.0.X.X) and check:
   - `ip route` — does it have a route for 10.255.200.0/24? (its default gw is the NL ASA 10.0.X.X, which should route via VTI)
   - `iptables -L -n` — any firewall rules blocking 10.255.200.x?
   - `ss -tlnp | grep 443` — is the service listening?
   - `tcpdump -i any host 10.255.200.3 and port 443 -c 10` — are SYNs arriving? Are SYN-ACKs being sent?
...
| NO VPS | `ssh -i ~/.ssh/one_key operator@185.125.171.172` (mainif, not eth0) |
| CH VPS | `ssh -i ~/.ssh/one_key operator@185.44.82.32` (eth0) |
