...
- xs4all host routes for VPN peers exist on NL ASA (203.0.113.X, 185.125.171.172, 185.44.82.32 via 195.190.228.131)
...
| NO VPS | `ssh -i ~/.ssh/one_key operator@185.125.171.172` |
| CH VPS | `ssh -i ~/.ssh/one_key operator@185.44.82.32` (or ProxyJump via NO VPS) |
