...
> **2026-07-09/2026-08-25 status:** the per-session SMS described below is
> **RETIRED** (daily Matrix HITL digest replaced it; `/alert-session` returns
> `suppressed`). The banding itself is unchanged and live. Platform paging is now
> ntfy-first with SMS only for the ULTRA allowlist — `docs/runbooks/paging-ntfy.md`.
...
| `AUTO_NOTICE` | reversible MIXED touching a **P0 host** OR with **wide blast (>= INFRAGRAPH_BLAST_THRESHOLD)** | `[AUTO-RESOLVE]` + parallel SMS *(retired 2026-07-09 → daily digest)* | out-of-band veto (`!session abort`) |
| `POLL_PAUSE` | HIGH / irreversible / Infragraph deviation / partial / no-prediction / jailbreak / P0-reboot | `[POLL]`, no-vote PAUSES, SMS *(retired 2026-07-09 → daily digest)* | mandatory |
...
and the operator watched SMS not polls — both since superseded by the daily digest).
...
### SMS path (RETIRED 2026-07-09 — wiring kept, endpoint returns `suppressed`)
`classify-session-risk.py` POSTs to the paging bridge `/alert-session` (port 9106;
`scripts/paging-bridge.py`, formerly alertmanager-twilio-bridge.py)
on `sms_required` bands, at classify time (earlier than the poll = more reaction
time), best-effort (never blocks classify). Dedup by root-cause cluster since
2026-06-23 (`AM_TWILIO_SESSION_CLUSTER=0` reverts to `issue_id`). The bridge gates
