...
- **Cascade-probability gating (-1118, model_version=2):** `INFRAGRAPH_CASCADE_GATING=0`
  = byte-identical legacy (no gate, structural confidence). `INFRAGRAPH_CASCADE_MIN_PROB`
  (default 0.10) tunes the emit-threshold — raise it for more precision at recall
  cost. Gating is inert until `infragraph-learn.py --from-cascades` (hourly :25 cron)
  populates `infragraph_cascade_stats`; `DELETE FROM infragraph_cascade_stats` makes it
  inert again. Shadow-only — never touches the action lane or suppression.
