...
|---|---|---|---|
| reachable | — | `ok` | none |
| **down** | up | `isolated` | **SMS + ntfy push over LTE** (InAlan WAN down, GR alive on LTE, mesh down) — ULTRA class, operator decision 2026-08-25 |
| down | down | `dark` | best-effort SMS + push (GR hard-down or this host's uplink dead) |
...
- **re-notify** hourly while isolated — ntfy push every hour, **SMS at most every `GR_WAN_SMS_RENOTIFY_S` (default 6 h)**; **recovery = ntfy push only** with outage duration on clear (SMS is reserved for the isolation edge since the 2026-08-25 paging cutover).
- SMS via the Twilio REST API (API-key basic auth) and ntfy push via the PUBLIC route (`NTFY_URL=https://matrix.example.net`, topic `alrt-tier1`, Bearer `NTFY_TOKEN`); all creds in `/etc/gr-inalan-wan-monitor.env` (root 600, not in git). Runbook for the channel: `docs/runbooks/paging-ntfy.md`.
...
gr-inalan-wan-monitor.py --test-push  # send one labelled validation ntfy push (verified 200 from GR 2026-08-26)
