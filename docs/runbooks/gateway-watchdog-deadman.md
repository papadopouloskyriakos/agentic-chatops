...
`severity=critical`. **Not tier-1 since 2026-08-01/-25** (operator SMS triage →
ntfy cutover): they reach **Matrix/YouTrack** via `webhook-n8n`; the tier-1 paging
population (ntfy push via the paging bridge, SMS only for `page="sms"`) does not
include them — see `docs/runbooks/paging-ntfy.md`:
...
# confirm GatewayWatchdogHeartbeatStale fires and lands in Matrix/YouTrack (no SMS — not tier-1 since 2026-08-25), restore cron, confirm resolve.
```
