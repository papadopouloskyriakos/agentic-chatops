...
| Apr 8, 03:00–04:00 | **Phase 4–5:** iBGP mesh verified (30/30 sessions), GR grdmz02 TS_UNACCEPTABLE fixed | ASA debug: DELETE+RE-CREATE crypto-map entries |
...
| 5 | **GR grdmz02 TS_UNACCEPTABLE fix** — Deleted and re-created crypto-map entries 37/38 on GR ASA to clear stale TS matching tables. | gr-fw01 | Child SA established immediately. GR DMZ reachable. |
...
| 9 | ~~**ONT health monitoring**~~ — 3 Prometheus metrics added to `freedom-qos-toggle.sh` (cron */2): `freedom_bng_rtt_ms` (SLA RTT to BNG), `freedom_ont_port_errors` (nl-sw01 Gi1/0/36 CRC errors), `freedom_pppoe_up` (session status). Written to node_exporter textfile collector. LibreNMS also monitors nl-sw01 Gi1/0/36 (port_id 7861) and ASA `outside_freedom` (port_id 2776). | ~~Medium~~ | **DONE** (2026-04-09) — 5-layer monitoring: ASA SLA (5s), QoS toggle (2min), Prometheus metrics, LibreNMS switch port, LibreNMS ASA interface. | — |
