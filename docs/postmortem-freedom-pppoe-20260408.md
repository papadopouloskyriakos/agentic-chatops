...
|---|--------|---------|--------|
| 1 | **GR VPN restoration** — Added xs4all (145.53.163.13) as secondary peer on GR ASA seq 60-73. Added 8 crypto-map entries + 27 NAT exemptions to NL ASA xs4all map. | nl-fw01, gr-fw01 | 5 child SAs established immediately. GR reachable at ~50ms via xs4all. |
...
| 7 | ~~**Tenant default route failover**~~ — SLA monitor pings Freedom BNG (185.93.175.233) every 5s via `outside_freedom`. Track 1 attached to Freedom default route (metric 1). xs4all backup default route at metric 10. Automatic failover within seconds. `write memory` saved. | ~~High~~ | **DONE** (2026-04-09) — 3-tier default route: Freedom (1, tracked) → xs4all (10) → LTE (20). | — |
