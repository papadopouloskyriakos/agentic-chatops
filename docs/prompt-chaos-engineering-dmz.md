...
1. **DMZ nodes not on the topology graph** — nldmz01 and gr-dmz01 are not visualized
...
|------|------|-----|----------|-------|-----|
| nldmz01 | NL | 10.0.X.X | nl-pve01 | 2 | 4GB |
...
ssh -i ~/.ssh/one_key operator@nldmz01
...
IaC paths:
- NL: `/app/infrastructure/nl/production/edge/dmz/nldmz01/`
...
ssh -i ~/.ssh/one_key operator@nldmz01 'docker ps --format "{{.Names}}" | wc -l'
# Or check specific containers
ssh -i ~/.ssh/one_key operator@nldmz01 'docker ps --format "{{.Names}}|{{.Status}}" | head -15'
```
...
|------|----------|----------|-------------|
| T1 | Kill `portfolio` on nldmz01 | GR serves portfolio, NL returns 5xx | Gatus NL DOWN, GR UP |
| T2 | Kill `portfolio` on gr-dmz01 | NL serves portfolio, GR returns 5xx | Gatus GR DOWN, NL UP |
| T3 | Kill all containers on nldmz01 | GR serves all services | All services: NL DOWN, GR UP |
