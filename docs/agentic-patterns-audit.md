...
**Recommendation:**
- Add an **embeddings column** to `incident_knowledge` using a lightweight local embedding model (e.g., `nomic-embed-text` via Ollama on nl-gpu01)
...
- Run these biweekly via a cron job that feeds synthetic alerts through the pipeline and compares outputs against expected results
...
## Summary Scorecard (updated 2026-03-29)

| # | Pattern (Chapter) | Score | Implementation |
|---|---|---|---|
| 1 | Prompt Chaining | **A+** | 7-step security-triage.sh chain with structured TRIAGE_JSON between steps |
| 2 | Routing | **A+** | Severity-based routing + 3-way Switch V3.2 + scenario-aware SOUL.md investigation paths (ssh-bf/http/CVE routing) |
| 3 | Parallelization | **A** | 3 per-slot locks, NL+GR receivers parallel, security+CrowdSec pipelines concurrent |
| 4 | Reflection | **A+** | Cross-tier AGREE/DISAGREE/AUGMENT + CrowdSec multi-source cross-verification (local+CTI) |
| 5 | Tool Use | **A+** | 9 MCP servers + CTI REST API + CrowdSec cscli via SSH on 6 hosts |
| 6 | Planning | **A** | Deterministic 22-node workflow DAGs + 7-step triage sequence |
| 7 | Multi-Agent | **A+** | 3-tier (OpenClaw→Claude→Human) + 6 independent CrowdSec agents providing consensus |
| 8 | Memory | **A+** | SQLite (sessions/incidents/lessons/quality/feedback/a2a/openclaw_memory/crowdsec_scenario_stats) + file persistence + Claude memory |
| 9 | Learning & Adaptation | **A+** | CrowdSec learning loop: auto-suppress noisy scenarios, auto-unsuppress on escalation, CTI→baseline feedback, weekly review |
| 10 | MCP | **A+** | 9 MCP servers + evaluated & correctly rejected CrowdSec MCP (not fit for purpose) |
| 11 | Goal Setting & Monitoring | **A+** | 63+ Grafana panels + 6 security Prometheus metrics + CrowdSec learning metrics + 61 golden tests |
| 12 | Exception Handling | **A+** | 5-layer watchdog + ERROR_CONTEXT + graceful CTI degradation + iptables persistence (survives reboot) |
| 13 | Human-in-the-Loop | **A+** | Polls + reactions + timeouts + baseline review approval gate (CTI→baseline requires human !baseline-apply) |
| 14 | RAG | **A+** | 5-source security RAG (NetBox + baseline + scan data + CrowdSec local + CrowdSec CTI global) |
| 15 | Inter-Agent Communication (A2A) | **A+** | NL-A2A/v1 + TRIAGE_JSON with ctiData + baselineSuggestion + register callback |
| 16 | Resource-Aware Optimization | **A+** | CTI gated to critical/high (30/week budget) + dedup TTLs + severity-based suppression + auto-suppress noise |
| 17 | Reasoning Techniques | **A+** | ReAct + step-back + chain-of-verification + CTI cross-source verification |
| 18 | Guardrails/Safety | **A+** | Exec enforcement + input sanitization + credential scanning + SOUL.md READ-ONLY + iptables persistence + CAPI blocklist |
| 19 | Evaluation & Monitoring | **A+** | 61 golden tests (T1-T21) + CrowdSec regression detection (instability/drought/flap) + A/B variants |
| 20 | Prioritization | **A+** | Severity classification + burst elevation + CTI rate limiting + auto-suppress noise = focus on real threats |
| 21 | Exploration & Discovery | **A+** | 6 CrowdSec hosts (24/7 real-time) + daily scanner crons + proactive scan with CrowdSec health checks |

**Score: 21/21 at A or A+. No patterns below A.**
...
### Evaluation & Monitoring (B+ -> A-)
- **APE prerequisites implemented:** session_log now tracks cost_usd (via total_cost_usd fix), confidence, duration_seconds (CLI duration_ms), alert_category, prompt_variant, resolution_type
- **A/B testing active:** react_v1 vs react_v2 (adds chain-of-verification), deterministic by issue hash, recorded in session_log
- **Goldset scenarios:** 10 synthetic alert scenarios (goldset-scenarios.json) covering all major categories: availability (NL+GR), K8s (OOM, etcd), network, storage, correlated burst, recovery, WAL healer, dev
- **Goldset validation harness:** goldset-validate.sh — validates column population, variant distribution, cost sanity, confidence distribution, per-variant comparison, APE readiness (200+ labeled sessions threshold)
- **Prometheus metrics:** goldset-validation.prom with pass/fail/warn/labeled_sessions/timestamp
- **Root cause fixed:** CLI outputs `total_cost_usd` but Wait for Claude extracted `cost_usd` (always 0). Fixed in both Wait for Claude and Wait for Claude Fresh nodes. Also fixed: promptVariant/alertCategory undefined in poll early-return path (Prepare Result bug)
