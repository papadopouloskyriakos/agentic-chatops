...
PURPOSES = {
    "meshsat-copycat-watch.py": "Watch the meshsat.info copycat (MESHSAT-984): extract every fetch-and-execute URL from its pages wherever hosted, resolve each, hash each page, and check GitHub, GitLab and Codeberg for repos carrying the MeshSat mark under the same handle. Emits meshsat_copycat_* gauges; alerts in nl k8s namespaces/monitoring/meshsat-copycat-alerts.tf",
...
def parse_crontab():
    ...
    # Cronicle's shell plugin writes the JOB JSON to stdin — that is not a
    # crontab, and treating it as one wiped the doc down to one garbage row
    # (QA 654/655 caught it). Anything starting with '{' is job metadata.
    stdin_data = ""
    if not sys.stdin.isatty():
        try:
            ...
            stdin_data = ""
    if stdin_data.strip() and not stdin_data.lstrip().startswith("{"):
        out = stdin_data
    else:
        try:
    ...
    for line in out.split("\n"):
        ...
        if not line:
            continue
        # Post-Cronicle-migration (2026-06-26): every migrated job is preserved in
        # the crontab as a `#CRONICLE# <5 cron fields> <command>` comment line (the
        # migration's authoritative record; Cronicle does the actual scheduling).
        # Strip that marker so these jobs are catalogued instead of silently dropped
        # (the broken parser skipped every `#`-prefixed line, wiping the doc down to
        # the lone surviving raw entry). Genuine human comments are still skipped.
        if line.startswith("#CRONICLE#"):
            line = line[len("#CRONICLE#"):].strip()
            if not line:
                continue
