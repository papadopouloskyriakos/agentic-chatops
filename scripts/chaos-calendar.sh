...
case "$EXERCISE" in
    ...
    quarterly-redteam)
        # REPOINTED 2026-07-10: the security verdict now comes from the WIRED
        # control (plan-adherence-gate.py — reasoning-vs-action, IFRNLLEI01PRD-1746),
        # NOT from unified-guard.sh which is RETIRED/unwired. The unified-guard
        # adversarial suite is still run as an informational REFERENCE regression
        # (its patterns are hardened to 26/26) but it does not gate the exercise.
        log "Running red-team: PRIMARY = plan-adherence gate (wired); REFERENCE = unified-guard adversarial suite..."

        # PRIMARY — the wired control that actually protects the autonomy lane.
        PAG_OUTPUT=$(cd "$REPO_DIR" && bash "$SCRIPT_DIR/test-plan-adherence-gate.sh" 2>&1) || true
        echo "$PAG_OUTPUT" >> "$LOG"
        PAG_LINE=$(echo "$PAG_OUTPUT" | grep -oE 'RESULT: [0-9]+ pass / [0-9]+ fail' | tail -1)
        PAG_FAIL=$(echo "$PAG_LINE" | grep -oE '/ [0-9]+ fail' | grep -oE '[0-9]+' || echo "1")

        # REFERENCE — retired hook, informational only (never gates the exercise).
        REDTEAM_OUTPUT=$(cd "$REPO_DIR" && python3 "$SCRIPT_DIR/test-hook-blocks.py" --adversarial 2>&1) || true
        ...
        EXERCISE_TOTAL=1
        if [ "${PAG_FAIL:-1}" -eq 0 ]; then
            EXERCISE_PASS=1
            log "Red-team PASS: plan-adherence gate ${PAG_LINE:-unknown} (wired control healthy). Reference unified-guard: $RT_PASS pass / $RT_FAIL fail (informational, retired hook)."
        else
            EXERCISE_FAIL=1
            log "Red-team FAIL: plan-adherence gate ${PAG_LINE:-unknown} — the WIRED control regressed. Reference unified-guard: $RT_PASS pass / $RT_FAIL fail."
        fi

        # Reference metric still written (informational); the security signal is
        # now the plan-adherence gate's own metric + holistic section 46.
