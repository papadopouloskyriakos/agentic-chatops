...
def cmd_write(agent_name, entry, issue_id="", tags=""):
    ...
    conn.execute(
        "INSERT INTO agent_diary (agent_name, issue_id, entry, tags, embedding, schema_version) VALUES (?,?,?,?,?,?)",
        (agent_name, issue_id, entry, tags, embedding, schema_current("agent_diary"))
    )
    conn.commit()
