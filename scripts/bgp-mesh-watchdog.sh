...
# Layer-2 intersite auto-heal consumer (IFRNLLEI01PRD-1833, 2026-08-14): feed the
# already-collected mesh state to the healer AFTER the metrics publish so the
# observability path is never delayed or broken by a heal. The consumer carries
# its own gates (down-streak, maintenance/chaos, shadow+exemption sentinel,
# arming sentinel, backoff/escalate, wedge-signature) and a flock singleton, so
# an overlapping watchdog run cannot double-heal. `|| true`: consumer failure
# must never fail the watchdog job.
printf '%s' "$out" | python3 "$REPO_DIR/scripts/intersite-tunnel-heal.py" --from-watchdog \
    >> "${HOME}/logs/claude-gateway/intersite-heal.log" 2>&1 || true
