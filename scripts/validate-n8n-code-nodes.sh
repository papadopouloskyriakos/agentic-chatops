...
for js in "$TMP"/*.js; do
    ...
    # Check 1: Node.js --check, but WRAPPED in an async function so the parse
    # matches how n8n actually runs Code-node JS. n8n executes the code inside an
    # async wrapper, so top-level `await` AND top-level `return` are both legal; a
    # bare `node --check` (CommonJS script context) rejects `await` and produced
    # false positives on valid workflows (IFRNLLEI01PRD-1083, 2026-06-16).
    WRAP="$TMP/.wrap.js"   # dotfile so the "$TMP"/*.js loop never picks it up
    { printf 'async function __n8n_code__($, $input, $json, items, Buffer, require) {\n'; cat "$js"; printf '\n}\n'; } > "$WRAP"
    if ! node --check "$WRAP" 2>&1 | sed 's/^/  /'; then
        FAIL=1
        echo "  [FAIL] node --check (async-wrapped) failed"
        continue
    fi
...
// AsyncFunction() parse matches n8n runtime (Code nodes run inside an async
// wrapper, so `await` is valid). Using a plain Function() here rejected valid
// top-level-await code — false positive (IFRNLLEI01PRD-1083).
const AsyncFunction = Object.getPrototypeOf(async function(){}).constructor;
try {
    new AsyncFunction('$', '$input', '$json', 'items', 'Buffer', 'require', code);
    console.log('[ok]   AsyncFunction() parse (n8n async runtime)');
} catch (e) {
    console.log('[FAIL] AsyncFunction() parse:', e.message);
    fail = 1;
}
