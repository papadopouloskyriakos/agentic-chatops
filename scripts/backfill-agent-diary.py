...
def backfill():
    ...
    for agent_name in sorted(agent_ops.keys()):
        ...
        conn.execute(
            "INSERT INTO agent_diary (agent_name, entry, tags) VALUES (?, ?, ?)",
            (agent_name, entry, tags),
        )
        inserted += 1
        ...
        if session_count >= 3:
            ...
            for sid in sample_sessions:
                ...
                if (agent_name, per_entry[:50]) in existing:
                    continue
                conn.execute(
                    "INSERT INTO agent_diary (agent_name, issue_id, entry, tags) VALUES (?, ?, ?, ?)",
                    (agent_name, sid, per_entry, "backfill,per-session"),
                )
                inserted += 1
