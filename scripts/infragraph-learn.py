...
def main() -> int:
    ...
    ap.add_argument("--from-cascades", action="store_true",
                    help="recompute cascade-probability stats from evaluated "
                         "shadow predictions (IFRNLLEI01PRD-1118)")
    ap.add_argument("--since", default="", help="ISO prefix filter for incidents pass")
    ap.add_argument("--until", default="")
    args = ap.parse_args()
    if not (args.from_chaos or args.from_incidents or args.from_cascades):
        ap.error("pick --from-chaos and/or --from-incidents and/or --from-cascades")
    ...
    try:
        ...
        if args.from_incidents:
            ...
            conn.commit()
        if args.from_cascades:
            report["cascades"] = infragraph.learn_cascade_stats(conn)
            conn.commit()
    finally:
        conn.close()
