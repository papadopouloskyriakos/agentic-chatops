...
def check_insufficient_hitl():
    ...
    POLL_PAUSE band with a parallel SMS page (paging-bridge /alert-session (retired path, suppressed));
    ...
    classifier = _read("scripts/classify-session-risk.py")
    sms = _has(classifier, "sms_required") and _exists("scripts/paging-bridge.py")
    ...
    if sms and invariant and poll_pause:
        return _f(cat, PASS,
                  ...
                  "parallel SMS page (sms_required + paging-bridge.py /alert-session); "
    ...
    if poll_pause and not sms:
        return _f(cat, WARN,
                  "POLL_PAUSE band exists but the SMS escalation (sms_required + paging bridge) "
