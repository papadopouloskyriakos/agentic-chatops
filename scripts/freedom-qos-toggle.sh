...
# ntfy push helper (tier-1 paging channel, 2026-08-25)
# shellcheck source=scripts/lib/ntfy.sh
source "$REPO_DIR/scripts/lib/ntfy.sh"
...
if [ "$freedom_status" = "DOWN" ] && [ "$current_state" != "freedom-down" ]; then
    ...
    # ULTRA-urgent per operator decision 2026-08-25: SMS + ntfy push
    ...
    ntfy_publish "[nl] Freedom ISP DOWN" "PPPoE lost on outside_freedom. Budget (rtr01) carrying all S2S tunnels + tenants at 15/5 Mbps (rtr01 HQoS). Physical fix: power-cycle ONT on nl-sw01 Gi1/0/36." 5 "rotating_light,nl"
    ...
    # recovery: ntfy only (SMS is reserved for the ULTRA DOWN edge, 2026-08-25)
    ntfy_publish "[nl] Freedom ISP RECOVERED" "PPPoE re-established. Tenants back on Freedom path at full bandwidth. rtr01 HQoS (15/5 cap) now idle." 3 "white_check_mark,nl"
