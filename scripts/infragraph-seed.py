...
# NL+GR PVE hosts share ONE Proxmox cluster, so `pvesh get /cluster/resources`
# against ANY member returns every guest. The seed used to be pinned to a single
# host; when that host is wedged under load its pmxcfs/pvesh call blocks past the
# timeout (observed 2026-06-27: nl-pve01 load-avg 80, pvesh >40s; nl-pve03
# 1.5s) and the pve seed silently goes stale (InfragraphSeedStale, 21h+ on
# pve while netbox/librenms stayed fresh). Iterate over the cluster members so a
# single wedged host no longer dark-stalls the seed.
# INFRAGRAPH_PVE_SSH_HOST stays the preferred first member;
# INFRAGRAPH_PVE_SSH_HOSTS (comma-separated) overrides the whole fallback list.
_DEFAULT_PVE_HOSTS = ["nl-pve01", "nl-pve03", "nlpve04", "nl-pve02"]


def _pve_ssh_hosts() -> list[str]:
    env_list = os.environ.get("INFRAGRAPH_PVE_SSH_HOSTS", "").strip()
    if env_list:
        hosts = [h.strip() for h in env_list.split(",") if h.strip()]
    else:
        # preferred host first, then the remaining default members as fallbacks
        hosts = [PVE_SSH_HOST] + [h for h in _DEFAULT_PVE_HOSTS if h != PVE_SSH_HOST]
    seen: set[str] = set()
...
def seed_pve(conn) -> dict:
    ...
    ONE Proxmox cluster, so a single pvesh call returns every guest. Tries each
    cluster member in turn so a single wedged host doesn't dark-stall the seed.
    """
    hosts = _pve_ssh_hosts()
    out = None
    ...
    for host in hosts:
        ...
        try:
            out = subprocess.check_output(
                cmd, timeout=30, stderr=subprocess.DEVNULL).decode()
            break
    ...
    if out is None:
        raise RuntimeError(
            f"pve seed: all cluster members unreachable ({', '.join(hosts)}): "
...
def seed_declared(conn) -> dict:
    ...
    with open(DECLARED_DOC, encoding="utf-8") as fh:
        for lineno, line in enumerate(fh, 1):
            ...
            if line.startswith("|---"):
                continue
            # The declared-edge table ends at the first non-pipe line. The doc
            # is UNION-merged with the P0-host / reversibility-class tables
            # (autonomy-gate, IFRNLLEI01PRD-1102) — without this boundary the
            # parser would walk those unrelated tables and reject every row.
