...
# ─── T2b intersite rule files pass syntax check (added 2026-08-14, IFRNLLEI01PRD-1833) ──
start_test "intersite_rules_syntax"
  if ! _pod_exists; then
    skip_test "pod unavailable"
  else
    for f in intersite-bgp-mesh.yml intersite-autoheal.yml pve-host-health.yml; do
      out=$(cat "$REPO_ROOT/prometheus/alert-rules/$f" | \
        kubectl -n "$NS" exec -i "$POD" -c prometheus -- /bin/promtool check rules /dev/stdin 2>&1)
      if ! echo "$out" | grep -q 'SUCCESS'; then
        fail_test "promtool check rules failed for $f: $out"
      fi
    done
  fi
end_test
...
# ─── T3b pve-host-health rules fire under synthetic time-series (added 2026-08-25) ───
start_test "pve_host_health_rules_fire_under_synthetic_conditions"
  if ! _pod_exists; then
    skip_test "pod unavailable"
  else
    # Stage rule + test file together, run promtool test rules, cleanup
    TEST_OUTPUT=$(tar -C "$REPO_ROOT/prometheus/alert-rules" -cf - pve-host-health.yml pve-host-health.test.yml 2>/dev/null | \
      kubectl -n "$NS" exec -i "$POD" -c prometheus -- sh -c '
        cd /prometheus &&
        tar -xf - &&
        TMPDIR=/prometheus /bin/promtool test rules pve-host-health.test.yml 2>&1
        RC=$?
        rm -f pve-host-health.yml pve-host-health.test.yml
        exit $RC
      ' 2>&1)
    if echo "$TEST_OUTPUT" | grep -q 'SUCCESS'; then
      :
...
start_test "test_file_exists_and_parses"
  for tf in agentic-health.test.yml pve-host-health.test.yml; do
    if [ ! -f "$REPO_ROOT/prometheus/alert-rules/$tf" ]; then
      fail_test "$tf missing"
    elif ! python3 -c "import yaml; yaml.safe_load(open('$REPO_ROOT/prometheus/alert-rules/$tf'))" >/dev/null 2>&1; then
      fail_test "$tf is not valid YAML"
    fi
  done
end_test
