...
# Fixture dates are computed (yesterday) — hardcoded 2026-06-02 aged out of the
# scorecard's sliding 30d window on 2026-07-02 and the gate-evidence test began
# failing by pure calendar (evaluated>=4 went False).
FIXDATE=$(date -u -d yesterday +%F)
FIXDB=$(mktemp --suffix=.db)
sqlite3 "$FIXDB" < "$REPO_ROOT/schema.sql"
FIXLOG=$(mktemp --suffix=.log)
cat > "$FIXLOG" << LOG
${FIXDATE}T11:01:00Z|nl-n8n01|-- ALERT -- nl-n8n01 -  Service up/down  - Critical Alert|nl|escalated|0|0|IFR-A
${FIXDATE}T11:02:00Z|nl-gpu01|-- ALERT -- nl-gpu01 -  Device Down! Due to no ICMP response.  - Critical Alert|nl|escalated|0|0|IFR-B
${FIXDATE}T11:03:00Z|gr-sw01|-- ALERT -- gr-sw01 -  Device Down! Due to no ICMP response.  - Critical Alert|gr|escalated|0|0|IFR-C
LOG
...
('${FIXDATE} 11:00:00', 'action', '', 'nl-pve01', 'reboot_host', 900,
...
start_test "verdict_detail_carries_the_diff"
  # action target = nl-pve01 (NL). Of the 2 unpredicted hosts, nl-gpu01 (NL) is a
  # real same-site surprise; gr-sw01 (GR) is now coincidental cross-site (#2 L1) and
  # excluded -> 1 surprise remains, verdict stays deviation, + the GR host in coincidental.
  d=$(sqlite3 "$FIXDB" "SELECT verdict_detail FROM infragraph_predictions WHERE plan_hash='h-dev'")
  s=$(printf '%s' "$d" | python3 -c "import json,sys; d=json.load(sys.stdin); print(len(d['surprises']), d['surprises'][0][0], len(d['matched']), len(d.get('coincidental_cross_site',[])))")
  assert_eq "1 nl-gpu01 1 1" "$s" "1 same-site surprise + 1 matched + 1 coincidental cross-site"
end_test

start_test "verify_cli_exit_codes_encode_verdict"
  # fresh row, already past window — verify synchronously
  sqlite3 "$FIXDB" "INSERT INTO infragraph_predictions (created_at, kind, parent_issue_id, parent_host, parent_rule, window_seconds, predicted, plan_hash, action_kind, action_target) VALUES ('${FIXDATE} 11:00:00', 'action', '', 'nl-pve01', 'reboot_host', 900, '[{\"host\": \"nl-n8n01\", \"rule\": \"Service up/down\"}, {\"host\": \"nl-gpu01\", \"rule\": \"Device Down! Due to no ICMP response.\"}, {\"host\": \"gr-sw01\", \"rule\": \"Device Down! Due to no ICMP response.\"}]', 'h-v', 'reboot_host', 'nl-pve01')"
...
# IFRNLLEI01PRD-1408 #1: the action's OWN target host alerting is the EXPECTED direct effect
# (you rebooted/restarted it), NOT a cascade surprise — excluded from the verdict so a clean
# self-recovery scores 'match' not a systematic 'deviation'. A real cascade to an UNPREDICTED
# host still deviates (safety preserved); legacy (no target_host) unchanged.
_av() { cd "$REPO_ROOT/scripts" && python3 -c "from lib import infragraph as I; print(I.action_verdict($1,$2,target_host=$3)[0])"; }
start_test "self_target_alert_is_match_not_deviation"
  assert_eq match "$(_av "[{'host':'h1','rule':'r'}]" "[{'host':'pve02','rule':'Service up/down'}]" "'pve02'")"
end_test
start_test "real_cascade_to_unpredicted_host_still_deviation"
  assert_eq deviation "$(_av "[{'host':'h1','rule':'r'}]" "[{'host':'pve02','rule':'x'},{'host':'hB','rule':'Down'}]" "'pve02'")"
end_test
start_test "verdict_target_host_defaults_none_legacy"
  assert_eq deviation "$(_av "[{'host':'h1','rule':'r'}]" "[{'host':'pve02','rule':'x'}]" "None")"
end_test
start_test "quiet_remediation_is_match"
  assert_eq match "$(_av "[{'host':'h1','rule':'r'}]" "[]" "'pve02'")"
end_test
