...
paths = [s.source_path for s in snips]
# A prompt-chaining doc should surface for the keyword (intent: keyword matches are preferred).
# Not over-specified to gulli-01: the doc corpus grew to include imported book chapters
# (docs/chapter-01-prompt-chaining.md etc.) which are equally-valid keyword matches and may
# rank first — the selection is still correctly keyword-driven.
assert any(('prompt' in p.lower() and 'chain' in p.lower()) for p in paths), paths
print('ok')
")
  assert_contains "$out" "ok"
end_test
