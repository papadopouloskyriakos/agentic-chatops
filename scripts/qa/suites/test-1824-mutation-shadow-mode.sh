...
# TG-403 follow-on: the header's hermeticity claim was only half true. The dark-path tests unset
# MUTATIONS_OFF and then fell through to the REAL ~/gateway.mutations_off — on a box where the operator
# armed global shadow (this one, since 2026-07-18), every dark test failed against live state, which the
# subshell bug then hid. An isolated GATEWAY_HOME makes sentinel reads hermetic for the whole suite; the
# shadow tests still force their state via MUTATIONS_OFF env, which wins over the (now absent) sentinel.
