...
# The JSON shapes asserted here are the FROZEN contract that infra-triage.sh
# Step 2-graph and classify-session-risk.py consume. model_version=2 since the
# cascade-probability gating landed (IFRNLLEI01PRD-1118); item shapes unchanged
# (the gate is inert on fixtures with no learned cascade stats).
set -u
...
start_test "cascade_predictions_and_window"
  ...
  assert_contains "$out" '"model_version": 2'
