...
# TG-403: fail/skip state must survive `( … )` subshells. Suites legitimately scope env per test with
# `( export X=…; assert_… )` — but a variable write dies with the subshell, so fail_test ran, set
# _qa_test_failed=1, and the parent's end_test still saw a clean test: 7 assertions in
# test-1824-mutation-shadow-mode could NEVER fail. An append to a file survives; $$ is the suite process
# even inside its subshells, so parallel suites cannot cross-mark. start_test clears, end_test consumes.
_qa_failmark="${QA_RESULT_FILE}.$$.failmark"
_qa_skipmark="${QA_RESULT_FILE}.$$.skipmark"
...
start_test() {
  ...
  _qa_test_failed=0
  _qa_test_skipped=0
  rm -f "$_qa_failmark" "$_qa_skipmark"
...
end_test() {
  ...
  # Consume marks left by subshelled fail_test/skip_test calls (TG-403): the variable half never made it
  # back to this shell, so the file is the only witness. FAIL keeps precedence over SKIP, as in the vars.
  if [ -s "$_qa_failmark" ] && [ "$_qa_test_failed" -eq 0 ]; then
    _qa_test_failed=1
    _qa_fail_detail="$(head -n 1 "$_qa_failmark")"
  fi
  if [ -s "$_qa_skipmark" ] && [ "$_qa_test_skipped" -eq 0 ]; then
    _qa_test_skipped=1
    _qa_skip_detail="$(head -n 1 "$_qa_skipmark")"
  fi
...
skip_test() {
  ...
  printf '%s\n' "$_qa_skip_detail" >>"$_qa_skipmark"
}

fail_test() {
  _qa_test_failed=1
  _qa_fail_detail="${1:-fail}"
  printf '%s\n' "$_qa_fail_detail" >>"$_qa_failmark"
}
