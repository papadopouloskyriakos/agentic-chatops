...
***REMOVED***═════
echo "[21] Phase 1b — blast-radius matches host_pattern + rule glob → dedup"
***REMOVED***═════
NOW="2026-05-11T10:00:00Z"
om_insert "'blast-radius','IFRNLLEI01PRD-891','{\"host_patterns\":[\"nl*\"],\"rules\":[\"*VTI*\",\"*BGP*\",\"*PPPoE*\"],\"description\":\"Freedom XGS-PON PPPoE outage cascade\"}','2026-05-08T07:46:00Z'"
OUT=$(run_cli --hostname nl-fw01 --rule-name 'VTI-Tunnel4-Down' --severity critical \
  --db "$DB" --triage-log "$LOG" --no-yt-check --now-utc "$NOW")
assert_jq "phase1b-host-pat: outcome=dedup" "$OUT" "d['outcome']" "dedup"
assert_jq "phase1b-host-pat: phase tag" "$OUT" "d['phase']" "phase1b-blast-radius"
assert_jq "phase1b-host-pat: parent surfaced" "$OUT" "d['existing_issue_id']" "IFRNLLEI01PRD-891"
assert_jq "phase1b-host-pat: confidence" "$OUT" "d['confidence']" "0.9"

***REMOVED***═════
echo "[22] Phase 1b — exact host match (hosts list) + rule glob → dedup"
***REMOVED***═════
om_insert "'blast-radius','IFRGRSKG01PRD-998','{\"hosts\":[\"gr-sw01\",\"gr-pve02\"],\"rules\":[\"*Device Down*\",\"*Service up*\"],\"description\":\"GR mass-flap (LibreNMS poller stall)\"}','2026-05-11T03:00:00Z'"
OUT=$(run_cli --hostname gr-sw01 --rule-name 'Device Down! Due to no ICMP response.' --severity critical \
  --db "$DB" --triage-log "$LOG" --no-yt-check --now-utc "$NOW")
assert_jq "phase1b-exact-host: outcome" "$OUT" "d['outcome']" "dedup"
assert_jq "phase1b-exact-host: parent" "$OUT" "d['existing_issue_id']" "IFRGRSKG01PRD-998"

***REMOVED***═════
echo "[23] Phase 1b — host doesn't match → escalate, journey records phase1b"
***REMOVED***═════
OUT=$(run_cli --hostname totally-unrelated-host --rule-name 'SomethingElse' --severity warning \
  --db "$DB" --triage-log "$LOG" --no-yt-check --now-utc "$NOW")
assert_jq "phase1b-no-match: outcome=escalate" "$OUT" "d['outcome']" "escalate"
assert_jq "phase1b-no-match: scanned in signals" "$OUT" "'phase1b' in d['signals']" "True"

***REMOVED***═════
echo "[24] Phase 1b — self-fold guard (current_issue_id == parent_id) → escalate"
***REMOVED***═════
OUT=$(run_cli --hostname nl-fw01 --rule-name 'VTI-Tunnel4-Down' --severity critical \
  --current-issue-id IFRNLLEI01PRD-891 \
  --db "$DB" --triage-log "$LOG" --no-yt-check --now-utc "$NOW")
assert_jq "phase1b-self-fold: outcome (self never folds)" "$OUT" "d['outcome']" "escalate"

***REMOVED***═════
echo "[25] Phase 1b — rule glob only (no hosts/host_patterns) → matches any host"
***REMOVED***═════
om_insert "'blast-radius','IFRNLLEI01PRD-997','{\"rules\":[\"AlertManagerSilenced*\"],\"description\":\"global silenced bucket\"}','2026-05-12T01:00:00Z'"
OUT=$(run_cli --hostname any-host --rule-name 'AlertManagerSilenced-FreedomCascade' --severity warning \
  --db "$DB" --triage-log "$LOG" --no-yt-check --now-utc "$NOW")
assert_jq "phase1b-rule-only: outcome" "$OUT" "d['outcome']" "dedup"
assert_jq "phase1b-rule-only: parent" "$OUT" "d['existing_issue_id']" "IFRNLLEI01PRD-997"

***REMOVED***═════
echo "[26] Phase 1b — malformed JSON in value → silently skipped, journey continues"
***REMOVED***═════
om_insert "'blast-radius','IFRNLLEI01PRD-996','not-actually-json','2026-05-12T02:00:00Z'"
OUT=$(run_cli --hostname only-this-host --rule-name 'OnlyThisRule' --severity warning \
  --db "$DB" --triage-log "$LOG" --no-yt-check --now-utc "$NOW")
# Should escalate (no other phase matches) without crashing
assert_jq "phase1b-bad-json: outcome=escalate (no crash)" "$OUT" "d['outcome']" "escalate"
