...
export REPO_ROOT  # IFRNLLEI01PRD-1089: suites' inline python reads os.environ['REPO_ROOT'];
                  # exporting it stops spurious KeyError failures when run from cron (no inherited env).
cd "$REPO_ROOT"
...
run_one() {
  ...
  # A suite that legitimately needs more headroom under full-suite load can
  # declare `# QA_SUITE_TIMEOUT: <seconds>` in its header (first 20 lines).
  # The declaration only ever RAISES the limit — an explicit
  # QA_PER_SUITE_TIMEOUT env above the declaration still wins, so strict CI
  # can't be loosened by a suite, and a wedge is still caught (just later).
  local declared
  declared=$(head -20 "$path" | grep -m1 -oE '^# QA_SUITE_TIMEOUT: [0-9]+' | grep -oE '[0-9]+$' || true)
  if [ -n "$declared" ] && (( declared > per_suite_timeout )); then
    per_suite_timeout="$declared"
  fi
  local start_ts
  start_ts=$(date +%s)
