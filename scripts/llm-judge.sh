...
OLLAMA_URL="${OLLAMA_URL:-http://nl-gpu01:11441}"
# 2026-08-26 (nl-gpu01 VRAM-starvation RCA): every local curl is bounded and a
# --recent/--backfill run pre-flights the local model ONCE instead of looping
# 22 s OOM load-failures over every unjudged session (which serialized every
# embed client on the shared GPU and then wedged the Ollama scheduler).
JUDGE_CURL_TIMEOUT="${JUDGE_CURL_TIMEOUT:-180}"
JUDGE_PREFLIGHT_TIMEOUT="${JUDGE_PREFLIGHT_TIMEOUT:-90}"
JUDGE_LOCK="${JUDGE_LOCK:-/tmp/llm-judge.batch.lock}"
...
" <<< "$LOCAL_PROMPT" 2>/dev/null)
    local LOCAL_RESPONSE=$(curl -s --max-time "$JUDGE_CURL_TIMEOUT" -X POST "$OLLAMA_URL/api/generate" \
...
import json,sys;d=json.load(sys.stdin);d['model']='$JUDGE_LOCAL_FALLBACK';print(json.dumps(d))")
      LOCAL_RESPONSE=$(curl -s --max-time "$JUDGE_CURL_TIMEOUT" -X POST "$OLLAMA_URL/api/generate" \
        -H "content-type: application/json" \
...
print(json.dumps(out))" 2>/dev/null)
    ...
    if [ "${JUDGE_JURY:-1}" = "1" ] && [ "$MODEL" != "$JUDGE_LOCAL_FALLBACK" ]; then
      ...
      local J2_RESPONSE=$(curl -s --max-time "$JUDGE_CURL_TIMEOUT" -X POST "$OLLAMA_URL/api/generate" -H "content-type: application/json" -d "$J2_PAYLOAD" 2>/dev/null)
...
# Batch-run guards (2026-08-26):
#  * one batch at a time (flock) — Cronicle stacked 5 concurrent --recent runs
#    when each hung on the wedged GPU;
#  * pre-flight the local judge model with a tiny generate before iterating
#    sessions; if the primary cannot load, drop to the fallback for this run
#    (jury off — it needs both models); if neither loads, skip the run.
_ollama_can_generate() {  # $1=model
  local r
  r=$(curl -s --max-time "$JUDGE_PREFLIGHT_TIMEOUT" -X POST "$OLLAMA_URL/api/generate" \
        -H "content-type: application/json" \
        -d "{\"model\":\"$1\",\"prompt\":\"ok\",\"stream\":false,\"options\":{\"num_predict\":1,\"num_ctx\":4096}}" 2>/dev/null)
  echo "$r" | python3 -c "import json,sys; d=json.load(sys.stdin); sys.exit(0 if d.get('done') else 1)" >/dev/null 2>&1
}
_co_resident() {  # $1=model $2=model — both currently loaded per /api/ps?
  curl -s --max-time 10 "$OLLAMA_URL/api/ps" | python3 -c "
import json,sys
n={m['name'].split(':')[0] for m in json.load(sys.stdin).get('models',[])}
sys.exit(0 if {'$1'.split(':')[0], '$2'.split(':')[0]} <= n else 1)" 2>/dev/null
}
local_judge_preflight() {
  [ "$JUDGE_BACKEND" = "local" ] || return 0
  if _ollama_can_generate "$JUDGE_LOCAL_MODEL"; then
    log "preflight: $JUDGE_LOCAL_MODEL loads on $OLLAMA_URL"
    # The RAG synth/rewrite/json path uses $JUDGE_LOCAL_FALLBACK (qwen2.5:7b)
    # all day. If the primary cannot co-reside with it, a judge batch on the
    # primary makes Ollama swap the two models on every concurrent RAG call —
    # each swap waits for the busy runner, fails with "timed out waiting", and
    # trips the rag_synth_ollama breaker. So: primary only when both fit,
    # otherwise run the batch on the shared model (jury needs both → off).
    if _ollama_can_generate "$JUDGE_LOCAL_FALLBACK" && _co_resident "$JUDGE_LOCAL_MODEL" "$JUDGE_LOCAL_FALLBACK"; then
      log "preflight: $JUDGE_LOCAL_MODEL + $JUDGE_LOCAL_FALLBACK co-resident — primary=$JUDGE_LOCAL_MODEL jury=${JUDGE_JURY:-1}"
    else
      log "preflight: $JUDGE_LOCAL_MODEL cannot co-reside with $JUDGE_LOCAL_FALLBACK (the shared RAG model) — running this batch on $JUDGE_LOCAL_FALLBACK to avoid swap-thrash, jury off"
      JUDGE_LOCAL_MODEL="$JUDGE_LOCAL_FALLBACK"; JUDGE_JURY=0
    fi
    return 0
  fi
  if _ollama_can_generate "$JUDGE_LOCAL_FALLBACK"; then
    log "preflight: $JUDGE_LOCAL_MODEL cannot load (VRAM?) — using $JUDGE_LOCAL_FALLBACK for this run, jury off"
    JUDGE_LOCAL_MODEL="$JUDGE_LOCAL_FALLBACK"; JUDGE_JURY=0
    return 0
  fi
  log "preflight: neither $JUDGE_LOCAL_MODEL nor $JUDGE_LOCAL_FALLBACK can generate on $OLLAMA_URL within ${JUDGE_PREFLIGHT_TIMEOUT}s — skipping this run (see docs/runbooks/rerank-service.md § Dynamic VRAM)"
  return 1
}
acquire_batch_lock() {
  exec 9>"$JUDGE_LOCK"
  if ! flock -n 9; then
    log "another batch run holds $JUDGE_LOCK — exiting (no overlapping runs)"
    exit 0
  fi
}
