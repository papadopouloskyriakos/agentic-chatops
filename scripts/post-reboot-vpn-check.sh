...
# VTI architecture (2026-04-09): per-peer SA clearing instead of global nuke.
# clear xlate removed — was only needed for crypto-map NAT exemption issues.
VTI_PEERS_GR="145.53.163.13 185.125.171.172 185.44.82.32"
VTI_PEERS_NL="203.0.113.X 185.125.171.172 185.44.82.32"

clear_stale_sas() {
  log "Step 4: CLEARING stale SAs on REMOTE side (per-peer VTI)..."

  if $DRY_RUN; then
    log "DRY-RUN: Would run per-peer 'clear crypto ipsec sa peer <IP>' on remote ASA"
    return 0
  fi
  ...
  if [ "$SITE" = "nl" ]; then
    # NL rebooted → clear GR VTI peers (remote side has stale SAs)
    local clear_cmds=""
    for peer in $VTI_PEERS_GR; do
      clear_cmds="${clear_cmds}clear crypto ipsec sa peer ${peer}
"
    done
    ssh -i ~/.ssh/one_key -o ConnectTimeout=15 -o StrictHostKeyChecking=no \
      "$GR_PVE_STEP" "ssh -o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedAlgorithms=+ssh-rsa \
      -o ConnectTimeout=10 -o StrictHostKeyChecking=no $ASA_USER@$GR_ASA_IP \
      '${clear_cmds}'" 2>/dev/null
    log "  Cleared per-peer VTI SAs on gr-fw01 (remote/stale side)"
  else
    # GR rebooted → clear NL VTI peers (remote side has stale SAs)
    local clear_cmds=""
    for peer in $VTI_PEERS_NL; do
      clear_cmds="${clear_cmds}clear crypto ipsec sa peer ${peer}
"
    done
    ssh -o HostKeyAlgorithms=+ssh-rsa -o PubkeyAcceptedAlgorithms=+ssh-rsa \
      -o ConnectTimeout=15 -o StrictHostKeyChecking=no \
      "$ASA_USER@$NL_ASA_IP" "${clear_cmds}" 2>/dev/null
    log "  Cleared per-peer VTI SAs on nl-fw01 (remote/stale side)"
  fi
...
if $TUNNEL_OK; then
  ...
  MSG="🔴 [Post-Reboot VPN Check] $DEVICE — Cross-site tunnel STALE after $MAX_RETRIES attempts ($DIRECTION). $SUBNET_FAILURES subnet pair(s) unreachable. Services at risk: Galera, ClusterMesh, SeaweedFS, alert pipeline. Manual: SSH to remote ASA, run 'clear crypto ipsec sa peer <PEER_IP>' for each VTI peer."
