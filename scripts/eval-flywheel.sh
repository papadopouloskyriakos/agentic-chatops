...
if [ -f "$REPO/scripts/eval-sets/holdout.json" ]; then
  # IFRNLLEI01PRD-1085: measure the SEALED holdout set, not regression. Previously
  # this ran --set regression while the overfit detector below also reads the
  # regression pass-rate -> it compared the set against itself (dead detector).
  log "Running holdout measurement via golden-test-suite --set holdout --offline (sealed set)..."
  HOLDOUT_OUTPUT=$(bash "$REPO/scripts/golden-test-suite.sh" --set holdout --offline 2>&1 || true)
  ...
  log "WARNING: No holdout eval set found, skipping holdout measurement"
fi
...
# Auto-refinement: test candidate patches against regression set before deployment
# (IFRNLLEI01PRD-576: automated prompt refinement from eval flywheel)
PATCHES_GENERATED=0
PATCHES_DEPLOYED=0
PATCHES_ROLLED_BACK=0

if [ "$(echo "$IMPROVEMENTS" | python3 -c 'import json,sys; print(len(json.loads(sys.stdin.read())))' 2>/dev/null)" -gt 0 ] 2>/dev/null; then
    log "Auto-refinement: testing generated patches against regression set..."

    # Run offline regression test to establish baseline before applying patches
    BASELINE_PASS=$(bash "$REPO/scripts/golden-test-suite.sh" --set regression --offline --quiet 2>/dev/null | grep -oP 'PASS: \K\d+' || echo "0")
    log "Regression baseline: $BASELINE_PASS tests passing"

    # Count patches applied by prompt-improver
    PATCHES_GENERATED=$(echo "$IMPROVEMENTS" | python3 -c 'import json,sys; print(len(json.loads(sys.stdin.read())))' 2>/dev/null || echo "0")
    PATCHES_DEPLOYED=$PATCHES_GENERATED

    # Run regression again after patches
    POST_PASS=$(bash "$REPO/scripts/golden-test-suite.sh" --set regression --offline --quiet 2>/dev/null | grep -oP 'PASS: \K\d+' || echo "0")
    log "Regression after patches: $POST_PASS tests passing"

    # Rollback if regression detected (fewer tests passing after patch)
    if [ "${POST_PASS:-0}" -lt "${BASELINE_PASS:-0}" ]; then
        log "REGRESSION DETECTED after prompt patch: $POST_PASS < $BASELINE_PASS — rolling back"
        python3 "$REPO/scripts/prompt-improver.py" --rollback 2>/dev/null || log "WARN: rollback failed"
        PATCHES_ROLLED_BACK=$PATCHES_DEPLOYED
        PATCHES_DEPLOYED=0
    else
        log "Regression test passed: $POST_PASS >= $BASELINE_PASS"
    fi
fi

# Export auto-refinement metrics to Prometheus
PROM_PATCH="/var/lib/node_exporter/textfile_collector/prompt_refinement.prom"
cat > "${PROM_PATCH}.tmp" << PATCHEOF
# HELP prompt_patches_generated Total patches generated this cycle
# TYPE prompt_patches_generated gauge
prompt_patches_generated $PATCHES_GENERATED
# HELP prompt_patches_deployed Patches that passed regression and were deployed
# TYPE prompt_patches_deployed gauge
prompt_patches_deployed $PATCHES_DEPLOYED
# HELP prompt_patches_rolled_back Patches rolled back due to regression
# TYPE prompt_patches_rolled_back gauge
prompt_patches_rolled_back $PATCHES_ROLLED_BACK
# HELP prompt_flywheel_last_run_timestamp Epoch of last flywheel run
# TYPE prompt_flywheel_last_run_timestamp gauge
prompt_flywheel_last_run_timestamp $(date +%s)
PATCHEOF
mv "${PROM_PATCH}.tmp" "$PROM_PATCH" 2>/dev/null || true

log "Auto-refinement complete: generated=$PATCHES_GENERATED deployed=$PATCHES_DEPLOYED rolled_back=$PATCHES_ROLLED_BACK"
