...
def _notify_matrix(message, room=None):
    ...
    txn_id = f"chaos-{int(_time.time())}-{os.getpid()}"
...
# Tunnel → ASA mapping (tunnels available for chaos testing)
CHAOS_TUNNELS = {
    # xs4all tunnels (dormant standby when Freedom is up — low-impact tests)
    ...
    # Freedom tunnels (PRIMARY active path — real failover tests)
    ("NL ↔ GR", "freedom"):  {"asa": "nl", "interface": "Tunnel4", "failover_via": "xs4all VTI (BGP LP 150)"},
    ("NL ↔ NO", "freedom"):  {"asa": "nl", "interface": "Tunnel5", "failover_via": "xs4all VTI (BGP LP 150)"},
    ("NL ↔ CH", "freedom"):  {"asa": "nl", "interface": "Tunnel6", "failover_via": "xs4all VTI (BGP LP 150)"},
...
SAFETY_DEPS = {
    ("NL ↔ GR", "xs4all"): [("NL ↔ GR", "freedom")],
    ("NL ↔ NO", "xs4all"): [("NL ↔ NO", "freedom")],
    ("NL ↔ CH", "xs4all"): [("NL ↔ CH", "freedom")],
    ("NL ↔ GR", "freedom"): [("NL ↔ GR", "xs4all")],
    ("NL ↔ NO", "freedom"): [("NL ↔ NO", "xs4all")],
    ("NL ↔ CH", "freedom"): [("NL ↔ CH", "xs4all")],
    ("GR ↔ NO", "inalan"): [],
    ("GR ↔ CH", "inalan"): [],
    ("NO ↔ CH", "vps"):   [],
}
...
def schedule_deadman_recovery(target_type, target_id, delay_seconds):
    ...
    # Belt + suspenders: setsid + nohup + start_new_session to survive SSH close.
    # Backup: per-minute watchdog cron (chaos-orphan-recovery.sh) catches any failure.
    proc = subprocess.Popen(
        ["setsid", "nohup", "bash", "-c",
         f"trap '' HUP; sleep {delay_seconds} && CHAOS_INTERNAL_RECOVER=1 python3 {recover_script} recover"],
...
def _cmd_start_locked(args):
    ...
    _notify_matrix(
        f"[Chaos] Experiment started: {target_desc} | "
...
def cmd_recover(args):
    ...
    # R2: Notify Matrix at experiment end
    convergence = verdict_details.get("convergence_seconds", "N/A") if isinstance(verdict_details, dict) else "N/A"
    _notify_matrix(
        f"[Chaos] Experiment {experiment_id} completed: {verdict} | "
        f"Convergence: {convergence}s | Recovery: {'OK' if not verify_failures else 'FAILED'}"
    )
