...
def verify_turnstile(token):
    """Verify a Cloudflare Turnstile token. Returns (ok: bool, detail: str).

    IFRNLLEI01PRD-1569: this used to collapse every distinct failure — invalid-input-secret,
    invalid-input-response, timeout-or-duplicate, missing-input-response, network errors — into a
    bare False and never logged Cloudflare's error-codes, which is exactly why the failure stayed a
    3-month mystery. It now returns + logs the specific Cloudflare error-codes so a single real
    visitor run names the cause deterministically. Verified 2026-07-08: the secret is VALID
    (a dummy-token probe returns invalid-input-response, not invalid-input-secret), so the live
    failure is a TOKEN-lifecycle issue (empty / expired / duplicate) on the frontend, not a secret
    mismatch. `detail` is surfaced in the gate's error JSON so the browser shows why."""
    ...
    if not token:
        return False, "missing-input-response (no token reached the backend)"
    ...
    try:
        ...
        detail = ",".join(codes)
        print(json.dumps({"event": "turnstile_denied", "error_codes": codes,
                          "hostname": result.get("hostname"), "challenge_ts": result.get("challenge_ts")}),
              file=sys.stderr)
        return False, detail
    except Exception as e:  # noqa: BLE001 — Cloudflare unreachable => deny, but say so
        print(json.dumps({"event": "turnstile_error", "detail": f"{type(e).__name__}:{e}"}),
              file=sys.stderr)
...
CHAOS_TUNNELS = {
    ...
    # 2026-05-05: NO-DMZ pair (notrf01dmz01 + notrf01dmz02 — public-IP Docker hosts at Gigahost,
    # see edge/CLAUDE.md "Hybrid public-IP DMZ pair"). Each dmz host terminates 6 tunnels — 5
    # outward (NL freedom, NL budget via rtr01, GR, CH-VPS, NO-VPS) + 1 inter-dmz `pair`. Chaos
    # surface is the outward tunnels (the `pair` link is the SINGLE point of replication
    # failure between the active/active SaaS halves — chaosable but breaks state replication).
    ("NL ↔ NO-DMZ01", "freedom"): {"asa": "nl",  "interface": "Tunnel7", "failover_via": "Budget VTI (NL rtr01) Tunnel4"},
    ("NL ↔ NO-DMZ02", "freedom"): {"asa": "nl",  "interface": "Tunnel8", "failover_via": "Budget VTI (NL rtr01) Tunnel5"},
    ("NL ↔ NO-DMZ01", "budget"):  {"asa": "rtr", "interface": "Tunnel4", "failover_via": "Freedom VTI (NL fw01) Tunnel7"},
    ("NL ↔ NO-DMZ02", "budget"):  {"asa": "rtr", "interface": "Tunnel5", "failover_via": "Freedom VTI (NL fw01) Tunnel8"},
    ("GR ↔ NO-DMZ01", "inalan"):  {"asa": "gr",  "interface": "Tunnel5", "failover_via": "NL transit (GR→NL→NO-DMZ01)"},
    ("GR ↔ NO-DMZ02", "inalan"):  {"asa": "gr",  "interface": "Tunnel6", "failover_via": "NL transit (GR→NL→NO-DMZ02)"},
    # 2026-05-06: txhou01vps01 (Houston, TX, US) — 7 tunnels. iFog AS34927 IPv6 transit.
    # NL has both Freedom + Budget VTIs to TX (dual-WAN); GR has single VTI.
    # CH/NO/NO-DMZ inter-VPS tunnels are NOT chaosable from the ASA side — they're
    # swanctl-only links between VPSes; recovery via swanctl --load-all on either end.
    ("NL ↔ TX", "freedom"):  {"asa": "nl",  "interface": "Tunnel9", "failover_via": "Budget VTI (NL rtr01) Tunnel6"},
    ("NL ↔ TX", "budget"):   {"asa": "rtr", "interface": "Tunnel6", "failover_via": "Freedom VTI (NL fw01) Tunnel9"},
    ("GR ↔ TX", "inalan"):   {"asa": "gr",  "interface": "Tunnel7", "failover_via": "NL transit (GR→NL→TX)"},
}

# Graph connectivity validator — replaces simple SAFETY_DEPS
# The mesh has 5 sites (NL, GR, NO, CH, TX), 27 tunnels, 8 chaosable edges
# (post txhou01vps01 onboarding 2026-05-06: 6 edges in the original 4-site mesh + 2 TX edges;
#  NL-TX and GR-TX. The 8 logical edges yield C(8,1..4)=162 kill combinations — see the
#  portfolio-site writeup, which recomputes 127 safe / 35 blocked / 44 four-kill survivors).
# NO↔CH IS chaosable (XFRM issues fixed: IFRNLLEI01PRD-466, swanctl --load-all recovery).
# For websites to survive: each VPS (NO, CH, TX) must reach at least one DMZ site (NL or GR).
#
...
TUNNEL_GRAPH_EDGE = {
    ...
    ("GR ↔ CH", "inalan"):  ("GR", "CH"),
    ("NO ↔ CH", "vps"):     ("NO", "CH"),
    # 2026-05-05: NO-DMZ pair edges. The dmz pair are CHILDREN of the NO physical
    # site (parent: notrf01vps01) — see vpn-mesh-stats.py get_dmz_status() and
    # the existing NL-DMZ/GR-DMZ pattern. Graph reachability collapses to ("X", "NO")
    # because the chaos safety BFS treats sites as monolithic nodes.
    ("NL ↔ NO-DMZ01", "budget"):  ("NL", "NO"),
    ("NL ↔ NO-DMZ01", "freedom"): ("NL", "NO"),
    ("GR ↔ NO-DMZ01", "inalan"):  ("GR", "NO"),
    ("NL ↔ NO-DMZ02", "budget"):  ("NL", "NO"),
    ("NL ↔ NO-DMZ02", "freedom"): ("NL", "NO"),
    ("GR ↔ NO-DMZ02", "inalan"):  ("GR", "NO"),
    # 2026-05-06: TX-VPS edges (txhou01vps01).
    ("NL ↔ TX", "freedom"): ("NL", "TX"),
    ("NL ↔ TX", "budget"):  ("NL", "TX"),
    ("GR ↔ TX", "inalan"):  ("GR", "TX"),
}
...
def validate_graph_connectivity(tunnel_keys):
    ...
    tx_reach = reachable("TX")
    no_ok = "NL" in no_reach or "GR" in no_reach
    ch_ok = "NL" in ch_reach or "GR" in ch_reach
    tx_ok = "NL" in tx_reach or "GR" in tx_reach

    if not no_ok and not ch_ok and not tx_ok:
        return False, "All three VPS nodes isolated from DMZ sites -- total website outage"
    if not no_ok:
    ...
    if not tx_ok:
        return False, "TX VPS isolated from all DMZ sites -- partial website outage"
...
DMZ_HOSTS = {
    ...
    # 2026-05-05: public-IP DMZ pair at Gigahost (no PVE — provider-hosted VMs).
    # Both children of the NO physical site (parent: notrf01vps01), matching the
    # NL/GR pattern where the DMZ shares its parent country's site label.
    "notrf01dmz01": {
        "site": "NO", "ssh": "direct",
        "public_ip": "193.200.238.138", "overlay": "10.255.X.X",
        "pve_host": None, "vmid": None,
        "net0": None,  # link-level chaos via Gigahost portal API not yet implemented
        "note": "Active/active HA pair with notrf01dmz02. SaaS deploy still pending.",
    },
    "notrf01dmz02": {
        "site": "NO", "ssh": "direct",
        "public_ip": "193.200.238.139", "overlay": "10.255.X.X",
        "pve_host": None, "vmid": None,
        "net0": None,
        "note": "Active/active HA pair with notrf01dmz01.",
    },
}
...
def _cmd_start_locked(args):
    ...
    # Verify Cloudflare Turnstile token (bot protection). IFRNLLEI01PRD-1569: the secret is valid
    # (probed 2026-07-08) — a failure here is a frontend token-lifecycle issue (empty/expired/
    # duplicate). The specific Cloudflare error-code is now surfaced so it is diagnosable per-run.
    ...
    if not os.environ.get("CHAOS_SKIP_TURNSTILE", "").lower() == "true":
        ...
        if not token:
            print(json.dumps({"error": "Turnstile token required — solve the challenge and retry.",
                              "turnstile_error": "missing-input-response"}))
            sys.exit(0)
        ok, detail = verify_turnstile(token)
        if not ok:
            hint = ("Challenge expired or already used — it refreshes; solve it again and click within ~30s."
                    if "timeout-or-duplicate" in detail or "duplicate" in detail
                    else "Solve the challenge and retry.")
            print(json.dumps({"error": f"Turnstile verification failed ({detail}). {hint}",
                              "turnstile_error": detail}))
            sys.exit(0)
    ...
    # Defensive: refuse a `chaos_type=tunnel|combined` run that didn't yield any
    # CHAOS_TUNNELS-resolvable targets. Without this check, mismatched payloads
    # (canonical-order bug, inter-VPS swanctl tunnels not in CHAOS_TUNNELS) would
    # silently produce a 600s "active" run with empty `tunnels_killed`, the
    # frontend's pre-call state.tunnelsKilled would still paint red briefly, and
    # the kill subprocess would later sys.exit(1) — net effect is a no-op chaos
    # run with a misleading green dashboard. Observed live 2026-05-06 19:13.
    if chaos_type in ("tunnel", "combined") and tunnel_keys and not tunnel_infos:
        print(json.dumps({
            "error": ("None of the requested tunnels are in CHAOS_TUNNELS. "
                      "Likely a canonical-order mismatch (e.g. 'NO-DMZ01 ↔ NL' instead of "
                      "'NL ↔ NO-DMZ01') or an inter-VPS swanctl tunnel that's not exposed "
                      "via the ASA chaos surface."),
    ...
    # IFRNLLEI01PRD-1039/-1041: every chaos drill is a LABELED experiment —
    # record the infragraph cascade prediction for each killed tunnel at kill
    # time so the hourly eval scores it against the real alert stream. This
    # is the ground-truth evidence generator for the -1040 gate (chaos-killed
    # alerts get tier1-suppressed before triage Step 2-graph, so the drill
    # itself must record). Fail-open: a broken graph never delays a kill.
    try:
        for _t in tunnel_infos:
            _tname = "tunnel:{}:{}".format(
                _t["tunnel"].replace(" ", "").replace("↔", "-"), _t["wan"])
            subprocess.Popen(
                ["python3",
                 os.path.join(os.path.dirname(os.path.abspath(__file__)),
                              "infragraph-query.py"),
                 "cascade", "--host", _tname, "--rule", "chaos-kill",
                 "--record", "--issue", experiment_id],  # id already carries the chaos- prefix
                start_new_session=True,
