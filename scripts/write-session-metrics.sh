...
###############################################################################
# PER-MODEL COST (from session_log.model, 30d)
###############################################################################
echo "# HELP chatops_cost_by_model Total cost by model (30d)" >> "$TMPOUT"
echo "# TYPE chatops_cost_by_model gauge" >> "$TMPOUT"
echo "# HELP chatops_sessions_by_model Session count by model (30d)" >> "$TMPOUT"
echo "# TYPE chatops_sessions_by_model gauge" >> "$TMPOUT"
sqlite3 "$DB" "SELECT model, ROUND(SUM(cost_usd),4), COUNT(*)
FROM session_log
WHERE model != '' AND model IS NOT NULL
  AND started_at > datetime('now', '-30 days')
GROUP BY model;" 2>/dev/null | \
while IFS='|' read -r model cost count; do
    [ -z "$model" ] && continue
    echo "chatops_cost_by_model{model=\"$model\"} $cost"
    echo "chatops_sessions_by_model{model=\"$model\"} $count"
done >> "$TMPOUT"
