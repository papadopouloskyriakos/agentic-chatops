...
def _load_repo_env():
    """Load KEY=VALUE pairs from the repo .env (gitignored) into os.environ.
    Stdlib only (no python-dotenv dep). Existing env wins, so an explicit
    export still overrides .env. Lets cron/SSH invocations pick up secrets
    without baking them into source. (IFRNLLEI01PRD-1082, 2026-06-16)"""
...
def _resolve_otlp_auth():
    """OTLP Basic auth — the secret is NEVER hardcoded in source (it would be
    committed and mirrored to a public GitHub repo). Resolution order:
      1. OTLP_AUTH (full 'Basic <b64>' value, from .env or explicit export)
      2. built from OPENOBSERVE_USER + OPENOBSERVE_TOKEN
      3. '' -> trace export is skipped + logged, never a silent auth failure.
    (IFRNLLEI01PRD-1082, 2026-06-16)"""
    auth = os.environ.get("OTLP_AUTH", "").strip()
    if auth:
        return auth
    user = os.environ.get("OPENOBSERVE_USER", "").strip()
    token = os.environ.get("OPENOBSERVE_TOKEN", "").strip()
    if user and token:
        return "Basic " + base64.b64encode(f"{user}:{token}".encode()).decode()
    return ""


OTLP_AUTH = _resolve_otlp_auth()
# Local trace storage table (fallback when OpenObserve is unreachable)
TRACE_TABLE = "otel_spans"


def generate_trace_id(issue_id: str, salt: str = "") -> str:
    """Deterministic 32-hex-char trace ID. Salt with the session_id so a
    re-triaged / reprocessed / reopened issue produces a DISTINCT trace instead
    of merging (and, with random span_ids, duplicating spans) into one
    (IFRNLLEI01PRD-1095)."""
    key = f"{issue_id}:{salt}" if salt else issue_id
...
def events_to_spans(events: list, trace_id: str) -> list:
    ...
    result_event = None
    tool_calls = []
    user_prompt = ""   # first user message text (IFRNLLEI01PRD-1095: content in trace)
    ...
    for event in events:
        ...
        elif event_type == "user" and not user_prompt:
            # capture the first user prompt text for the trace (content debugging)
            c = event.get("message", {}).get("content", "")
            if isinstance(c, list):
                c = " ".join(p.get("text", "") for p in c
                             if isinstance(p, dict) and p.get("type") == "text")
            user_prompt = (c if isinstance(c, str) else "")[:2000]
    ...
    root_attrs = {
        ...
        # Content in the trace so a faulty step is debuggable (IFRNLLEI01PRD-1095):
        "gen_ai.prompt": user_prompt,
        "gen_ai.completion": (result_event.get("result", "") if result_event else "")[:2000],
    }
    ...
    # Tool call spans — approximate duration from the next tool's start (or the
    # session end for the last tool), so spans aren't zero-duration (IFRNLLEI01PRD-1095).
    for i, tc in enumerate(tool_calls):
        tc_end = tool_calls[i + 1]["timestamp"] if i + 1 < len(tool_calls) else end_ts
        spans.append({
            "traceId": trace_id,
            "spanId": generate_span_id(),
            "parentSpanId": root_span_id,
            ...
            "endTime": tc_end or tc.get("timestamp", ""),
            "attributes": {
                "tool.name": tc["name"],
...
def _mark_trace_exported(trace_id: str):
    """Mark a trace's locally-stored spans as exported (=1) after a successful immediate OTLP push,
    so the */5 flush cron does not re-submit them. Pairs with the always-store-locally change."""
    import sqlite3
    try:
        conn = sqlite3.connect(DB_PATH, timeout=30)
        conn.execute("PRAGMA busy_timeout=30000")
        conn.execute(f"UPDATE {TRACE_TABLE} SET exported_to_otlp = 1 WHERE trace_id = ?", (trace_id,))
        conn.commit()
        conn.close()
    except Exception:
        pass
...
def process_and_export(path: str, issue_id: str, do_otlp: bool = False) -> dict:
    ...
    # Collision-safe trace_id: salt with the session_id so re-triage doesn't merge
    # distinct runs of the same issue into one trace (IFRNLLEI01PRD-1095).
    _sid = ""
    for _e in events:
        if _e.get("type") == "system" and _e.get("subtype") == "init":
            _sid = _e.get("session_id", "")
            break
    trace_id = generate_trace_id(issue_id, _sid)
    ...
    if spans:
        # ALWAYS store locally first — the record + the */5 retry source. The old code skipped the
        # store on immediate-OTLP success, so a successful --otlp push left otel_spans empty and the
        # table "went dark". Now: store, then (if --otlp) push FRESH at session-end, then mark exported
        # so the cron doesn't re-push. Fresh push keeps spans inside OpenObserve's ~5h ingest window
        # (bench IFRNLLEI01PRD-1422: the deferred store-only flush aged spans out -> 99.9% rejected).
        stored_locally = store_spans_locally(spans, issue_id)
        if do_otlp:
            otlp_data = convert_to_otlp(spans)
            exported = submit_otlp(otlp_data)
            if exported:
                _mark_trace_exported(trace_id)
...
def export_unexported_spans(batch_size: int = 512) -> dict:
    ...
    """
    import sqlite3
    stats = {"total": 0, "exported": 0, "failed": 0, "already_done": 0, "expired": 0}
    ...
    # Retention pre-filter (bench IFRNLLEI01PRD-1422, 2026-06-26): OpenObserve rejects spans
    # older than its ingest window (ZO_INGEST_ALLOWED_UPTO, default ~5h) with HTTP 206. Pushing
    # them is doomed and they'd retry every */5 forever, so mark spans older than 4h as expired
    # (state 2) and only push fresh ones. (The historical backlog accrued while OTLP_AUTH was
    # stale — root cause fixed 2026-06-26; this stops the un-exportable tail from churning.)
    import time as _time
    _cutoff_ns = int((_time.time() - 4 * 3600) * 1_000_000_000)
    _expired = [(r["trace_id"], r["span_id"]) for r in rows
                if (iso_to_nanos(r["start_time"] or "") or 0) < _cutoff_ns]
    if _expired:
        for tid, sid in _expired:
            conn.execute(
                f"UPDATE {TRACE_TABLE} SET exported_to_otlp = 2 WHERE trace_id = ? AND span_id = ?",
                (tid, sid))
        conn.commit()
        stats["expired"] = len(_expired)
    rows = [r for r in rows if (iso_to_nanos(r["start_time"] or "") or 0) >= _cutoff_ns]
    if not rows:
        conn.close()
        return stats
