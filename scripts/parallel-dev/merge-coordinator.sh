...
#
# STATUS (IFRNLLEI01PRD-1571 / -1570, reviewed 2026-07-04): this is INTENTIONALLY
# deterministic-only. The whole parallel-dev subsystem is DORMANT — the planner /
# ci-resume n8n workflows are active:false with triggerCount:0 — so nothing exercises
# the two scaffold TODOs below (the LLM-assist reconcile branch at "MERGE FAILED",
# and PROJECT_ID hardcoded to 27). They are deferred-by-design, not bugs. Revive path
# when parallel-dev is reactivated: (a) wire the reconcile branch to a `claude -p`
# session with conflict context, (b) derive PROJECT_ID from the repo origin URL via a
# GitLab project-search API call instead of the hardcoded meshsat id.
#
# Flow:
