...
def main() -> int:
    ...
    # MUTATIONS=OFF shadow: promoting a prompt variant changes future dispatched-session behavior —
    # a live-config actuation. Force dry-run (all `if not args.dry_run` writes skip) and log.
    if mutation_mode and mutation_mode.is_shadow():
        mutation_mode.log_wouldve("prompt-trial-finalize", rationale="would promote/abort prompt-patch trials (live config)")
        args.dry_run = True
