...
def cmd_rollback():
    """Revert the most recently applied active patch(es): deactivate the patch(es)
    with the latest applied_at. The eval-flywheel calls this when the SEALED
    holdout regresses after a patch ships, so a bad patch is undone instead of the
    call being a no-op (IFRNLLEI01PRD-1079 — was a phantom mode)."""
    patches = load_patches()
    active = [p for p in patches if p.get('active', False) and p.get('applied_at')]
    if not active:
        print('No active patches with applied_at to roll back.')
        return
    latest = max(p['applied_at'] for p in active)
    rolled = 0
    for p in patches:
        if p.get('active', False) and p.get('applied_at') == latest:
            p['active'] = False
            p['rolled_back_at'] = datetime.now(timezone.utc).isoformat()
            rolled += 1
            log(f'{p.get("dimension","?")}: rolled back (applied {p["applied_at"]})')
    if rolled:
        save_patches(patches)
        print(f'Rolled back {rolled} patch(es) applied at {latest}.')
    else:
...
def main():
    if len(sys.argv) < 2:
        print('Usage: prompt-improver.py [--analyze|--apply|--report|--promote|--expire|--rollback]')
        sys.exit(1)
