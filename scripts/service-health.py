...
(CrowdSec, ClusterMesh, Ollama, OpenClaw, n8n, Matrix, YouTrack)
from Prometheus and direct HTTP probes.
...
def gatus_fetch(url=None):
    """Fetch all endpoint statuses from a Gatus API instance."""
    try:
...
def get_supplementary_checks():
    ...
    # OpenClaw
    up, rt = http_check("https://openclaw.example.net/health", timeout=8)
    checks.append({
        "name": "OpenClaw (GPT-5.1)",
        "group": "AI & Automation",
        "up": up,
        "response_time_ms": rt,
        "site": "NL",
    ...
    for r in prom_query('up{job="crowdsec"}'):
        if "nl" in r["metric"].get("instance", ""):
    ...
    for r in prom_query('up{job="crowdsec"}'):
        if "gr" in r["metric"].get("instance", ""):
...
def main():
    ...
    supp_gr_urls = [
        ...
        "https://openclaw.example.net/health",
    ...
    supp_gr_name_map = {
        ...
        "openclaw.example.net": "OpenClaw (GPT-5.1)",
    ...
    for name in list(nl_gatus.keys()):
        if nl_gatus[name]["rt_ms"] > 5000 and nl_gatus[name]["up"] is False:
            ...
            if "Sandefjord" in name:
                gatus_url = "https://185.125.171.172"
            elif "rich" in name:
                gatus_url = "https://185.44.82.32"
            if gatus_url:
    ...
    # Merge CrowdSec into one entry with both columns
    crowdsec_nl = None
    crowdsec_gr = None
    merged = []
    for svc in services:
        if svc["name"] == "CrowdSec LAPI (NL)":
            crowdsec_nl = svc
        elif svc["name"] == "CrowdSec LAPI (GR)":
            crowdsec_gr = svc
        else:
            merged.append(svc)
    if crowdsec_nl or crowdsec_gr:
        merged.append({
            "name": "CrowdSec LAPI",
            "group": "Security & Detection",
            "up": (crowdsec_nl or {}).get("up", False) and (crowdsec_gr or {}).get("up", False),
            "site": "cross-site",
            "uptime_pct": 100.0,
            "response_time_nl_ms": 0 if crowdsec_nl and crowdsec_nl["up"] else -1,
            "response_time_gr_ms": 0 if crowdsec_gr and crowdsec_gr["up"] else -1,
            "up_from_gr": (crowdsec_gr or {}).get("up", False),
        })
    services = merged
