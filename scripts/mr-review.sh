...
# Verdict may be inline ("Verdict: APPROVE") OR on the line AFTER a "### Verdict" heading — grep is
# line-based so a single Verdict.*APPROVE pattern misses the multi-line form. Search from the Verdict
# heading to end-of-output first, then fall back to the whole review. (Fixes spurious UNKNOWN → POLL.)
VERDICT=$(echo "$REVIEW_RESULT" | sed -n '/[Vv]erdict/,$p' | grep -oP '\b(APPROVE|REQUEST_CHANGES|NEEDS_DISCUSSION)\b' | head -1)
[ -z "$VERDICT" ] && VERDICT=$(echo "$REVIEW_RESULT" | grep -oP '\b(APPROVE|REQUEST_CHANGES|NEEDS_DISCUSSION)\b' | head -1)
