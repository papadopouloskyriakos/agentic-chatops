...
if [ "$b_int" = "0" ] && [ "$f_int" = "0" ] && [ "$current_state" != "dual-fail" ]; then
    ...
    # ULTRA-urgent (site isolation) per operator decision 2026-08-25: SMS + ntfy push
    ...
    ntfy_publish "[nl] BUDGET+FREEDOM BOTH DOWN" "Total internet loss. Budget PPPoE (nlrtr01 Dialer1) unavailable AND Freedom PPPoE (nl-fw01 outside_freedom) unavailable. Check nlrtr01 + nl-fw01 + ONTs." 5 "rotating_light,nl"
    echo "dual-fail" > "$STATE_FILE"
elif [ "$b_int" = "1" ] && [ "$current_state" = "dual-fail" ]; then
    logger "$LOG_TAG Budget recovered from dual-fail"
    # recovery: ntfy only (SMS reserved for the ULTRA dual-fail edge, 2026-08-25)
    ntfy_publish "[nl] BUDGET RECOVERED" "At least one ISP path restored (budget=${budget}, freedom=${freedom})." 3 "white_check_mark,nl"
