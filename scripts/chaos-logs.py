...
# 2026-04-22 [IFRNLLEI01PRD-674]: post xs4all→budget migration, Budget VTIs
# terminate on nlrtr01. Primary log source for budget entries is rtr01;
# fw01 kept in the host list for ~3 months of legacy syslog history.
TUNNEL_LOG_MAP = {
    ("NL ↔ GR", "budget"): {
        "nl_hosts": ["nlrtr01", "nl-fw01"],
        "gr_hosts": ["gr-fw01"],
        "patterns": ["Tunnel1|vti-gr|203.0.113.X|shutdown|no shutdown|line protocol|IKE_SA|CHILD_SA|DPD|BFD"],
    },
    ("NL ↔ NO", "budget"): {
        "nl_hosts": ["nlrtr01", "nl-fw01", "nlstrongswan01"],
        "gr_hosts": [],
        "patterns": ["Tunnel2|vti-no|198.51.100.X|shutdown|no shutdown|line protocol|IKE_SA|CHILD_SA|DPD|BFD"],
    },
    ("NL ↔ CH", "budget"): {
        "nl_hosts": ["nlrtr01", "nl-fw01", "nlstrongswan01"],
        "gr_hosts": [],
        "patterns": ["Tunnel3|vti-ch|198.51.100.X|shutdown|no shutdown|line protocol|IKE_SA|CHILD_SA|DPD|BFD"],
    },
    ("GR ↔ NO", "inalan"): {
        "nl_hosts": [],
        "gr_hosts": ["gr-fw01"],
        "patterns": ["Tunnel2|vti-no|198.51.100.X|shutdown|no shutdown|line protocol|IKE_SA|CHILD_SA|DPD"],
    },
    ("GR ↔ CH", "inalan"): {
        "nl_hosts": [],
        "gr_hosts": ["gr-fw01"],
        "patterns": ["Tunnel3|vti-ch|198.51.100.X|shutdown|no shutdown|line protocol|IKE_SA|CHILD_SA|DPD"],
    },
}
...
def ssh_grep_logs(syslog_host, hostname, date_str, pattern, time_window=None):
    ...
    # Days older than 14 are gzipped by syslog-archive.sh on the syslog host, and some hosts
    # rotate the day file to .log.1 — zgrep reads plain and .gz alike; missing variants are ignored.
    variants = " ".join(f"{log_path}{suffix}" for suffix in ("", ".1", ".gz", ".1.gz"))

    grep_cmd = f"zgrep -hE '{pattern}' {variants} 2>/dev/null"
    if time_window:
...
def main():
    ...
    # Token validation: if active test exists, allow access (logs are non-sensitive
    # syslog/HAProxy entries). Token check only blocks access when no test is active
    # and no recent history exists (prevents random probing).
    try:
        with open(STATE_FILE) as f:
            json.load(f)  # active test exists -- allow without token
    except (FileNotFoundError, json.JSONDecodeError):
        # No active test -- require token for historical access
        if not args.token:
            print(json.dumps({"status": "unauthorized", "message": "No active test", "logs": []}))
            return
    ...
    if containers_killed:
        ...
        for dmz_host in dmz_hosts_to_check:
            ...
            try:
                if dmz_host not in ("nl-dmz01", "gr-dmz01"):
                    continue
    ...
    # Fetch VPS logs directly via SSH (journal: charon/IKE, FRR/BGP, HAProxy)
    vps_hosts = {
        "198.51.100.X": "notrf01vps01",
        "198.51.100.X": "chzrh01vps01",
    }
    pw = get_asa_password()
    for vps_ip, vps_name in vps_hosts.items():
        try:
            # charon + FRR logs (tunnel chaos)
            result = subprocess.run(
        ...
        # HAProxy backend failover logs (DMZ chaos -- shows health check failures + backup activation)
        if containers_killed:
            try:
                result = subprocess.run(
                    ["ssh", "-o", "StrictHostKeyChecking=accept-new", "-o", "ConnectTimeout=5",
                     "-i", os.path.expanduser("~/.ssh/one_key"),
                     f"operator@{vps_ip}",
                     f"echo '{pw}' | sudo -S journalctl -u haproxy "
                     f"--since \"{start_dt.strftime('%Y-%m-%d %H:%M')}\" "
                     f"--until \"{(start_dt + datetime.timedelta(minutes=15)).strftime('%Y-%m-%d %H:%M')}\" "
                     f"--no-pager -q 2>/dev/null | "
                     f"grep -iE 'DOWN|UP|backup|NOSRV|Layer[467]|check|portfolio|cubeos|meshsat|mulecube' | "
                     f"tail -20"],
                    capture_output=True, text=True, timeout=15,
                )
                for line in result.stdout.strip().split("\n"):
                    line = line.strip()
                    if line:
                        all_logs.append({"source": f"{vps_name}-haproxy", "raw": sanitize_line(line)})
            except Exception:
                pass
