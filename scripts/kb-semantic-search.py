...
from rag_config import (
    OLLAMA_URL, OLLAMA_GEN_URL, RERANK_API_URL,
    EMBED_TABLES,
...
# IFRNLLEI01PRD-1739: the synth stage was the 15-17s p95 excursion driver — a
# 400-token generation over 10 docs on the contended gpu01 blows the ~12s SLO
# whenever it triggers. 256 tokens / 8 docs keeps weak-match totals inside the
# budget; restore the old shape via env if synthesis quality regresses.
SYNTH_NUM_PREDICT = int(os.environ.get("SYNTH_NUM_PREDICT", "256"))
SYNTH_TOP_N = int(os.environ.get("SYNTH_TOP_N", "8"))
...
def _embed_raw(texts):
    ...
    payload = json.dumps({"model": EMBED_MODEL, "input": payload_texts, "keep_alive": OLLAMA_KEEP_ALIVE}).encode()
...
def cosine_similarity(a, b):
    ...
    # Dimension guard: a model swap or a truncated/corrupt vector must score 0,
    # never silently mis-score via zip() truncation (IFRNLLEI01PRD-1091).
    if len(a) != len(b):
        return 0.0
...
# IFRNLLEI01PRD-1739: keep the RAG hot models resident between the */5 latency
# probe and operator searches — gpu01 VRAM pressure (milvus GPU restore
# 2026-07-07) makes ollama evict them, and every search then pays a 4-8s model
# reload before a sub-second generation. Ollama's LRU eviction under genuine
# VRAM pressure still overrides this (safe).
OLLAMA_KEEP_ALIVE = os.environ.get("RAG_OLLAMA_KEEP_ALIVE", "30m")
...
# NOTE: REWRITE_MODEL is defined once near the top (default qwen2.5:7b). A second
# definition here previously shadowed it to llama3.2:1b, silently downgrading
# HyDE / RAG-Fusion query rewriting to a 1B model (IFRNLLEI01PRD-1081, 2026-06-16).
...
def _synth_fresh_candidates(conn, query, limit=10):
    ...
    try:
        ik_rows = conn.execute(
            ...
            # IFRNLLEI01PRD-1153: governance demotion markers are not knowledge — never retrieve them.
            "AND COALESCE(project,'') != 'chatops-governance'"
        ).fetchall()
        for r in ik_rows:
            try:
                v = json.loads(r["embedding"])
...
def synthesize_answer(query, rerank_pool, top_n=10, conn=None):
    ...
    def _call_qwen(prompt_text):
        ...
        body = {
            ...
            "stream": False,
            "keep_alive": OLLAMA_KEEP_ALIVE,
            "options": {"temperature": 0.0, "num_predict": SYNTH_NUM_PREDICT, "num_ctx": NUM_CTX_MED},
        }
        req = urllib.request.Request(
            f"{OLLAMA_GEN_URL}/api/generate",
            data=json.dumps(body).encode(),
    ...
    # IFRNLLEI01PRD-1446: faithfulness/citation-validity oracle.
    # The synth sentence carries a high (0.95) confidence tag downstream, so before
    # emitting it we verify every [N] citation marker resolves to a REAL retrieved
    # chunk (1 <= N <= len(chunks)). A sentence citing a non-existent chunk (e.g. [99])
    # is an ungrounded claim — drop it rather than emit a confident-but-fabricated answer.
    # Mirrors quiz_generator.py's hallucination gate (reject if cited content unsupported).
    # FAIL-SAFE: any error here returns None so the caller degrades to raw chunks.
    try:
        n_chunks = len(chunks)
        cited = re.findall(r"\[(\d+)\]", winner)
        # A synth answer with citations must have at least one, and every one must
        # index a real chunk. No citations at all = ungrounded composed claim -> drop.
        if not cited:
            print("[synth] citation oracle: winner has no [N] citations — dropping ungrounded synth", file=sys.stderr)
            return None
        for c in cited:
            n = int(c)
            if n < 1 or n > n_chunks:
                print(f"[synth] citation oracle: invalid citation [{n}] (only {n_chunks} chunks) — dropping ungrounded synth", file=sys.stderr)
                return None
    except Exception as e:
        print(f"[synth] citation oracle errored ({e}) — dropping synth (fail-safe)", file=sys.stderr)
        return None
...
def cmd_embed(backfill=False):
    ...
    if backfill:
        rows = conn.execute(
            ...
            # IFRNLLEI01PRD-1153: don't embed governance demotion markers (not knowledge).
            "AND COALESCE(project,'') != 'chatops-governance'"
        ).fetchall()
    else:
...
def search_diary(conn, query_vec, limit=5):
    """6th RRF signal: agent_diary — MemPalace per-agent curated takeaways (written by the
    SubagentStop hook, scripts/hooks/mempal-subagent-diary.sh). Tiny table, so a brute-force
    embedding scan is fine. Rows are shaped like session_transcripts rows so the existing
    transcript-source handling applies downstream (no formatter/RRF changes needed).
    GRACEFUL: returns {} on any error — must never break retrieval."""
    if not query_vec:
        return {}
    try:
        rows = conn.execute(
            "SELECT id, agent_name, issue_id, entry, embedding, created_at "
            "FROM agent_diary WHERE embedding IS NOT NULL AND embedding != ''"
        ).fetchall()
    except Exception:
        return {}
    scored = []
    for row in rows:
        ...
        if sim > 0.3:
            drow = {
                "id": row["id"],
                "issue_id": row["issue_id"] or ("diary-" + str(row["agent_name"])),
                "chunk_index": 0,
                "role": "assistant",
                "content": "[" + str(row["agent_name"]) + " diary] " + (row["entry"] or ""),
                "embedding": "",
                "created_at": row["created_at"],
            }
            scored.append((sim, drow))
    scored.sort(key=lambda x: x[0], reverse=True)
    result = {}
    for rank, (sim, drow) in enumerate(scored[:limit], 1):
        result["diary:" + str(drow["id"])] = {"rank": rank, "sim": sim, "row": drow}
    return result
...
def _faiss_topk_ids(table, query_vecs, k=40):
    """ANN candidate-id lookup against the warm HNSW index built by
    faiss-index-sync.py (IFRNLLEI01PRD-1086). Returns (set_of_pks, synced_at_iso),
    or None to mean 'fall back to brute force' (kill-switch RAG_FAISS_READ!=1,
    faiss/numpy missing, index absent, embed-model drift, or any error). The
    caller MUST union the result with a brute-force scan of rows newer than
    synced_at so recent (un-indexed) rows are never missed."""
    if os.environ.get("RAG_FAISS_READ", "1") != "1" or not query_vecs:
        return None
    try:
        import faiss
        import numpy as np
        ...
        return None
    idx_dir = os.environ.get("FAISS_INDEX_DIR", "/var/claude-gateway/vector-indexes")
    idx_path = os.path.join(idx_dir, f"{table}.faiss")
    idmap_path = os.path.join(idx_dir, f"{table}.idmap.json")
    if not (os.path.exists(idx_path) and os.path.exists(idmap_path)):
        return None
    try:
        mtime = os.path.getmtime(idx_path)
        cached = _FAISS_INDEX_CACHE.get(table)
        if cached and cached[0] == mtime:
            index, meta = cached[1], cached[2]
        else:
            with open(idmap_path) as f:
                meta = json.load(f)
            if meta.get("embed_model") and meta["embed_model"] != EMBED_MODEL:
                return None  # embed-model drift -> brute force is safer
            index = faiss.read_index(idx_path)
            _FAISS_INDEX_CACHE[table] = (mtime, index, meta)
        idmap = meta["idmap"]
        ids = set()
        for qv in query_vecs:
            a = np.asarray([qv], dtype=np.float32)
            n = np.linalg.norm(a, axis=1, keepdims=True)
            n[n == 0] = 1.0
            _, I = index.search(a / n, k)
            for pos in I[0]:
                if 0 <= pos < len(idmap):
                    ids.add(idmap[pos]["pk"])
        return ids, meta.get("synced_at", "")
    except Exception as e:
        print(f"[faiss] read failed for {table}; brute-force fallback: {e}", file=sys.stderr)
        return None


def extract_self_query_filters(query):
    """Self-query (IFRNLLEI01PRD-1092): pull hard metadata filters (hostname, site)
    out of the query so retrieval can pre-filter the candidate pool to the relevant
    host instead of relying on the embedding to encode it. Returns a dict with keys
    present only when confidently found. Applied as filter-WITH-fallback so it can
    never drop all results."""
    q = (query or "").lower()
    out = {}
    m = re.search(r'\b([a-z]{2,}[a-z0-9]*[0-9]{2}[a-z0-9]+)\b', q)
    if m:
...
def cmd_hybrid_search(query, limit=5, days=90, threshold=0.3, use_rewrite=False):
    ...
    semantic_ranked = {}
    # IFRNLLEI01PRD-1739: FAISS top-k narrowing, mirroring the IFRNLLEI01PRD-1086
    # session_transcripts pattern — the 4-variant pure-python cosine scan over the
    # full embedded incident pool was a fixed multi-second CPU floor on every
    # search. Union with rows newer than the index sync so fresh incidents are
    # never missed; any FAISS problem falls back to the original full scan.
    _day_clause = "AND created_at > datetime('now', ?) " if days > 0 else ""
    _day_args = (f"-{days} days",) if days > 0 else ()
    _ik_qvecs = [query_vecs_map[q] for q in queries if query_vecs_map.get(q)]
    _ik_faiss = _faiss_topk_ids("incident_knowledge", _ik_qvecs, k=max(80, limit * 16))
    if _ik_faiss is not None and _ik_faiss[0]:
        _ik_ids, _ik_synced = _ik_faiss
        _ph = ",".join("?" * len(_ik_ids))
        rows = conn.execute(
            f"SELECT * FROM incident_knowledge WHERE id IN ({_ph}) "
            f"AND embedding IS NOT NULL AND embedding != '' {_day_clause}{validity}",
            tuple(_ik_ids) + _day_args,
        ).fetchall()
        if _ik_synced:
            rows = list(rows) + conn.execute(
                "SELECT * FROM incident_knowledge "
                f"WHERE embedding IS NOT NULL AND embedding != '' AND created_at > ? {_day_clause}{validity}",
                (_ik_synced,) + _day_args,
            ).fetchall()
    elif days > 0:
        rows = conn.execute(
            "SELECT * FROM incident_knowledge "
    ...
    # Self-query metadata pre-filter (IFRNLLEI01PRD-1092): if the query names a
    # host AND enough host-scoped incidents exist, restrict scoring to them
    # (filtered vector search); fall back to the full pool if too few so we never
    # drop all results.
    _sq = extract_self_query_filters(query)
    if rows and _sq.get("hostname"):
        _hr = [r for r in rows if (r["hostname"] or "").lower() == _sq["hostname"]]
        if len(_hr) >= min(limit, 3):
            rows = _hr
    ...
    try:
        try:
            if temporal_window:
                ...
                # IFRNLLEI01PRD-1739: same FAISS narrowing for the wiki corpus (no
                # newer-rows union: the wiki recompiles daily while the index syncs
                # every 15 min, so the exposure window is at most one sync period;
                # temporal queries above keep the full-scan path).
                _wk_faiss = _faiss_topk_ids("wiki_articles", _ik_qvecs, k=max(80, limit * 16))
                if _wk_faiss is not None and _wk_faiss[0]:
                    _wk_ids, _ = _wk_faiss
                    _ph = ",".join("?" * len(_wk_ids))
                    wiki_rows = conn.execute(
                        "SELECT path, title, section, embedding, content_preview FROM wiki_articles "
                        f"WHERE id IN ({_ph}) AND embedding IS NOT NULL AND embedding != ''",
                        tuple(_wk_ids),
                    ).fetchall()
                else:
                    wiki_rows = conn.execute(
    ...
    transcript_ranked = {}
    try:
        # IFRNLLEI01PRD-1086: use the warm FAISS index to fetch only top candidates
        # instead of scanning all ~16.8k transcript rows; union with a brute-force
        # scan of rows newer than the index so recent rows are never missed.
        # Falls back to the full scan if FAISS is unavailable (kill-switch/missing).
        _dayclause = "AND created_at > datetime('now', ?)" if days > 0 else ""
        _dayargs = (f"-{days} days",) if days > 0 else ()
        _qvecs = [query_vecs_map[q] for q in queries if query_vecs_map.get(q)]
        _faiss = _faiss_topk_ids("session_transcripts", _qvecs, k=max(40, limit * 8))
        if _faiss is not None and _faiss[0]:
            _cand_ids, _synced = _faiss
            _ph = ",".join("?" * len(_cand_ids))
            t_rows_all = conn.execute(
                "SELECT id, issue_id, chunk_index, role, content, embedding, created_at "
                f"FROM session_transcripts WHERE id IN ({_ph}) "
                f"AND embedding IS NOT NULL AND embedding != '' {_dayclause}",
                tuple(_cand_ids) + _dayargs,
            ).fetchall()
            if _synced:
                t_rows_all = list(t_rows_all) + conn.execute(
                    "SELECT id, issue_id, chunk_index, role, content, embedding, created_at "
                    "FROM session_transcripts WHERE embedding IS NOT NULL AND embedding != '' "
                    f"AND created_at > ? {_dayclause}",
                    (_synced,) + _dayargs,
                ).fetchall()
        else:
            t_rows_all = conn.execute(
                "SELECT id, issue_id, chunk_index, role, content, embedding, created_at "
                f"FROM session_transcripts WHERE embedding IS NOT NULL AND embedding != '' {_dayclause}",
                _dayargs,
            ).fetchall()
        for q in queries:
            q_vec = query_vecs_map.get(q)
    ...
    # 6th signal: agent_diary (MemPalace per-agent curated takeaways) — surfaced through the
    # transcript pathway (rows shaped like session_transcripts). Fully guarded so a failure
    # in the diary signal can only ever add nothing, never break retrieval.
    try:
        diary_ranked = search_diary(conn, query_vecs_map.get(query), limit=5)
        for _dkey, ddata in diary_ranked.items():
            dscore = rrf_score(None, None, transcript_rank=ddata["rank"], sem_quality=sem_quality)
            fused.append((dscore, ddata["sim"], ddata["row"], "transcript"))
    except Exception:
        pass  # diary signal must never break the RAG
    ...
    if trigger and _budget_exceeded_hybrid():
        ...
        synthesized = synthesize_answer(query, synthesis_pool, top_n=SYNTH_TOP_N, conn=conn)
        if synthesized:
