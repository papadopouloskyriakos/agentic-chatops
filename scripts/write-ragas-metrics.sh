...
# HELP ragas_answer_relevance_avg Average answer relevance (7d) — IFRNLLEI01PRD-1088
# TYPE ragas_answer_relevance_avg gauge
...
read -r FAITH PREC RECALL ANSREL <<< "$(sqlite3 "$DB" "
    SELECT
        ...
        COALESCE(ROUND(AVG(CASE WHEN answer_relevance >= 0 THEN answer_relevance END), 3), -1)
...

{
    ...
    echo "ragas_answer_relevance_avg ${ANSREL:--1}"
