...
def get_ipsec_tunnels():
    ...
    9 unique tunnels in the mesh (post 2026-04-21 budget migration):
      NL fw01:  3 Freedom VTIs (Tunnel4/5/6) → GR, NO, CH
      NL rtr01: 3 Budget  VTIs (Tunnel1/2/3) → GR, NO, CH
      GR ASA:   2 independent → NO, CH (via outside_inalan)
      VPS:      1 independent → NO ↔ CH (swanctl)

    NL↔GR tunnels are the same tunnel from each end, so we count them once,
    using the NL-side device as the source of truth.
    """
    # NL fw01 tunnel mapping (Freedom only, post-migration):
    #   Tunnel4=vti-gr-f  Tunnel5=vti-no-f  Tunnel6=vti-ch-f
    # NL rtr01 tunnel mapping (Budget, NEW 2026-04-21):
    #   Tunnel1=vti-gr(budget) Tunnel2=vti-no(budget) Tunnel3=vti-ch(budget)
    # GR ASA tunnel mapping:
    #   Tunnel1=vti-nl(budget) — same as NL rtr01 T1, skip
    #   Tunnel2=vti-no(inalan) Tunnel3=vti-ch(inalan)
    #   Tunnel4=vti-nl-f(freedom) — same as NL fw01 T4, skip

    # Query fw01 + rtr01 + GR + NO VPS in parallel-ish (sequential, fast)
    ...
    # 9 unique tunnels — each counted once from the originating device
    arch_tunnels = [
        # NL rtr01 tunnels (3 Budget VTIs, active-active with Freedom)
        ...
        # NL fw01 tunnels (3 Freedom VTIs)
    ...
    for t in arch_tunnels:
        ...
        tunnels.append({
            ...
            "device": {"nl": "nl-fw01", "rtr": "nlrtr01",
                        "gr": "gr-fw01", "no-vps": "notrf01vps01"}.get(t["src"], "?"),
            "latency_ms": None,
            "uptime_hours": None,
        })
