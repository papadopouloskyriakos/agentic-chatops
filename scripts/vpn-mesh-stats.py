...
# 2026-04-22: xs4all → budget rename (IFRNLLEI01PRD-673). Budget PPPoE now
# terminates on nlrtr01 (ISR 4321), not fw01. Labels preserved as
# *-budget for ISP identity; *-rtr01* where the label implies device.
PEER_LABELS = {
    "203.0.113.X": "GR",
    "203.0.113.X": "NL-budget",
    "203.0.113.X": "NL-freedom",
    "198.51.100.X": "NO",
    "198.51.100.X": "CH",
    "185.121.169.27": "TX",
    "10.0.X.X": "NL-rtr01",
    "10.0.X.X": "NL-ASA-budget",
    "10.0.X.X": "NL-ASA",
    "10.0.X.X": "GR-ASA",
    "10.0.X.X": "NL-FRR01",
    "10.0.X.X": "NL-FRR02",
    ...
    "10.255.X.X": "NO-VPS",
    "10.255.X.X": "TX-VPS",
    # NO-DMZ pair (public-IP DMZ Docker hosts at Gigahost, onboarded 2026-05-05)
    "193.200.238.138": "NO-DMZ01",
    "193.200.238.139": "NO-DMZ02",
    "10.255.X.X": "NO-DMZ01-overlay",
    "10.255.X.X": "NO-DMZ02-overlay",
    ...
    "10.0.58.X": "GR-K8s-w2",
    "10.0.58.X": "GR-K8s-w3",
    # VTI direct BGP peers (Budget via rtr01, Freedom via fw01)
    "10.255.200.X": "NL-VTI-budget",
    "10.255.200.X": "GR-VTI-budget",
    "10.255.200.X": "NL-VTI-freedom",
    "10.255.200.X": "GR-VTI-freedom",
    # NL-FRR → VPS transit peers
    "10.255.200.X": "NL-rtr01-VTI",
    "10.255.200.X": "NO-VPS-budget",
    "10.255.200.X": "NL-rtr01-CH-VTI",
    "10.255.200.X": "CH-VPS-budget",
    ...
    # NO-DMZ VTI peer ends (dmz pair onboarded 2026-05-05)
    # rtr01 (xs4all/budget) ↔ NO-DMZ01: .18 (rtr01) / .19 (dmz01)
    "10.255.200.X": "NL-rtr01-NO-DMZ01-VTI",
    "10.255.200.X": "NO-DMZ01-budget",
    # GR ASA ↔ NO-DMZ01: .20 (gr) / .21 (dmz01)
    "10.255.200.X": "GR-ASA-NO-DMZ01-VTI",
    "10.255.200.X": "NO-DMZ01-inalan",
    # CH VPS ↔ NO-DMZ01: .22 (ch) / .23 (dmz01)
    "10.255.200.X": "CH-VPS-NO-DMZ01-VTI",
    "10.255.200.X": "NO-DMZ01-ch",
    # NL fw01 (Freedom) ↔ NO-DMZ01: .24 (nl) / .25 (dmz01)
    "10.255.200.X": "NL-ASA-NO-DMZ01-VTI",
    "10.255.200.X": "NO-DMZ01-freedom",
    # NO-VPS ↔ NO-DMZ01: .26 (no-vps) / .27 (dmz01)
    "10.255.200.X": "NO-VPS-NO-DMZ01-VTI",
    "10.255.200.X": "NO-DMZ01-novps",
    # NO-DMZ01 ↔ NO-DMZ02 inter-host pair: .28 (dmz02) / .29 (dmz01)
    "10.255.200.X": "NO-DMZ02-pair",
    "10.255.200.X": "NO-DMZ01-pair",
    # rtr01 (xs4all/budget) ↔ NO-DMZ02: .30 (rtr01) / .31 (dmz02)
    "10.255.200.X": "NL-rtr01-NO-DMZ02-VTI",
    "10.255.200.X": "NO-DMZ02-budget",
    # GR ASA ↔ NO-DMZ02: .32 (gr) / .33 (dmz02)
    "10.255.200.X": "GR-ASA-NO-DMZ02-VTI",
    "10.255.200.X": "NO-DMZ02-inalan",
    # CH VPS ↔ NO-DMZ02: .34 (ch) / .35 (dmz02)
    "10.255.200.X": "CH-VPS-NO-DMZ02-VTI",
    "10.255.200.X": "NO-DMZ02-ch",
    # NL fw01 (Freedom) ↔ NO-DMZ02: .36 (nl) / .37 (dmz02)
    "10.255.200.X": "NL-ASA-NO-DMZ02-VTI",
    "10.255.200.X": "NO-DMZ02-freedom",
    # NO-VPS ↔ NO-DMZ02: .38 (no-vps) / .39 (dmz02)
    "10.255.200.X": "NO-VPS-NO-DMZ02-VTI",
    "10.255.200.X": "NO-DMZ02-novps",
    # txhou01vps01 VTI peer ends (TX VPS onboarded 2026-05-06; 7 tunnels)
    # rtr01 (NL budget) ↔ TX-VPS: .40 (tx) / .41 (rtr01)
    "10.255.200.X": "TX-VPS-budget",
    "10.255.200.X": "NL-rtr01-TX-VTI",
    # NL fw01 (Freedom) ↔ TX-VPS: .42 (tx) / .43 (nl)
    "10.255.200.X": "TX-VPS-freedom",
    "10.255.200.X": "NL-ASA-TX-VTI",
    # GR fw01 (inalan) ↔ TX-VPS: .44 (tx) / .45 (gr)
    "10.255.200.X": "TX-VPS-inalan",
    "10.255.200.X": "GR-ASA-TX-VTI",
    # CH-VPS ↔ TX-VPS: .46 (tx) / .47 (ch)
    "10.255.200.X": "TX-VPS-ch",
    "10.255.200.X": "CH-VPS-TX-VTI",
    # NO-VPS ↔ TX-VPS: .48 (tx) / .49 (no-vps)
    "10.255.200.X": "TX-VPS-novps",
    "10.255.200.X": "NO-VPS-TX-VTI",
    # NO-DMZ01 ↔ TX-VPS: .50 (tx) / .51 (no-dmz01)
    "10.255.200.X": "TX-VPS-no-dmz01",
    "10.255.200.X": "NO-DMZ01-TX-VTI",
    # NO-DMZ02 ↔ TX-VPS: .52 (tx) / .53 (no-dmz02)
    "10.255.200.X": "TX-VPS-no-dmz02",
    "10.255.200.X": "NO-DMZ02-TX-VTI",
...
# 2026-04-22: rtr01 SSH helper (IFRNLLEI01PRD-673, introduced 2026-04-22)
# Budget VTIs now terminate on nlrtr01, not fw01.
from ios_ssh import ssh_rtr01_command, parse_tunnel_status as parse_rtr_tunnels  # noqa: E402
...
# parse_xs4all_bgp_state() removed 2026-04-22 [IFRNLLEI01PRD-673].
# Pre-migration, xs4all ESP was blocked by BCP38 on outside_freedom, so the
# 3 xs4all VTIs were dormant standby when Freedom was primary. Post-migration
# Budget terminates on nlrtr01 with its own Dialer1 public IP, so BGP
# over the Budget VTIs is fully Established all the time — the "standby"
# reclassification is obsolete and misleading.
...
def get_ipsec_tunnels():
    """Build tunnel list from live ASA + rtr01 + VPS swanctl status via SSH.

    28 tunnels in the mesh (post 2026-05-06 txhou01vps01 onboarding):
      NL fw01:  6 Freedom VTIs (Tunnel4-9) → GR, NO, CH, NO-DMZ01, NO-DMZ02, TX
      NL rtr01: 6 Budget  VTIs (Tunnel1-6) → GR, NO, CH, NO-DMZ01, NO-DMZ02, TX
      GR ASA:   5 independent → NO, CH, NO-DMZ01, NO-DMZ02, TX (via outside_inalan)
      VPS:      1 NO ↔ CH (swanctl)
      NO-DMZ pair: 5 tunnels per dmz host queried via swanctl (overlap counted
                   once from the originator side)
      TX VPS:   1 inter-VPS each to CH + NO + 2 to NO-DMZ pair = 4 tunnels (swanctl)

    NL↔GR tunnels are the same tunnel from each end, counted once using the
    NL-side device as source of truth. Same convention for NL↔NO-DMZ0X / NL↔TX —
    counted once from the NL/rtr/gr endpoint.
    """
    # Query fw01 + rtr01 + GR + 3 VPSes + both NO-DMZ hosts via swanctl
    nl_output = ssh_nl_asa([
        "show interface ip brief | include Tunnel",
        "show track 1",
    ])
    rtr_output = ssh_rtr01_command([
        "show ip interface brief | include ^Tunnel",
    ])
    gr_output = ssh_gr_asa([
        "show interface ip brief | include Tunnel",
    ])
    ch_vps_conns = ssh_vps_swanctl("198.51.100.X")
    no_vps_conns = ssh_vps_swanctl("198.51.100.X")
    # 2026-05-05: also query the new NO-DMZ pair (public-IP Docker hosts at Gigahost)
    # for the inter-dmz `pair` tunnel state. Outward tunnels are already covered by
    # the NL/GR/CH/NO-VPS endpoints above; querying dmz hosts gives us the `pair`
    # link state that no other peer can report (it's strictly between the two dmz hosts).
    no_dmz01_conns = ssh_vps_swanctl("193.200.238.138")
    no_dmz02_conns = ssh_vps_swanctl("193.200.238.139")
    # 2026-05-06: TX-VPS (txhou01vps01) NOT queried directly — iFog FastNetMon null-routes
    # claude01's SSH for handshake flooding (per memory feedback_ssh_multiplex_for_ddos_safety).
    # All 7 TX tunnels are reported from the OTHER side's swanctl (conn name "txhou01"
    # on each peer) or from the Cisco endpoint for NL/GR.
    ...
    rtr_tunnels = parse_rtr_tunnels(rtr_output) if rtr_output and not rtr_output.startswith("ERROR:") else {}
    gr_tunnels = parse_tunnel_status(gr_output)
    freedom_wan = parse_sla_track(nl_output)

    arch_tunnels = [
        # NL rtr01 tunnels (Budget VTIs, active-active with Freedom)
        {"label": "NL ↔ GR",       "type": "direct",  "wan": "budget",  "src": "rtr", "key": 1},
        {"label": "NL ↔ NO",       "type": "transit", "wan": "budget",  "src": "rtr", "key": 2},
        {"label": "NL ↔ CH",       "type": "transit", "wan": "budget",  "src": "rtr", "key": 3},
        # NL rtr01 — NEW NO-DMZ tunnels (added 2026-05-05)
        {"label": "NL ↔ NO-DMZ01", "type": "transit", "wan": "budget",  "src": "rtr", "key": 4},
        {"label": "NL ↔ NO-DMZ02", "type": "transit", "wan": "budget",  "src": "rtr", "key": 5},
        # NL fw01 tunnels (Freedom VTIs)
        ...
        # NL fw01 — NEW NO-DMZ tunnels (added 2026-05-05)
        {"label": "NL ↔ NO-DMZ01", "type": "transit", "wan": "freedom", "src": "nl",  "key": 7},
        {"label": "NL ↔ NO-DMZ02", "type": "transit", "wan": "freedom", "src": "nl",  "key": 8},
        ...
        # GR ASA — NEW NO-DMZ tunnels (added 2026-05-05)
        {"label": "GR ↔ NO-DMZ01", "type": "transit", "wan": "inalan",  "src": "gr",  "key": 5},
        {"label": "GR ↔ NO-DMZ02", "type": "transit", "wan": "inalan",  "src": "gr",  "key": 6},
        # VPS tunnel (1): NO↔CH (swanctl connection "ch" on NO VPS)
        {"label": "NO ↔ CH",       "type": "transit", "wan": "vps",     "src": "no-vps", "key": "ch"},
        # NEW: NO VPS ↔ NO-DMZ pair (2 tunnels, swanctl on NO VPS — added 2026-05-05)
        {"label": "NO ↔ NO-DMZ01", "type": "transit", "wan": "vps",     "src": "no-vps", "key": "no-dmz01"},
        {"label": "NO ↔ NO-DMZ02", "type": "transit", "wan": "vps",     "src": "no-vps", "key": "no-dmz02"},
        # NEW: CH VPS ↔ NO-DMZ pair (counted from dmz side via swanctl conn "ch")
        {"label": "CH ↔ NO-DMZ01", "type": "transit", "wan": "vps",     "src": "no-dmz01", "key": "ch"},
        {"label": "CH ↔ NO-DMZ02", "type": "transit", "wan": "vps",     "src": "no-dmz02", "key": "ch"},
        # NEW: inter-dmz `pair` tunnel (only the dmz hosts know its state)
        {"label": "NO-DMZ01 ↔ NO-DMZ02", "type": "direct", "wan": "pair", "src": "no-dmz01", "key": "pair"},
        # txhou01vps01 — 7 tunnels (added 2026-05-06). Reported from the OTHER side
        # because claude01 → txhou is null-routed by ifog FastNetMon.
        # NL rtr01 (Tunnel6) ↔ TX-VPS (budget)
        {"label": "NL ↔ TX",       "type": "transit", "wan": "budget",  "src": "rtr",      "key": 6},
        # NL fw01 (Tunnel9) ↔ TX-VPS (freedom)
        {"label": "NL ↔ TX",       "type": "transit", "wan": "freedom", "src": "nl",       "key": 9},
        # GR ASA (Tunnel7) ↔ TX-VPS (inalan)
        {"label": "GR ↔ TX",       "type": "transit", "wan": "inalan",  "src": "gr",       "key": 7},
        # CH-VPS ↔ TX-VPS (read chzrh swanctl conn name "txhou01")
        {"label": "CH ↔ TX",       "type": "transit", "wan": "vps",     "src": "ch-vps",   "key": "txhou01"},
        # NO-VPS ↔ TX-VPS (read notrf01vps01 swanctl conn name "txhou01")
        {"label": "NO ↔ TX",       "type": "transit", "wan": "vps",     "src": "no-vps",   "key": "txhou01"},
        # NO-DMZ01 ↔ TX-VPS (read notrf01dmz01 swanctl conn name "txhou01")
        {"label": "TX ↔ NO-DMZ01", "type": "transit", "wan": "vps",     "src": "no-dmz01", "key": "txhou01"},
        # NO-DMZ02 ↔ TX-VPS (read notrf01dmz02 swanctl conn name "txhou01")
        {"label": "TX ↔ NO-DMZ02", "type": "transit", "wan": "vps",     "src": "no-dmz02", "key": "txhou01"},
    ]
    ...
    for t in arch_tunnels:
        if t["src"] == "nl":
            ...
            status = rtr_tunnels.get(t["key"], "down")
        elif t["src"] == "gr":
            status = gr_tunnels.get(t["key"], "down")
        elif t["src"] == "no-vps":
            status = no_vps_conns.get(t["key"], "down")
        elif t["src"] == "no-dmz01":
            status = no_dmz01_conns.get(t["key"], "down")
        elif t["src"] == "no-dmz02":
            status = no_dmz02_conns.get(t["key"], "down")
        elif t["src"] == "ch-vps":
            status = ch_vps_conns.get(t["key"], "down")
        else:
            status = "down"

        # Budget is no longer reclassified to "standby" — it's active-active
        # with Freedom on a dedicated edge router (rtr01) since 2026-04-21.
        ...
        tunnels.append({
            ...
            "wan": t["wan"],
            "status": status,
            "device": {"nl": "nl-fw01", "rtr": "nlrtr01",
                        "gr": "gr-fw01", "no-vps": "notrf01vps01",
                        "no-dmz01": "notrf01dmz01", "no-dmz02": "notrf01dmz02",
                        "ch-vps": "chzrh01vps01"}.get(t["src"], "?"),
            "latency_ms": None,
            "uptime_hours": None,
        })
...
def get_mesh_watchdog_stats():
    """Read the cross-device iBGP mesh watchdog textfile (IFRNLLEI01PRD-671).

    bgp-mesh-watchdog.sh emits total-expected / established / missing counts
    across all 9 BGP-speaking devices (rtr01, fw01, GR-fw01, 4× FRR, 2× VPS).
    Each bidirectional session is counted twice (once per endpoint), so the
    numbers are higher than the FRR exporter's peer count — this is a
    mesh-audit measurement, not a unique-session count.
    """
    path = "/var/lib/node_exporter/textfile_collector/bgp_mesh_watchdog.prom"
    out = {"total": 0, "established": 0, "missing": 0, "last_run_ts": 0}
    try:
        with open(path) as f:
            for line in f:
                line = line.strip()
                if line.startswith("#") or not line:
                    continue
                if line.startswith("bgp_mesh_total_expected "):
                    out["total"] = int(float(line.split()[-1]))
                elif line.startswith("bgp_mesh_established_count "):
                    out["established"] = int(float(line.split()[-1]))
                elif line.startswith("bgp_mesh_missing_count "):
                    out["missing"] = int(float(line.split()[-1]))
                elif line.startswith("bgp_mesh_last_run_timestamp "):
                    out["last_run_ts"] = int(float(line.split()[-1]))
...
def get_latency_matrix():
    ...
    # NL fw01 pings Freedom endpoints (active): .11=GR, .13=NO-VPS, .15=CH-VPS.
    # NL rtr01 pings Budget endpoints (active-active with Freedom since
    # 2026-04-21): .1=GR, .3=NO-VPS, .5=CH-VPS. Budget path runs independently
    # via Dialer1 on rtr01, no BCP38 collision with Freedom anymore.
    # GR pings inalan endpoints: .7=NO, .9=CH.
    static_fallback = {
        "NL-GR": 50.3, "NL-NO": 33.1, "NL-CH": 18.4,
        "GR-NO": 62.7, "GR-CH": 45.2, "NO-CH": 29.8,
        # TX (iFog Houston) onboarded 2026-05-06 — typical trans-Atlantic RTT
        "NL-TX": 120.0, "GR-TX": 135.0, "NO-TX": 125.0, "CH-TX": 125.0,
    }
    ...
    nl_tx = parse_ping_rtt(nl_ping_output, "10.255.200.X")
    gr_no = parse_ping_rtt(gr_ping_output, "10.255.200.X")
    gr_ch = parse_ping_rtt(gr_ping_output, "10.255.200.X")
    gr_tx = parse_ping_rtt(gr_ping_output, "10.255.200.X")
    ...
    matrix["NL-TX"] = nl_tx if nl_tx else static_fallback["NL-TX"]
    matrix["GR-NO"] = gr_no if gr_no else static_fallback["GR-NO"]
    matrix["GR-CH"] = gr_ch if gr_ch else static_fallback["GR-CH"]
    matrix["GR-TX"] = gr_tx if gr_tx else static_fallback["GR-TX"]
    ...
    # NO-TX / CH-TX: no direct ping path from NL/GR. Triangulate from NL-TX +
    # the European leg (NL-NO / NL-CH). Trans-Atlantic delay dominates so this
    # is a reasonable estimate until a TX-side ping cron is added.
    if nl_tx and nl_no:
        matrix["NO-TX"] = round(nl_tx + nl_no, 1)
    else:
        matrix["NO-TX"] = static_fallback["NO-TX"]
    if nl_tx and nl_ch:
        matrix["CH-TX"] = round(nl_tx + nl_ch, 1)
    else:
        matrix["CH-TX"] = static_fallback["CH-TX"]
...
def get_ripe_bgp():
    ...
    try:
        # Looking glass: AS path diversity. Aggregate by (transit, upstream)
        # suffix-pair so RIS-peer prepending doesn't fragment the count
        # across long paths like `2497 6939 34927 214304` vs `2500 6939
        # 34927 214304`. Then filter the long-tail (MIN_OBS=4) and cap
        # per upstream (MAX=7) so the output matches bgp.tools' view for
        # this prefix (iFog=7, Terrahost=2 as of 2026-05-16).
        url = f"{RIPE}/looking-glass/data.json?resource={urllib.request.quote(PREFIX_V6)}"
        with urllib.request.urlopen(url, timeout=15) as resp:
            data = json.loads(resp.read()).get("data", {})
            from collections import Counter
            MIN_TRANSIT_OBSERVATIONS = 4
            MAX_TRANSITS_PER_UPSTREAM = 7
            paths = []
            transit_asns = set()
            ...
            for rrc in data.get("rrcs", []):
                for peer in rrc.get("peers", []):
                    ...
                    if len(asns) >= 3:
                        suffix_counts[(asns[-3], asns[-2])] += 1

            result["total_as_paths"] = len(paths)
            result["unique_as_paths"] = len(Counter(paths))
            result["unique_transit_asns"] = len(transit_asns)
            by_upstream = {}
            for (t, u), c in suffix_counts.items():
                if c >= MIN_TRANSIT_OBSERVATIONS:
                    by_upstream.setdefault(u, []).append((t, c))
            top_rows = []
            for u, entries in by_upstream.items():
                entries.sort(key=lambda tc: (-tc[1], tc[0]))
                for t, c in entries[:MAX_TRANSITS_PER_UPSTREAM]:
                    top_rows.append((t, u, c))
            top_rows.sort(key=lambda r: -r[2])
            result["top_paths"] = [
                {"path": f"{t} {u} {AS}", "count": c}
                for (t, u, c) in top_rows
            ]
    except Exception:
        pass
...
def get_dmz_status():
    """Get Docker container status from all DMZ hosts via SSH.

    Returns list of DMZ node dicts with container health for the topology graph.
    NL + GR DMZ: direct SSH over VPN. NO DMZ pair (notrf01dmz01/02 — public-IP
    Docker hosts at Gigahost, onboarded 2026-05-05): direct SSH over the public
    Internet. The NO pair are children of site=NO (parent: notrf01vps01).
    """
    # (id, parent-site, hostname) — id matches existing convention "<SITE>-DMZ"
    # for the single-DMZ-per-site case (NL, GR). The NO active/active pair uses
    # "NO-DMZ01"/"NO-DMZ02" — same suffixes as the tunnel labels (e.g. tunnel
    # label "NL ↔ NO-DMZ01") so mesh-graph.js can resolve link source/target
    # to actual graph nodes via the existing parts.split-on-arrow lookup.
    DMZ_TARGETS = [
        ("NL-DMZ",   "NL", "nl-dmz01"),
        ("GR-DMZ",   "GR", "gr-dmz01"),
        ("NO-DMZ01", "NO", "notrf01dmz01"),
        ("NO-DMZ02", "NO", "notrf01dmz02"),
    ]
    ...
    def _ssh_docker_ps(host):
        # Returns list of {name,status} on success; None on ssh/transport failure.
        # A successful ssh that yields an empty docker ps returns [] (legitimate
        # "0 containers" reading). The None vs [] split lets the caller mark
        # the DMZ node as stale instead of baking 0/0 into Hugo data —
        # see memory/dmz_container_count_zero_baked_20260513.md
        try:
    ...
    from concurrent.futures import ThreadPoolExecutor
    with ThreadPoolExecutor(max_workers=len(DMZ_TARGETS)) as pool:
        futures = {dmz_id: pool.submit(_ssh_docker_ps, host)
                   for dmz_id, _site, host in DMZ_TARGETS}
        results = {dmz_id: f.result() for dmz_id, f in futures.items()}

    nodes = []
    for dmz_id, site, host in DMZ_TARGETS:
        services = results[dmz_id]
        if services is None:
            nodes.append({
                "id": dmz_id,
                "label": "DMZ",
...
def main():
    ...
    bgp = get_bgp_state()
    bfd = get_bfd_state()
    mesh_watchdog = get_mesh_watchdog_stats()
    ...
    # Determine NL WAN status dynamically. Budget is always-active on rtr01
    # (post 2026-04-21); Freedom presence on fw01 is tracked via SLA track 1.
    nl_wan_active = ["budget"]
    nl_wan_down = []
    if freedom_wan == "down":
    ...
    # Build failover section from real data. MTTR pulled from the last-30d
    # chaos_experiments table; fall back to a BFD-era default when the DB
    # has no fresh samples. [IFRNLLEI01PRD-677]
    mttr_seconds = 42  # fallback — pre-BFD chaos-test historical average
    try:
        import sqlite3 as _sqlite3
        db_path = os.path.expanduser("~/gitlab/products/cubeos/claude-context/gateway.db")
        with _sqlite3.connect(f"file:{db_path}?mode=ro", uri=True, timeout=3) as _conn:
            _cur = _conn.execute(
                "SELECT ROUND(AVG(recovery_seconds), 1) "
                "FROM chaos_experiments "
                "WHERE recovery_seconds IS NOT NULL AND recovery_seconds > 0 "
                "AND started_at > datetime('now', '-30 days')"
            )
            _row = _cur.fetchone()
            if _row and _row[0] is not None:
                mttr_seconds = float(_row[0])
    except Exception:
        pass
    ...
    failover_section = {
        ...
        "mttr_source": "chaos_experiments_30d_mean",
    }
    ...
    output = {
        ...
        "architecture": {
            ...
            "mesh_sessions_monitored": mesh_watchdog["total"],
            "mesh_sessions_established": mesh_watchdog["established"],
            "mesh_sessions_missing": mesh_watchdog["missing"],
            "failover_layers": 3,
            "failover_layers_desc": [
                "BFD sub-second (~900ms)",
                "BGP hold-time 30s (aggressive VTI timers 10/30)",
                "ISP SLA-track default route (~6-10s)",
            ],
            "k8s_clusters": 2,
            "k8s_nodes": 13,
        ...
        "sites": [
            {
                ...
                "tunnels_total": sum(1 for t in tunnels if "NL" in t["label"]) or 10,  # 6 base + 4 NL↔NO-DMZ0X
                "wan_interfaces": 2,
            ...
            },
            {
                ...
                "tunnels_total": sum(1 for t in tunnels if "GR" in t["label"]) or 6,
                "wan_interfaces": 1,
                "wan_active": ["inalan"],
                "wan_down": [],
            ...
            },
            {
                ...
                "country": "NO",
                "role": "transit",
                # All NO-substring labels (covers notrf01vps01 + notrf01dmz01/02 — same physical site).
                ...
                "tunnels_total": sum(1 for t in tunnels if "NO" in t["label"]) or 15,
            ...
            },
            {
                ...
                "tunnels_total": sum(1 for t in tunnels if "CH" in t["label"]) or 6,
                "availability_pct": 100.0,
                "devices_monitored": None,
                "active_alerts": None,
            },
            {
                # txhou01vps01 onboarded 2026-05-06 — 7 tunnels in/out
                "label": "TX",
                "country": "US",
                "role": "transit",
                "tunnels_up": sum(1 for t in tunnels if "TX" in t["label"] and t["status"] == "up"),
                "tunnels_standby": sum(1 for t in tunnels if "TX" in t["label"] and t["status"] == "standby"),
                "tunnels_total": sum(1 for t in tunnels if "TX" in t["label"]) or 7,
                "availability_pct": 100.0,
                "devices_monitored": None,
                "active_alerts": None,
            },
...
def _sanitize_for_public(data):
    ...
    # 2026-04-22 [IFRNLLEI01PRD-676]: preserve the device role suffix (fw01,
    # rtr01, sw01, etc.) so the topology graph can distinguish fw01 vs rtr01
    # at the NL site. Site prefix still redacted to two-letter country code.
    ...
    def _host_redact(m):
        site = m.group(1)[:2].upper()  # "nl", "gr", "no", "ch"
        suffix = m.group(2)            # "fw01", "rtr01", "sw01", etc.
        return f"{site}-{suffix}"

    def _scrub(obj):
        if isinstance(obj, str):
            obj = hostname_re.sub(_host_redact, obj)
...
def _peer_wan(rr, peer):
    """Derive the WAN (freedom/budget/inalan/vps) carrying a BGP session.

    Must tag sessions precisely so _mark_impaired_tunnels only flags a
    tunnel when BGP on *its own* WAN is idle. Pre-2026-04-22 this helper
    didn't exist, so a Freedom-down event marked the same-site-pair Budget
    tunnel as "impaired" too — which painted NL↔NO / NL↔CH Budget tunnels
    dark on the status-page graph during every Freedom outage, despite
    them being fully operational.

    Returns None when the WAN can't be resolved (caller then skips the
    tunnel — safer than a false impair).
    """
    combined = f"{rr} {peer}".lower()
    if "-freedom" in combined:
        return "freedom"
    if "-budget" in combined or "-xs4all" in combined:
        return "budget"
    # Freedom-side RR is NL-FRR01 / GR-FRR01; Budget-side is NL-FRR02 / GR-FRR02.
    if "frr01" in combined:
        return "freedom"
    if "frr02" in combined:
        return "budget"
    # VPS-to-VPS (NO ↔ CH) — both endpoints are "-edge" labels.
    if "-edge" in rr and "-edge" in peer:
        return "vps"
    return None


def _failed_tunnel_pairs(unexpected_idle):
    """Derive the set of (site-pair, wan) tunnels responsible for unexpected-idle BGP.

    A single broken tunnel often shows as 2-4 idle BGP sessions (both RRs
    trying + reverse views). Collapse to unique ({A, B}, wan) tuples so
    same-site-pair tunnels on different WANs are tracked independently.
    """
    pairs = set()
    for p in unexpected_idle or []:
        ...
        if not (a and b and a != b):
            continue
        wan = _peer_wan(p.get("rr", ""), p.get("peer", ""))
        if wan is None:
            continue
        pairs.add((frozenset((a, b)), wan))
    return pairs
...
def _mark_impaired_tunnels(tunnels, unexpected_idle):
    ...
    Catches asymmetric-forwarding failures — a tunnel where the ASA's own
    ping succeeds but no real traffic traverses (so BGP over it can't
    establish). Match is (sites, wan) so a Freedom-down event does not
    drag a healthy Budget tunnel into "impaired".
    """
    REDACTED_a7b84d63
    ...
    for t in tunnels:
        ...
        if len(sites) != 2:
            continue
        key = (frozenset(sites), t.get("wan"))
        if key in failed:
            t["status"] = "impaired"
            t["bgp_down"] = True


def _wan_failover_intact(tunnels):
    """Return the name of a fully-down WAN iff redundant paths hold every site-pair.

    Designed failover state: one WAN (Freedom or Budget) has EVERY tunnel
    down AND the WAN has no remaining up tunnels, but every inter-site
    pair still has at least one tunnel up on another WAN. Only then do we
    label the state "Freedom down, Budget holding" / equivalent.

    A single-tunnel flap does NOT trigger this — otherwise the banner
    text + BGP-peer reclassification cascade mis-fires for transient
    events (observed 2026-04-22 ~22:00Z: 1/3 Freedom tunnels briefly
    reported down while the other 2 remained up; banner still read
    "Freedom down" which was wrong). See fix for IFRNLLEI01PRD-693
    follow-up.

    Returns the down WAN name (e.g. "freedom") or None.
    """
    REDACTED_a7b84d63

    per_wan = {}  # wan -> {"up": int, "pairs_down": set((site_a, site_b))}
    pairs_up_any_wan = set()

    for t in tunnels:
        wan = t.get("wan")
        if not wan:
            continue
        sites = tuple(sorted(re.findall(r"\b(NL|GR|NO|CH)\b", t.get("label", ""))))
        if len(sites) != 2:
            continue
        entry = per_wan.setdefault(wan, {"up": 0, "pairs_down": set()})
        if t.get("status") == "up":
            entry["up"] += 1
            pairs_up_any_wan.add(sites)
        elif t.get("status") == "down":
            entry["pairs_down"].add(sites)

    # A WAN is "fully down" only if it has zero up tunnels AND at least
    # one down tunnel. Must be exactly one such WAN.
    fully_down = [w for w, e in per_wan.items() if e["up"] == 0 and e["pairs_down"]]
    if len(fully_down) != 1:
        return None

    down_wan = fully_down[0]
    pairs_down = per_wan[down_wan]["pairs_down"]
    if pairs_down and pairs_down.issubset(pairs_up_any_wan):
        return down_wan
    return None
...
def _compound_status(tunnels, bgp, clustermesh):
    ...
    Post 2026-04-22 [IFRNLLEI01PRD-673]: xs4all/Budget is active-active on
    nlrtr01 (not cold-standby behind Freedom), so the "expected standby"
    reclassification used pre-migration (NL-FRR02 VPS-xs4all peers) no
    longer applies. Any BGP session not Established is "unexpected idle"
    except for the Freedom-standby case (Freedom tunnels are still
    SLA-track conditionally active, and Freedom-path VPS peers can legitimately
    stand by when Freedom is down).

    WAN-failover exception: when one WAN is entirely down but every
    inter-site pair still has a redundant tunnel up, this is the designed
    failover — peers on the down WAN become "expected idle" and the
    compound level downgrades from "critical" to "degraded".
    """
    ...
    failover_wan = _wan_failover_intact(tunnels)
    has_standby_freedom = failover_wan == "freedom" or any(
    ...
    for p in bgp.get("peers", []):
        ...
        is_standby = False
        # Only Freedom-side peers can legitimately stand by; Budget-side peers
        # on NL-FRR02 are now active-active with Freedom on NL-FRR01.
        if p.get("type") == "ibgp_rr_vps":
            if "NL-FRR01" in rr and has_standby_freedom:
                is_standby = True
    ...
    if down == 0 and bgp_ok and cm_ok:
        level = "nominal"
    elif failover_wan and bgp_ok and cm_ok:
        # Designed WAN failover: one WAN down, every site-pair still covered,
        # BGP count honest (down-WAN peers reclassified as expected-idle).
        level = "degraded"
    ...
    if failover_wan:
        other = "Budget" if failover_wan == "freedom" else "Freedom"
        parts[0] += f" ({failover_wan.title()} down, {other} holding)"
