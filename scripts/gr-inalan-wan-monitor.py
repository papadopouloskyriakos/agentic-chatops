...
------
  ...
  creds: /etc/gr-inalan-wan-monitor.env  (TWILIO_* + NTFY_* keys, root 600 -- NOT in git)
...
-----
  ...
  gr-inalan-wan-monitor.py --test-push# send one labelled ntfy validation push and exit

2026-08-25 paging cutover: isolation still pages by SMS (ULTRA class: the mesh is
down, but LTE still reaches Twilio AND the public ntfy route) + an ntfy push.
Hourly re-notify + recovery are ntfy-ONLY; a re-notify SMS goes out at most every
GR_WAN_SMS_RENOTIFY_S (default 6h). NTFY_URL here is the PUBLIC route
(https://matrix.example.net) -- topic path alrt-tier1, Bearer NTFY_TOKEN.
"""
...
SMS_RENOTIFY_S = int(os.environ.get("GR_WAN_SMS_RENOTIFY_S", "21600"))  # SMS re-notify at most every 6h (pushes stay hourly)
...
def send_push(cfg: dict, title: str, body: str, priority: int = 5,
              tags: str = "rotating_light,gr") -> tuple[bool, str]:
    """ntfy push over the PUBLIC route (works from LTE). Inline (this file deploys
    standalone to grsyslogng01, no lib/). Metadata in query params, not
    headers (latin-1 codec). Best-effort: never raises."""
    url = cfg.get("NTFY_URL", "")
    topic = cfg.get("NTFY_TOPIC_TIER1", "alrt-tier1")
    token = cfg.get("NTFY_TOKEN", "")
    if not (url and token):
        return False, "ntfy creds missing"
    params = urllib.parse.urlencode({"title": title[:200], "priority": str(int(priority)), "tags": tags})
    try:
        req = urllib.request.Request(f"{url.rstrip('/')}/{topic}?{params}",
                                     data=body[:3800].encode("utf-8"), method="POST")
...
def run(cfg: dict, dry_run: bool) -> int:
    ...
    def notify(body: str, tag: str, *, sms: bool = True, push: bool = True,
               priority: int = 5, tags: str = "rotating_light,gr") -> None:
        """2026-08-25 paging cutover: every event pushes to ntfy; SMS only where
        the caller says so (ULTRA class / capped re-notify)."""
        nonlocal sent
        title = f"[gr] InAlan WAN {tag}"
        if dry_run:
            log(f"[DRY-RUN] would {'SMS+' if sms else ''}push ({tag}): {body}")
            sent.append(tag + ":dry")
            return
        if push:
            pok, pdetail = send_push(cfg, title, body, priority=priority, tags=tags)
            log(f"push ({tag}) -> {pok} {pdetail}")
            sent.append(f"{tag}:push:{pok}")
        if sms:
            ok, detail = send_sms(cfg, body)
            log(f"SMS ({tag}) -> {ok} {detail}")
            sent.append(f"{tag}:sms:{ok}")
    ...
    if state == "ok":
        if st.get("alert_active"):
            ...
            notify(body, "recovery", sms=False, priority=3, tags="white_check_mark,gr")
        ...
        if confirmed and not st.get("alert_active"):
            ...
            notify(body, "isolation", sms=True, priority=5)  # ULTRA: SMS + push
            st["alert_active"] = True
            st["last_alert"] = now
            st["last_sms_alert"] = now
            ...
            sms_due = (now - st.get("last_sms_alert", 0)) >= SMS_RENOTIFY_S
            notify(body, "renotify", sms=sms_due, priority=4)
            st["last_alert"] = now
            if sms_due:
                st["last_sms_alert"] = now
...
def main() -> int:
    ...
    ap.add_argument("--test-push", action="store_true", help="send one labelled ntfy test push, exit")
    args = ap.parse_args()
    cfg = load_env(ENV_FILE)

    if args.test_push:
        ok, detail = send_push(
            cfg, "[gr] InAlan WAN monitor: validation push",
            "OOB-over-LTE ntfy path works. Real pushes only on an actual InAlan outage. -gateway",
            priority=2, tags="white_check_mark,gr")
        print(f"test-push sent={ok} {detail}")
