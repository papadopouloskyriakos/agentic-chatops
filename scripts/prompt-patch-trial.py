...
def candidates_for_dim(dim: str) -> list[Candidate]:
    """GEPA generator (IFRNLLEI01PRD-1159) when PROMPT_GEPA_ENABLED=1, else the
    hand-authored pool. GEPA is GENERATE-ONLY — the Welch t-test in
    finalize-prompt-trials.py remains the sole promotion gate. Any GEPA failure
    (CLI missing, timeout, thin/garbled output) falls back to the hand-authored
    pool, so flag-off (and flag-on-but-failed) is byte-identical legacy."""
    hand = candidates_for(dim)
    try:
        from lib import gepa_generator
        if gepa_generator.ENABLED:
            seed = hand[0].instruction if hand else ""
            gen = gepa_generator.evolve_candidates(dim, seed, n_variants=len(hand))
            if gen:
                return gen
    except Exception:  # noqa: BLE001 — generation must never break trial start
        pass
    return hand
