...
score_session() {
  ...
  if [ ! -f "$JSONL" ] && [ "$USING_B64" -eq 0 ]; then
    ...
    # schema_version=1 per scripts/lib/schema_version.py (IFRNLLEI01PRD-635).
...
print('TEXT_START')
# ALL text blocks, not the last 5: long sessions (70+ turns) state CONFIDENCE
# and THOUGHT/OBSERVATION structure mid-transcript, and the tail is a compact
# summary — the last-5 window under-credited real investigations (2026-07-03,
# judge-fooled residual: 33-61-tool-call sessions scored 'structurally thin').
print('\n'.join(texts))
print('TEXT_END')
" 2>/dev/null)
  ...
  # [AUTO-RESOLVE] is the autonomy-forward (IFRNLLEI01PRD-1102) equivalent of the
  # approval poll: band=AUTO sessions END with it INSTEAD of polling, by design.
  # Without it every legit auto-resolve lost this step, landed at 5/8=62 (<75 =
  # "structurally incomplete") and branded the judge "fooled" for approving it.
  echo "$RESPONSE" | grep -qiE '\[POLL\]|\[AUTO-RESOLVE\]|awaiting approval|Which approach' && HAS_POLL=1
  echo "$RESPONSE" | grep -qiE 'CONFIDENCE:\s*[0-9]' && HAS_CONF=1

  # ── tool_call_log enrichment (IFRNLLEI01PRD-1713) ─────────────────────────
  # The text greps above look for literal CLI strings ('ssh ', 'kubectl ') and go blind once the
  # ephemeral /tmp JSONL is cleaned up — so modern sessions that investigate via MCP tools
  # (mcp__kubernetes__kubectl_*, mcp__proxmox__*, mcp__netbox__*, mcp__youtrack__add_comment) were
  # scored 'structurally thin' and branded the judge 'fooled' for correctly approving real work.
  # tool_call_log is the PERSISTENT, structured record of what the session actually did — OR it in.
  local TL_TOOLS
  TL_TOOLS=$(sqlite3 "$DB" "SELECT DISTINCT tool_name FROM tool_call_log WHERE issue_id='$ISSUE_ID'" 2>/dev/null)
  if [ -n "$TL_TOOLS" ]; then
    echo "$TL_TOOLS" | grep -qiE 'netbox' && HAS_NETBOX=1
    echo "$TL_TOOLS" | grep -qiE 'kubernetes|proxmox|kubectl|exec_in_pod|^Bash$|ios_|asa_|codegraph' && { HAS_EVIDENCE=1; HAS_SSH=1; }
    echo "$TL_TOOLS" | grep -qiE 'youtrack.*(add_comment|update_issue)|yt.?post.?comment' && HAS_YT=1
    echo "$TL_TOOLS" | grep -qiE 'ToolSearch|codegraph|semantic|kb.?search|incident' && HAS_KB=1
  fi
  # Coarse fallback: substantial tool activity IS investigation even when no specific marker matched
  # (>=5 tool_use blocks). Only fires when TOOLS is known (>0), so never inflates a thin session.
  if [ "${TOOLS:-0}" -ge 5 ]; then HAS_EVIDENCE=1; HAS_SSH=1; fi
  # KB/incident context is AUTO-INJECTED into every infra prompt (5-signal RRF) — an infra session
  # never 'queries' it, so has_incident_kb_query is a structural false-negative. Credit it for an
  # infra session that actually investigated (has evidence). Thin sessions stay uncredited.
  if [ "$IS_INFRA" -eq 1 ] && [ "$HAS_EVIDENCE" -eq 1 ]; then HAS_KB=1; fi
