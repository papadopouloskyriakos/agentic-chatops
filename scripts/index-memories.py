...
def main():
    ...
    # CDC: load existing content hashes so UNCHANGED chunks are not re-embedded
    # every run (true incremental, matching wiki-compile.py). (IFRNLLEI01PRD-1091)
    existing_hash = {r["path"]: r["content_hash"] for r in
                     conn.execute("SELECT path, content_hash FROM wiki_articles")}
    skipped = 0
    texts, metas = [], []
    for src_type, fpath in sources:
        ...
        if len(body) <= 2000:
            ...
            if existing_hash.get(path_key_base) == content_hash:
                skipped += 1
                continue
            parts = [title_base]
            if fm_description:
        ...
        for section, section_text in sections:
            ...
            if existing_hash.get(path_key) == content_hash:
                skipped += 1
                continue
            parts = [title_base]
            if fm_description:
    ...
    conn.close()
    print(f"indexed {inserted} memory/CLAUDE.md files ({skipped} unchanged, skipped re-embed); wiki_articles now {total} rows")
