...
# IFRNLLEI01PRD-635: schema version registry for session_transcripts rows.
...
def archive_transcript(issue_id, session_id="", embed=True, source_path=None):
    ...
    for chunk in chunks:
        ...
        conn.execute(
            "INSERT INTO session_transcripts (issue_id, session_id, chunk_index, role, content, embedding, source_file, schema_version) VALUES (?,?,?,?,?,?,?,?)",
            (issue_id, session_id, chunk["chunk_index"], chunk["role"], chunk["content"], embedding, jsonl_path, schema_current("session_transcripts"))
        )
        inserted += 1
...
def generate_session_summary(issue_id, jsonl_path, char_threshold=5000):
    ...
    try:
        ...
        conn.execute(
            ...
            "(issue_id, session_id, chunk_index, role, content, embedding, source_file, schema_version) "
            "VALUES (?, '', -1, 'summary', ?, ?, ?, ?)",
            (issue_id, summary, emb, jsonl_path, schema_current("session_transcripts")),
        )
        conn.commit()
        conn.close()
