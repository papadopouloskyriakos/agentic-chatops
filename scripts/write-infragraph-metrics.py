...
def main() -> int:
    ...
    lines += [
        ...
        "# HELP infragraph_invalidated_edges Edges superseded by a contradiction (invalid_at set; IFRNLLEI01PRD-1158)",
        "# TYPE infragraph_invalidated_edges gauge",
        f"infragraph_invalidated_edges {h.get('invalid_edges', 0)}",
        "# HELP infragraph_decay_at_risk Valid edges whose decayed confidence is below the re-ratification threshold (reporting-only)",
        "# TYPE infragraph_decay_at_risk gauge",
        f"infragraph_decay_at_risk {h.get('decay_at_risk', 0)}",
    ...
    for k in ("precision_30d", "recall_30d",
              "precision_family_30d", "recall_family_30d",
              "precision_fold_family_30d"):
        v = preds.get(k)
        if v is not None:
            if k == "precision_fold_family_30d":
                helptext = ("30d family precision on the fold-band "
                            "(cascade_prob_family >= 0.60) — the operative "
                            "fold/suppression precision the 0.80 gate applies to "
                            "(IFRNLLEI01PRD-1040)")
            else:
                helptext = (f"30d {k.split('_', 1)[0]} of evaluated predictions"
                            + (" at rule-family granularity (IFRNLLEI01PRD-1119)"
                               if "family" in k else ""))
            lines += [
                f"# HELP infragraph_{k} {helptext}",
                f"# TYPE infragraph_{k} gauge",
                f"infragraph_{k} {v}",
            ]
    fold_n = preds.get("fold_band_n_30d")
    if fold_n is not None:
        lines += [
            "# HELP infragraph_fold_band_n_30d 30d count of predictions in the "
            "fold-band (cascade_prob_family >= 0.60), denominator for "
            "precision_fold_family_30d",
            "# TYPE infragraph_fold_band_n_30d gauge",
            f"infragraph_fold_band_n_30d {fold_n}",
        ]
    lines += [
