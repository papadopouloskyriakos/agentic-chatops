...
# Store in execution_log
sqlite3 "$DB" "INSERT INTO execution_log (session_id, issue_id, step_index, device, command, pre_state, exit_code, rollback_command, duration_ms) VALUES ('$SESSION_ID', '$ISSUE_ID', $STEP_INDEX, '$DEVICE', '$(echo "$COMMAND" | sed "s/'/''/g")', '$(echo "$PRE_STATE" | sed "s/'/''/g")', -1, '$(echo "$ROLLBACK_CMD" | sed "s/'/''/g")', $DURATION)" 2>/dev/null
