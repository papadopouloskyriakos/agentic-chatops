...
# Also clean orphaned alert suppressions (H2 audit fix)
python3 -c "
import sys; sys.path.insert(0, '$REPO/scripts/lib'); sys.path.insert(0, '$REPO/scripts')
from chaos_baseline import cleanup_orphan_suppressions
cleanup_orphan_suppressions()
" 2>/dev/null || true
