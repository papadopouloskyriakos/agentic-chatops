...
declare -A WORKFLOWS=(
  ...
  # [session-end] rgRGPOZgPcFCvv84 — RETIRED 2026-06-26: its analytics logic was ported into
  #   reconcile-completed-sessions.py (_post_archive_side_effects, dark-component fix A). The
  #   workflow is dormant (0 executions ever) and is now deactivated in n8n. Removed from the
  #   monitored set so the watchdog does NOT re-activate it. Do not re-add.
...
# --- Dead-man's-switch metrics (IFRNLLEI01PRD-1152) ---
# The watchdog watches the pipeline; these metrics let Prometheus watch the
# watchdog. The heartbeat timestamp going stale (cron/host/node_exporter dead, or
# this script wedged) fires GatewayWatchdogHeartbeatStale -> Twilio SMS — the
# out-of-band dead-man's-switch, since the Matrix alerts below are muted by the
# operator. Emitted via a trap so it fires on EVERY exit path (maintenance,
# n8n-down, normal, or a set -e abort) = the truest "this script executed" signal.
METRICS_OUT="/var/lib/node_exporter/textfile_collector/gateway_watchdog.prom"
METRICS_ACC="$(mktemp 2>/dev/null || echo "/tmp/gw_watchdog_metrics.$$")"
WATCHDOG_HOST="$(hostname -s 2>/dev/null || echo nl-claude01)"

record_gauge() { printf '%s\n' "$1" >> "$METRICS_ACC" 2>/dev/null || true; }

emit_metrics() {
  local now; now=$(date +%s)
  {
    echo "# HELP gateway_watchdog_heartbeat_timestamp_seconds Unix time the gateway watchdog last ran (dead-man's-switch; IFRNLLEI01PRD-1152)."
    echo "# TYPE gateway_watchdog_heartbeat_timestamp_seconds gauge"
    echo "gateway_watchdog_heartbeat_timestamp_seconds{host=\"$WATCHDOG_HOST\"} $now"
    echo "# HELP gateway_n8n_healthy 1 if n8n /healthz responded ok on the last watchdog run, else 0."
    echo "# TYPE gateway_n8n_healthy gauge"
    echo "# HELP gateway_workflow_active 1 if the n8n workflow was found active, 0 if found inactive/unreachable (pre-reactivation)."
    echo "# TYPE gateway_workflow_active gauge"
    cat "$METRICS_ACC" 2>/dev/null
  } > "${METRICS_OUT}.tmp" 2>/dev/null && mv "${METRICS_OUT}.tmp" "$METRICS_OUT" 2>/dev/null || true
  rm -f "$METRICS_ACC" 2>/dev/null || true
}
HEALS_ONLY=0
if [ "${1:-}" = "--heals-only" ]; then HEALS_ONLY=1; fi  # platform-controller.py invokes this: run the proven heals, skip metrics + Layer-1 (the controller owns the dead-man metrics + reactivation)
if [ "$HEALS_ONLY" = "0" ]; then
  trap emit_metrics EXIT
else
  trap 'rm -f "$METRICS_ACC" 2>/dev/null || true' EXIT  # heals-only: clean the temp acc, do NOT emit metrics (controller owns them)
fi
...
check_workflow_active() {
  ...
  if [ -z "$response" ]; then
    record_gauge "gateway_workflow_active{workflow=\"$name\"} 0"
  ...
  if [ "$active" = "true" ]; then
    record_gauge "gateway_workflow_active{workflow=\"$name\"} 1"
...
cleanup_zombie_executions() {
  ...
  if [ "$killed" -gt 0 ]; then
    ...
    # Fix D: Detect persistent zombies — if the same IDs keep reappearing, bounce Bridge
    local zombie_ids_file="$STATE_DIR/zombie_ids"
    local prev_zombie_ids=""
    [ -f "$zombie_ids_file" ] && prev_zombie_ids=$(cat "$zombie_ids_file")
    local curr_zombie_ids=$(echo "$killed_ids" | sort)
    echo "$curr_zombie_ids" > "$zombie_ids_file"

    # Count overlap: if 3+ of the same IDs were zombies last run too, the DELETE isn't working
    local persistent=0
    for zid in $curr_zombie_ids; do
      echo "$prev_zombie_ids" | grep -qw "$zid" && persistent=$((persistent + 1))
    done

    if [ "$persistent" -ge 3 ]; then
      log_msg "PERSISTENT zombies detected ($persistent IDs survived cleanup) — bouncing Bridge"
      local bridge_id="${WORKFLOWS[matrix-bridge]}"
      curl -sf --max-time 10 -X POST \
        -H "X-N8N-API-KEY: $N8N_API_KEY" \
        "$N8N_URL/api/v1/workflows/$bridge_id/deactivate" >/dev/null 2>&1 || true
      ...
      curl -sf --max-time 10 -X POST \
        ...
        "$N8N_URL/api/v1/workflows/$bridge_id/activate" >/dev/null 2>&1 || true
      echo "$(date +%s)" > "$STATE_DIR/bridge_bounce_ts"
      post_alert "[WATCHDOG] ⚠️ Persistent zombie executions ($persistent IDs survived DELETE) — Bridge bounced to clear stuck schedule"
    else
...
main() {
  ...
  # Maintenance mode: skip all checks during scheduled maintenance
  if [ -f "/home/app-user/gateway.maintenance" ]; then
    log_msg "Maintenance mode active — watchdog suppressed"
  ...
  if ! check_n8n_health; then
    record_gauge "gateway_n8n_healthy 0"
    log_msg "n8n is down, skipping workflow checks"
    log_msg "=== Watchdog run complete ==="
    return
  fi
  record_gauge "gateway_n8n_healthy 1"

  # Layer 1: All workflows active (skipped under --heals-only — platform-controller.py owns reactivation)
  if [ "$HEALS_ONLY" = "0" ]; then
