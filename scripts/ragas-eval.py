...
OLLAMA_GEN_URL = os.environ.get("OLLAMA_GEN_URL", "http://nl-gpu01:11441")  # generate plane (2026-08-26 split)
...
def _call_ollama_judge(model, system, user, max_tokens=2048):
    ...
    NOTE: we deliberately do NOT pass `"format": "json"`. Ollama's JSON mode
    forces a top-level JSON *object*, which is incompatible with the RAGAS
    metrics that expect a JSON *array* (faithfulness / context_recall /
    answer_relevance). Under format:json gemma3:12b collapses an "array of
    strings" into {str: str, ...} and an "array of objects" into a single
    object holding only the FIRST item — so _unwrap_list degenerated to a
    non-list and every one of those three dims returned -1 on every local
    run-golden row since 2026-04-18 (commit 700fe07 flipped the judge to
    local Ollama + format:json). Without format:json gemma emits a proper
    fenced JSON array covering all items, which _parse_json_response already
    de-fences. context_precision survived because it parses a single object.
    (fix/ragas-minus-one)"""
    payload = json.dumps({
        "model": model,
        "prompt": f"{system}\n\n{user}",
    ...
    req = urllib.request.Request(f"{OLLAMA_GEN_URL}/api/generate", data=payload, method="POST")
...
def call_haiku(system, user, max_tokens=2048):
    ...
    # Plane B: prefer the gateway LiteLLM (per-component spend via x-litellm-tags=ragas-eval); fall
    # back to direct Anthropic on ANY failure (reachability/onboot/bad response). Fallback = the
    # original direct path below, untouched.
    _ll_key = os.environ.get("LITELLM_GATEWAY_KEY", "")
    if not _ll_key:
        try:
            with open(os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", ".env")) as _ef:
                for _l in _ef:
                    if _l.startswith("LITELLM_GATEWAY_KEY="):
                        _ll_key = _l.split("=", 1)[1].strip(); break
        except Exception:
            pass
    if _ll_key:
        try:
            _u = os.environ.get("LITELLM_URL", "http://10.0.181.X:4000") + "/v1/messages"
            _p = json.dumps({"model": "gw-deepseek", "max_tokens": max_tokens,
                             "system": system, "messages": [{"role": "user", "content": user}]}).encode("utf-8")
            _r = urllib.request.Request(_u, data=_p, method="POST")
            _r.add_header("Content-Type", "application/json")
            _r.add_header("Authorization", "Bearer " + _ll_key)
            _r.add_header("anthropic-version", "2023-06-01")
            _r.add_header("x-litellm-tags", "ragas-eval")
            with urllib.request.urlopen(_r, timeout=60) as _resp:
                _b = json.loads(_resp.read())
                try:
                    _us = _b.get("usage", {})
                    _cx = sqlite3.connect(DB_PATH)
                    _cx.execute("INSERT INTO llm_usage (tier, model, input_tokens, output_tokens, cost_usd, issue_id) VALUES (2, 'deepseek-v4-pro', ?, ?, 0.0, 'ragas-judge')",
                                (_us.get("input_tokens", 0), _us.get("output_tokens", 0)))
                    _cx.commit(); _cx.close()
                except Exception:
                    pass
                # deepseek-v4 returns [thinking, text] blocks -> join the TEXT blocks only.
                return "".join(_blk.get("text", "") for _blk in _b.get("content", [])
                               if isinstance(_blk, dict) and _blk.get("type") == "text")
        except Exception as _e:
            print(f"[ragas] LiteLLM(deepseek) failed ({type(_e).__name__}: {_e}); falling back to LOCAL ollama", file=sys.stderr)

    # No-Anthropic fallback: local Ollama (gemma3:12b -> qwen2.5:7b).
    for _m in (JUDGE_LOCAL_MODEL, JUDGE_LOCAL_FALLBACK):
        try:
            return _call_ollama_judge(_m, system, user, max_tokens)
        except Exception as _exc:
            print(f"[ragas:local:{_m}] {type(_exc).__name__}: {_exc}", file=sys.stderr)
    return ""
...
def _unwrap_list(obj):
    """Unwrap a list that the local Ollama judge (format:json) wrapped in a single-key
    object — {"claims":[...]}, {"verdicts":[...]}, {"questions":[...]}, {"response":[...]}.
    The list-expecting RAGAS metrics otherwise degenerate to [] -> metric -1 (bench
    IFRNLLEI01PRD-1422 dim7: faithfulness/context_recall/answer_relevance all returned -1 on
    every local run-golden row because gemma3 returns an object, not a bare array)."""
    if isinstance(obj, list):
        return obj
    if isinstance(obj, dict):
        for k in ("claims", "verdicts", "questions", "statements", "items", "results",
                  "response", "data", "answer", "output", "list"):
            if isinstance(obj.get(k), list):
                return obj[k]
        for v in obj.values():
            if isinstance(v, list):
                return v
        # Defensive fallbacks for the Ollama format:json collapse shapes
        # (kept as a belt-and-suspenders guard even though format:json is now
        # off — a future model in JSON mode would otherwise regress to -1):
        #  (a) "array of objects" -> a single record object. Detect a record
        #      by the metric keys it carries and wrap it in a list so the
        #      caller sees [{...}] instead of {...} (recovers the first item).
        if any(rk in obj for rk in ("verdict", "covered", "claim")):
            return [obj]
        #  (b) "array of strings" -> {str: str, ...}. The keys ARE the claims/
        #      questions; return them as a list. Only when every value is a
        #      scalar (no nested list/dict), to avoid mangling real objects.
...
def _decompose_claims(answer):
    ...
    try:
        claims = _unwrap_list(_parse_json_response(raw))
...
def _verify_claims_nli(claims, context):
    ...
    try:
        verdicts = _unwrap_list(_parse_json_response(raw))
...
def generate_answer(query, context):
    """Generate the system's ACTUAL RAG answer from the retrieved context, so
    RAGAS measures real generation quality instead of the answer=ground_truth
    tautology (IFRNLLEI01PRD-1088). Uses the same judge backend (local gemma3
    by default). Returns '' on failure so the caller can fall back."""
    system = ("You are an infrastructure operations assistant. Answer the question "
              "using ONLY the provided context. Be concise and factual. If the "
              "context is insufficient, say so explicitly.")
    user = f"Context:\n{context}\n\nQuestion: {query}\n\nAnswer:"
    try:
        return (call_haiku(system, user, max_tokens=400) or "").strip()
    except Exception as e:
        print(f"  generate_answer failed: {e}", file=sys.stderr)
        return ""
...
def compute_answer_relevance(query, answer, n_synth=3):
    ...
    try:
        synth_qs = _unwrap_list(_parse_json_response(raw))
...
def cmd_run_golden(limit=0, only_category=None):
    ...
    for i, entry in enumerate(golden_set):
        ...
        try:
            ...
            # Generate the system's ACTUAL RAG answer from the retrieved context
            # (no longer the answer=ground_truth tautology). faithfulness then
            # measures answer-vs-context, answer_relevance measures answer-vs-query,
            # and ground_truth still drives context precision/recall.
            # Fall back to ground_truth only if generation fails. (IFRNLLEI01PRD-1088)
            answer = generate_answer(query, full_context) or ground_truth
