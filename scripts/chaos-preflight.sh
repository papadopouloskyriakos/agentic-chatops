...
#    IFRNLLEI01PRD-721: CHAOS_STATE_PATH env override lets QA fixtures redirect
#    to a scratch tempdir instead of clobbering the real marker. Production
#    crons don't set it, so $HOME/chaos-state/chaos-active.json stays default.
STATE_FILE="${CHAOS_STATE_PATH:-$HOME/chaos-state/chaos-active.json}"
...
# 5. All 6 NL tunnels up — post 2026-04-21 migration, sum fw01 (Freedom
#    Tunnel4/5/6) + rtr01 (Budget Tunnel1/2/3). Expected 6 total NL-side
#    tunnels. [IFRNLLEI01PRD-674]
TUNNELS=$(python3 -c "
import sys; sys.path.insert(0, '$REPO_DIR/scripts/lib')
from asa_ssh import ssh_nl_asa_command
from ios_ssh import ssh_rtr01_command
fw = ssh_nl_asa_command(['show interface ip brief | include Tunnel'])
rt = ssh_rtr01_command(['show ip interface brief | include ^Tunnel'])
def count(out):
    ...
    return up, total
fw_up, fw_total = count(fw)
rt_up, rt_total = count(rt)
print(f'{fw_up + rt_up}|{fw_total + rt_total}')
" 2>/dev/null)
TUP=$(echo "$TUNNELS" | cut -d'|' -f1)
TTOT=$(echo "$TUNNELS" | cut -d'|' -f2)
if [ "${TUP:-0}" -ge 6 ]; then
    check "NL tunnels $TUP/$TTOT up (fw01 Freedom + rtr01 Budget)" "ok"
elif [ "${TUP:-0}" -ge 4 ]; then
    check "NL tunnels $TUP/$TTOT up (some down)" "warn"
else
    check "NL tunnels $TUP/$TTOT up" "fail"
fi
...
if [ "$FAIL" = "0" ]; then
    ...
    # IFRNLLEI01PRD-708 follow-up: MUST exit non-zero so callers using
    # `set -e` or `if ! bash chaos-preflight.sh` (e.g., freedom-ont-drill-trigger.sh)
    # see the NOT READY result. chaos-calendar.sh wraps this call in `|| true` and
    # parses the output instead, so that caller is unaffected.
    exit 1
fi
