...
# A2A spec requires: name, url, capabilities, version. Our cards extend it
# with _nla2a.tier for the routing tier (1/2/3). Check both shapes.
...
assert '_nla2a' in d and 'tier' in d['_nla2a'], 'missing _nla2a.tier'
" 2>/dev/null; then
...
# ── Test 19: CrowdSec learning DB table (skip in offline mode) ──
...
# ── Test 21: CrowdSec auto-suppression logic (skip in offline mode) ──
...
# ── Test 28: Prompt scorecard table exists (skip in offline mode) ──
