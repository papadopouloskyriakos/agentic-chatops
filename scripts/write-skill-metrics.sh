...
# Source the repo .env so requires.env checks emit the same OK state an
# actual agent invocation would see. Without this, cron-driven metric
# writes flag GITLAB_TOKEN / YOUTRACK_API_TOKEN / etc as unset (those vars
# live in .env, not the bare cron environment) and SkillPrereqMissing
# false-fires (IFRNLLEI01PRD-827).
if [ -f "$REPO_ROOT/.env" ]; then set -a; . "$REPO_ROOT/.env"; set +a; fi

# Cron's default PATH=/usr/bin:/bin excludes /usr/local/bin where kubectl
# (and other third-party bins) live. Without this, shutil.which("kubectl")
# in the embedded Python returns None and SkillPrereqMissing false-fires
# even though kubectl resolves fine under interactive shells
# (IFRNLLEI01PRD-843; mirror of the weekly-scan.sh PATH fix).
