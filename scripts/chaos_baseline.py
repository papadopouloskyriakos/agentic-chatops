...
def _scenario_convergence_slo(chaos_type, targets_json):
    """Convergence SLO matching the scenario's DESIGNED failover path
    (mirrors _build_hypothesis). GR<->NO/CH transit via NL = ~45s (two-hop);
    cross-site container DNS = ~300s; direct BFD-capable tunnels = 30s.
    IFRNLLEI01PRD-1744: replaces the flat 30s/120s that flagged designed
    behaviour as SLO breaches from day one."""
    try:
        ...
        if isinstance(t, dict):
            for tk in (t.get("tunnels_killed") or []):
                ...
                if "GR" in lbl and any(x in lbl for x in ("NO", "CH")):
                    return 45.0  # GR transit via NL is a designed two-hop path
        return 30.0
    return 120.0
...
def generate_retrospective(experiment_id, exercise_type="automated"):
    ...
    # Per-scenario convergence SLO (IFRNLLEI01PRD-1744): matches the harness's own
    # designed failover path instead of a flat 30s/120s that flagged GR<->NO/CH
    # two-hop transit (~45s) and cross-site DNS (~300s) as breaches forever.
    slo_convergence = _scenario_convergence_slo(chaos_type, targets_json)
    slo_mttd = 5.0

    # Container/dmz convergence is a hard-coded sentinel (300, _build_hypothesis
    # line ~1159), not a live measurement — do not raise a convergence finding
    # from it until real measurement lands.
    _container_sentinel = "container" in (chaos_type or "") or "dmz" in (chaos_type or "")
    if convergence is not None and convergence > slo_convergence and not _container_sentinel:
        findings.append({
...
BASELINE_TEST_MATRIX = [
    ...
    {"type": "tunnel", "tunnel": "NL ↔ TX", "wan": "freedom", "label": "NL↔TX Freedom (VPS transit, US)"},
    {"type": "tunnel", "tunnel": "NL ↔ TX", "wan": "budget",  "label": "NL↔TX Budget (VPS transit dual-WAN)"},
    ...
    {"type": "tunnel", "tunnel": "GR ↔ TX", "wan": "inalan", "label": "GR↔TX inalan (single-WAN, US)"},
