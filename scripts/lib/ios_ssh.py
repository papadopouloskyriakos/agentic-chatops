...
def ssh_rtr01_exec(commands: list[str], timeout: int = 30) -> tuple[bool, str]:
    """Execute privileged EXEC (non-config) commands on nlrtr01 — e.g.
    `clear crypto session remote <peer>`. send_command_timing because clear-class
    commands return no prompt-changing output. Returns (ok, concatenated_output).
    ...
    try:
        ...
        out = []
        for cmd in commands:
            out.append(c.send_command_timing(cmd, read_timeout=timeout))
        return True, "\n".join(out)
    except Exception as e:
