...
def _connect(db_path: Optional[str] = None) -> sqlite3.Connection:
    ...
    conn.execute("PRAGMA busy_timeout = 10000")  # set FIRST so the WAL switch below also waits
    # Only switch to WAL if not already there. Re-running PRAGMA journal_mode=WAL concurrently on a
    # fresh DELETE-mode DB races the rollback->WAL transition and raises a transient 'database is
    # locked' BEFORE bump()'s try-block (so it could not be retried) -> lost update (qa/643). Tolerate
    # it: whichever writer wins sets WAL; the rest operate in the current mode (BEGIN IMMEDIATE +
    # busy_timeout still serialize). Mirrors session_events._emit_insert.
...
def bump(
    ...
    *,
    ...
    # Retry the whole IMMEDIATE transaction on a transient lock so a contended bump is never
    # silently lost (qa/643-concurrent). Mirrors session_events._emit_insert.
    for attempt in range(8):
        ...
        try:
            ...
            break  # success or deliberate refusal — done
            ...
            try:
                ...
                pass
            last_exc = e
            _t.sleep(0.03 * (attempt + 1))  # back off, then retry the transaction
        except Exception:
            try:
                conn.execute("ROLLBACK")
            except Exception:
        ...
        # Every attempt hit a lock — surface it instead of returning a silently-missed bump.
        raise last_exc if last_exc else sqlite3.OperationalError("handoff bump failed after retries")
