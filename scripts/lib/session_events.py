...
@dataclass
class TeamCharterEvent(SessionEvent):
    """IFRNLLEI01PRD-750: team-formation roster declared at session start."""

    category: str = ""
    risk_level: str = ""
    hostname: str = ""
    agents: list = field(default_factory=list)   # [{agent, role, when, rationale}]
    rationale: str = ""

    def __post_init__(self):
        self.event_type = "team_charter"
...
@dataclass
class SessionReplayInvokedEvent(SessionEvent):
    """IFRNLLEI01PRD-751: server-side replay endpoint was invoked for a session."""

    outcome: str = "success"           # success | error | parse_error | unknown_session
    prompt_chars: int = 0
    cost_usd: float = 0.0
    num_turns: int = 0
    model: str = ""
    ...
    def __post_init__(self):
        self.event_type = "session_replay_invoked"
...
@dataclass
class IntermediateRailCheckEvent(SessionEvent):
    """IFRNLLEI01PRD-749: intermediate semantic rail observed-only check."""
    ...
    def __post_init__(self):
        self.event_type = "intermediate_rail_check"

    def _payload(self) -> dict[str, Any]:
        return {
            "is_in_distribution": bool(self.is_in_distribution),
...
@dataclass
class ITSBudgetConsumedEvent(SessionEvent):
    """IFRNLLEI01PRD-750: explicit Inference-Time-Scaling budget consumption."""

    budget_s: int = 0                # configured budget
    observed_turns: int = -1         # actual turns Claude took
    observed_thinking_chars: int = -1
    category: str = ""               # alert category at time of decision

    def __post_init__(self):
        self.event_type = "its_budget_consumed"

    def _payload(self) -> dict[str, Any]:
        return {
            "budget_s": self.budget_s,
            "observed_turns": self.observed_turns,
            "observed_thinking_chars": self.observed_thinking_chars,
            "category": self.category,
        }
...
EVENT_TYPES = (
    ...
    "agent_as_tool_call",
    "team_charter",                  # IFRNLLEI01PRD-750
    "its_budget_consumed",           # IFRNLLEI01PRD-750
    "intermediate_rail_check",       # IFRNLLEI01PRD-749
    "session_replay_invoked",        # IFRNLLEI01PRD-751
)
...
_EVENT_CLASSES = {
    ...
    "team_charter": TeamCharterEvent,
    "its_budget_consumed": ITSBudgetConsumedEvent,
    "intermediate_rail_check": IntermediateRailCheckEvent,
    "session_replay_invoked": SessionReplayInvokedEvent,
}
...
_EVENT_INSERT_SQL = (
    "INSERT INTO event_log (issue_id, session_id, turn_id, agent_name, event_type,"
    " payload_json, duration_ms, exit_code, schema_version) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)"
)


def _emit_insert(path: str, values: tuple) -> int:
    """INSERT one event_log row, hardened for CONCURRENT emits (fix: 637-events::
    concurrent_emit_no_loss lost 1-2 of 10). Two fixes: (1) set journal_mode=WAL only if not
    already WAL — re-setting it concurrently on a fresh DB races the rollback->WAL transition,
    raising a transient 'database is locked' that the old soft-error path silently dropped;
    (2) retry on sqlite3.OperationalError with backoff so a racing writer waits and succeeds
    instead of losing the event. Returns the new row id; raises the last OperationalError only
    ...
    for attempt in range(8):
        ...
        try:
            ...
            m = conn.execute("PRAGMA journal_mode").fetchone()
            if not m or str(m[0]).lower() != "wal":
                try:
                    conn.execute("PRAGMA journal_mode=WAL")
                except sqlite3.OperationalError:
                    pass  # another writer is mid-transition; the insert retry below covers it
            cur = conn.execute(_EVENT_INSERT_SQL, values)
            row_id = int(cur.lastrowid or -1)
            conn.commit()
            return row_id
        except sqlite3.OperationalError as e:
            last = e
            _t.sleep(0.03 * (attempt + 1))
        finally:
            if conn is not None:
                try:
                    conn.close()
                    ...
                    pass
    raise last if last else sqlite3.OperationalError("event_log insert failed after retries")
...
def emit(event: SessionEvent, db_path: Optional[str] = None) -> int:
    ...
    try:
        return _emit_insert(path, (
            row["issue_id"], row["session_id"], row["turn_id"], row["agent_name"],
            row["event_type"], row["payload_json"], row["duration_ms"],
            row["exit_code"], row["schema_version"],
        ))
