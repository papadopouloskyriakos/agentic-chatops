        ...
        with open(triage_log_path, "r") as fh:
            for line in fh:
                ...
                try:
                    ...
                    continue
                # Window is [now-window, now]: reject entries older than the floor AND entries
                # dated AFTER now (clock skew / future-dated / negative age). Without the upper
                # bound a future-dated triage.log line yields age_min<0 and wrongly dedups an alert
                # instead of escalating (IFRNLLEI01PRD-1706). Rejected -> fails open to escalation.
...
# ─────────────────────────────────────────────────────────────────────
# Phase 1b — parent-incident blast-radius fold
# ─────────────────────────────────────────────────────────────────────
# Direct analog of Prometheus Alertmanager `inhibit_rules` and PagerDuty
# Event Intelligence parent-child. When a sustained outage is open (Freedom
# PPPoE, GR poller stall, gpu01 io-error, etc.), downstream symptoms on
# OTHER hosts and OTHER rules cascade. Phase 1 only catches the exact same
# (host, rule) pair; the cascade needs a wider scope.
#
# Operator declares the blast radius by writing one row to openclaw_memory:
#   category = 'blast-radius'
#   key      = parent_issue_id    (e.g. 'IFRNLLEI01PRD-891')
#   value    = JSON {"hosts":[...], "host_patterns":[...], "rules":[...],
#                    "description":"...", "started_at":"..."}
#
# `hosts` is exact-match; `host_patterns` and `rules` are fnmatch globs. Any
# match triggers the fold. The parent issue's YT state determines whether
# the rule is still active — fail-OPEN if YT check fails (escalate, don't
# silently swallow). The current issue (if any) is never folded into itself.
def _match_blast_radius(rule_json: dict, hostname: str, rule_name: str) -> bool:
    """True iff (hostname, rule_name) is within the blast radius."""
    hosts = rule_json.get("hosts") or []
    host_patterns = rule_json.get("host_patterns") or []
    rules = rule_json.get("rules") or []
    host_match = (hostname in hosts) or any(
        fnmatch.fnmatchcase(hostname, p) for p in host_patterns)
    if not host_match and (hosts or host_patterns):
        return False
    if not host_match and not (hosts or host_patterns):
        # No host scoping — pure rule-based fold (rare; e.g. "all VTI alerts")
        pass
...
def check_phase1b_blast_radius(hostname: str, rule_name: str, severity: str,
                               db_conn: sqlite3.Connection, current_issue_id: str,
                               yt_issue_open_checker: Optional[Callable[[str], bool]]
                               ) -> SuppressionDecision:
    """Fold an alert into an open parent incident if it matches the parent's blast
    radius. Allowed for severity=critical (the parent IS critical work). Returns
    dedup outcome — not a resolution, since the parent incident is the active
    workstream."""
    try:
        rows = db_conn.execute(
            "SELECT key, value, updated_at FROM openclaw_memory "
            "WHERE category='blast-radius'"
        ).fetchall()
    except sqlite3.Error as exc:
        return SuppressionDecision(
            outcome="escalate", phase="none",
            reason=f"phase1b: SQL error ({type(exc).__name__}) — failing open",
            signals={"error": str(exc)[:200]},
        )

    scanned = 0
    for parent_id, value, updated_at in rows:
        scanned += 1
        if not parent_id:
            continue
        if current_issue_id and parent_id == current_issue_id:
            # Don't fold an incident into itself.
            continue
        try:
            rule_json = json.loads(value or "{}")
        except (json.JSONDecodeError, TypeError):
            continue
        if not _match_blast_radius(rule_json, hostname, rule_name):
            continue
        # Confirm parent is open; fail-OPEN on checker error.
        if yt_issue_open_checker is not None:
            try:
                is_open = bool(yt_issue_open_checker(parent_id))
            except Exception as exc:
                return SuppressionDecision(
                    outcome="escalate", phase="none",
                    reason=f"phase1b: yt_open_checker failed ({type(exc).__name__}) — failing open",
                    existing_issue_id=parent_id,
                    signals={"parent_id": parent_id, "error": str(exc)[:200]},
                )
            if not is_open:
                continue  # this rule's parent is closed; try the next rule
        description = rule_json.get("description") or "(no description)"
        comment = (
            f"Tier 1 deduplicated downstream cascade of open parent {parent_id}.\n"
            f"Parent blast-radius rule (openclaw_memory updated_at={updated_at}): {description}\n"
            f"This alert ({hostname}, {rule_name}) matches the parent's declared "
            f"scope and is NOT spawning a new Tier 2 session. When the parent is "
            f"resolved, this rule should be deleted from openclaw_memory."
        )
        return SuppressionDecision(
            outcome="dedup", phase="phase1b-blast-radius",
            reason=f"phase1b: matched parent {parent_id} blast radius — {description[:80]}",
            existing_issue_id=parent_id,
            comment_text=comment,
            confidence=0.90,
            signals={
                ...
                "match": {"hostname": hostname, "rule_name": rule_name},
            },
        )

    return SuppressionDecision(
        outcome="escalate", phase="none",
        reason=f"phase1b: no blast-radius rule matched (scanned {scanned})",
        signals={"rules_scanned": scanned},
    )


# ─────────────────────────────────────────────────────────────────────
# Phase SR — scheduled-reboot registry (self-learning suppression)
# ─────────────────────────────────────────────────────────────────────
# Suppress an on-schedule reboot on a host that has a LIVE, un-killed,
# un-expired deterministic schedule (cron/systemd-timer/unattended) registered
# in `discovered_scheduled_reboots`. A schedule is registered observing, then
# promoted to live only after >=2 confirmed in-window reboots (observe-before-
# live). The matcher is pure DB+math in scripts/lib/scheduled_reboots.py; this
# wrapper builds the SuppressionDecision. Full safety floor documented there.
# Runs AFTER blast-radius fold (a cascade parent wins over a per-host schedule)
# and BEFORE the knowledge-base transient guess (a deterministic schedule is a
# stronger signal). severity=critical never suppresses. Fails OPEN.
def check_phase_scheduled_reboot(hostname: str, rule_name: str, severity: str,
                                 db_conn: sqlite3.Connection,
                                 now_utc: datetime.datetime) -> SuppressionDecision:
    try:
        import scheduled_reboots as sr  # sibling module in scripts/lib/
    except Exception:
        try:  # package-style import (qa / alternate invocation paths)
            from lib import scheduled_reboots as sr  # type: ignore
        except Exception as exc:
            return SuppressionDecision(
                outcome="escalate", phase="none",
                reason=f"phaseSR: import failed ({type(exc).__name__}) — failing open",
                signals={"error": str(exc)[:200]},
            )
    try:
        res = sr.match_scheduled_reboot(hostname, rule_name, severity, now_utc, db_conn)
    except Exception as exc:
        return SuppressionDecision(
            outcome="escalate", phase="none",
            reason=f"phaseSR: matcher raised ({type(exc).__name__}) — failing open",
            signals={"error": str(exc)[:200]},
        )
    if not res.get("matched"):
        return SuppressionDecision(
            outcome="escalate", phase="none",
            reason=res.get("reason", "phaseSR: no match"),
            signals=res.get("signals", {}),
        )
    sig = dict(res.get("signals") or {})
    row_id = sig.get("row_id")
    try:
        sr.renew_on_match(db_conn, row_id)  # keep an active schedule from expiring
    except Exception:
        pass
    fire = sig.get("fire_utc", "?")
    comment = (
        f"Tier 1 suppressed as a SCHEDULED reboot (cron '{sig.get('cron_expr','?')}' "
        f"in host tz {sig.get('tz','?')}, fire {fire}). Intentional behavior — no "
        f"investigation spawned.\n"
        f"If this was NOT the expected reboot, REOPEN immediately. The two-phase "
        f"verify (verify-scheduled-reboot-boot.sh) also re-checks the boot reason "
        f"within ~60s and reopens automatically on a non-clean boot.\n"
        f"row_id={row_id}, rationale={sig.get('rationale','')}"
    )
    return SuppressionDecision(
        outcome="resolved-scheduled-reboot", phase="phaseSR-scheduled-reboot",
        reason=res.get("reason", "phaseSR: on-schedule reboot"),
        existing_issue_id="",          # digest-only: no per-host control issue
        comment_text=comment,
        confidence=0.95,
        signals={"phase": "phaseSR", **sig},
    )
...
def check_phase2_knownpattern(hostname: str, rule_name: str, severity: str,
    ...
    # IFRNLLEI01PRD-1153 — governance demotion consumer. A (host, rule) that the
    # governance metrics flagged as a repeat-offender (auto-resolved >=3x then
    # recurred) is demoted to analysis_only; STOP auto-resolving it and escalate
    # for root-cause. Safe-direction only (never causes suppression). Auto-set by
    # write-governance-metrics.py; auto-expires at valid_until (reversible).
    try:
        now_iso = now_utc.strftime("%Y-%m-%dT%H:%M:%SZ")
        demoted = db_conn.execute(
            "SELECT id FROM incident_knowledge "
            ...
            (hostname, rule_name, now_iso),
        ).fetchone()
    except sqlite3.Error:
        demoted = None  # pre-migration DB / column absent -> no demotions
    if demoted:
        return SuppressionDecision(
            outcome="escalate", phase="none",
            reason="phase2: (host,rule) governance-demoted to analysis_only "
                   "(repeat false-auto-resolve) — escalate for root-cause, do not auto-resolve",
            signals={"governance_demoted": True, "ik_id": demoted[0]},
        )
    cutoff_iso = (now_utc - datetime.timedelta(days=window_days)).strftime("%Y-%m-%dT%H:%M:%SZ")
    try:
        # hostname='*' = host-agnostic transient knowledge (a known-flappy alert_rule
        # that self-resolves regardless of host, e.g. TargetDown / ContainerOOMKilled).
        # Same safety envelope as a host-pinned row: still gated by alert_rule exact
        # match + severity!=critical (above) + confidence>=0.7 + a transient keyword.
        rows = db_conn.execute(
            """SELECT id, root_cause, resolution, confidence, tags, created_at, issue_id
               FROM incident_knowledge
               WHERE (hostname = ? OR hostname = '*') AND alert_rule = ?
                 AND (hostname = '*' OR created_at >= ?)
                 AND confidence >= ?
...
def check_suppression(hostname: str, rule_name: str, severity: str,
    ...
    try:
        # Phase 1b — blast-radius fold (must run before phase 2 so that downstream
        # cascade alerts get folded into the parent incident before being matched
        # against the knowledge base for THIS host+rule).
        try:
            d1b = check_phase1b_blast_radius(hostname, rule_name, severity, db_conn,
                                             current_issue_id, yt_issue_open_checker)
            if d1b.is_suppression():
                return d1b
            journey.append(("phase1b", d1b.reason))
            if d1b.signals:
                accumulated_signals["phase1b"] = d1b.signals
        except Exception as exc:
            journey.append(("phase1b", f"uncaught {type(exc).__name__}: {exc}"))
            accumulated_signals["phase1b"] = {"error": str(exc)[:200]}

        # Phase SR — scheduled-reboot registry (between blast-radius fold and
        # the knowledge-base transient guess). Lands dark behind env
        # TIER1_SCHED_REBOOT_ENABLED; fails OPEN like every other phase.
        try:
            dSR = check_phase_scheduled_reboot(hostname, rule_name, severity, db_conn, now_utc)
            if dSR.is_suppression():
                return dSR
            journey.append(("phaseSR", dSR.reason))
            if dSR.signals:
                accumulated_signals["phaseSR"] = dSR.signals
        except Exception as exc:
            journey.append(("phaseSR", f"uncaught {type(exc).__name__}: {exc}"))
            accumulated_signals["phaseSR"] = {"error": str(exc)[:200]}
