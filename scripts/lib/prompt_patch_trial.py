...
def _betacf(a: float, b: float, x: float) -> float:
    """Continued fraction for the incomplete beta (Numerical Recipes), stdlib only."""
...
def _student_t_sf(t: float, df: float) -> float:
    """One-sided upper-tail survival P(T > t) for a Student-t with df degrees."""
    if df <= 0:
        return 0.5
    x = df / (df + t * t)
...
def welch_one_sided(a: list[float], b: list[float]) -> tuple[float, float]:
    """One-sided Welch t-test: H0: mean(a) <= mean(b). Returns (t, p).

    Uses the proper Student-t distribution with Welch-Satterthwaite degrees of
    freedom (stdlib incomplete-beta, no scipy). The previous normal-approximation
    was anti-conservative at our K=15-30 sample sizes (IFRNLLEI01PRD-1096).
    """
    na, nb = len(a), len(b)
    if na < 2 or nb < 2:
        return 0.0, 1.0
...
def _persist_finalize(
    *,
    ...
    if write_patch and status == "completed" and winner_idx is not None:
        apply_now, gate_reason = _promotion_checkpoint(trial, winner_idx)
        if apply_now:
            _promote_to_patches_file(trial, winner_idx)
        else:
            _record_pending_promotion(trial, winner_idx, gate_reason)
            _try_emit("prompt_promotion_held", {
                "trial_id": trial_id, "winner_idx": winner_idx, "reason": gate_reason})
...
# ── Promotion checkpoint (IFRNLLEI01PRD-1267) ────────────────────────────────
# Human-as-circuit-breaker on the one fully-live self-modifying loop. Default OFF
# => byte-identical legacy (auto-promote). Flags are env-truthy or ~/gateway.<flag>.

PENDING_FILE = os.environ.get(
    "PROMPT_PROMOTIONS_PENDING_FILE",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "..",
                 "config", "prompt-promotions-pending.json"))


def _flag(name: str) -> bool:
    """Default OFF: env truthy, or a ~/gateway.<name> sentinel exists."""
...
def _flag_default_on(name: str) -> bool:
    """Default ON: only env=0/false (or a ~/gateway.<name>_off sentinel) disables it."""
    v = os.environ.get(name)
    if v is not None:
        return v not in ("0", "false", "False", "no", "NO")
    return not os.path.exists(os.path.expanduser(f"~/gateway.{name.lower()}_off"))


def _holdout_integrity_status() -> str:
    """'clean' | 'contaminated' | 'unverifiable' — is the sealed eval baseline safe to
    self-modify against? (runs check-eval-set-integrity.py)."""
    import subprocess
    checker = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..",
                           "check-eval-set-integrity.py")
    ...
    try:
        ...
        return "unverifiable"
    return "clean" if proc.returncode == 0 else "contaminated"


def _promotion_checkpoint(trial: "TrialRow", winner_idx: int) -> tuple[bool, str]:
    """Return (apply_now, reason).

    DEFAULT = AUTONOMOUS: a statistical winner auto-promotes, gated only by a
    holdout-integrity SAFETY RAIL (won't self-modify against a contaminated eval
    baseline; proceeds-with-warning if integrity is merely unverifiable, so infra
    flakiness never stalls the loop). Operator controls:
      * PROMPT_PROMOTION_REVIEW=1            -> opt into a human-review hold (less autonomous)
      * PROMPT_PROMOTION_HOLDOUT_GATE=0      -> disable the safety rail (pure legacy auto-promote)
      * ~/gateway.prompt_promotion_holdout_gate_off -> same kill-switch via sentinel
    """
    if _flag("PROMPT_PROMOTION_REVIEW"):
        return False, "held for human review (PROMPT_PROMOTION_REVIEW)"
    if not _flag_default_on("PROMPT_PROMOTION_HOLDOUT_GATE"):
        return True, "auto-promoted (holdout safety rail disabled)"
    status = _holdout_integrity_status()
    if status == "contaminated":
        return False, "held: eval sets contaminated — self-modification paused for review"
    if status == "unverifiable":
        print("[prompt_patch_trial] WARN: holdout integrity unverifiable; auto-promoting anyway",
              file=sys.stderr)
        return True, "auto-promoted (holdout integrity unverifiable — warned)"
    return True, "auto-promoted (holdout integrity clean)"


def _record_pending_promotion(trial: "TrialRow", winner_idx: int, reason: str) -> None:
    """Record a winning promotion for human review instead of applying it live."""
    winner = trial.candidates[winner_idx]
    now_iso = datetime.now(timezone.utc).strftime("%Y-%m-%dT%H:%M:%SZ")
    rec = {
        "trial_id": trial.id, "dimension": trial.dimension, "category": winner.category,
        "instruction": winner.instruction, "winner_idx": winner_idx, "label": winner.label,
        "score_before": trial.baseline_mean, "recorded_at": now_iso,
    ...
    try:
        ...
        print(f"[prompt_patch_trial] WARN: could not write {PENDING_FILE}: {e}", file=sys.stderr)
