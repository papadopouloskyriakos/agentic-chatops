...
"""
    try:
        ...
        # A failed hop (stone down, netmiko crash) exits non-zero with empty
        # stdout — surface that as ERROR so callers count the host UNREACHABLE
        # instead of "all sessions missing" (bgp-mesh-watchdog consumer relies
        # on this distinction; gap found 2026-08-14).
        if result.returncode != 0 and not result.stdout.strip():
            return f"ERROR: gr-asa hop rc={result.returncode}: {result.stderr.strip()[:200]}"
        return result.stdout
    except Exception as e:
        return f"ERROR: {e}"


def ssh_gr_asa_exec(commands, timeout=120):
    """Execute privileged EXEC (non-config) commands on the GR ASA via the OOB
    stone — e.g. `vpn-sessiondb logoff tunnel-group <ip> noconfirm` and
    `clear crypto ipsec sa peer <ip>` during a wedged-SA re-key. Uses
    send_command_timing (clear-class commands emit little/no output) and a
    longer budget than ssh_gr_asa_command's 30s. Returns (ok, output).

    Introduced 2026-08-14 for the intersite tunnel auto-heal (IFRNLLEI01PRD-1833).
    """
    pw = get_asa_password()
    py_script = f"""
from netmiko import ConnectHandler
...
for cmd in {commands!r}:
    print('### ' + cmd)
    print(net.send_command_timing(cmd, read_timeout=30))
net.disconnect()
print('EXEC_OK')
"""
    try:
        result = subprocess.run(
            ...
            ["-i", os.path.expanduser("~/.ssh/one_key"),
             f"{GR_OOB_USER}@{GR_OOB_HOST}",
             f"export CISCO_ASA_PASSWORD='{pw}'; "
             f"/tmp/netmiko-venv/bin/python3 -c \"{py_script}\" 2>/dev/null "
             f"|| python3 -c \"{py_script}\""],
            capture_output=True, text=True, timeout=timeout,
        )
        ok = "EXEC_OK" in result.stdout
        out = result.stdout if result.stdout.strip() else f"rc={result.returncode}: {result.stderr.strip()[:200]}"
        return ok, out
    except Exception as e:
...
# ── Shared tunnel status (single source of truth for IFRNLLEI01PRD-530) ────

def parse_tunnel_interfaces(output):
    """Parse 'show interface ip brief | include Tunnel' output.

    Returns dict {tunnel_key: 'up'|'down'} where key is int (Tunnel number).
    Used by vpn-mesh-stats.py, chaos_baseline.py, and chaos-preflight.sh.
    """
    tunnels = {}
    for line in output.splitlines():
        parts = line.split()
        if len(parts) >= 2 and "Tunnel" in parts[0]:
            try:
                key = int(parts[0].replace("Tunnel", ""))
            except ValueError:
                continue
            status = "up" if len(parts) >= 6 and parts[-1] == "up" else "down"
            tunnels[key] = status
    return tunnels


def get_all_tunnel_status():
    """Get tunnel status from both ASAs + NO VPS. Single source of truth.

    Returns dict {(label, wan): status} for all 9 tunnels.
    Status is 'up', 'down', or 'standby' (xs4all when Freedom is active).
    """
    # NL ASA: 6 tunnels
    try:
        nl_output = ssh_nl_asa_command([
    ...
    # GR ASA: 4 tunnels (T1+T4 are NL↔GR duplicates, skip)
    try:
        gr_output = ssh_gr_asa_command(["show interface ip brief | include Tunnel"])
    except Exception:
        gr_output = ""

    # NO VPS: swanctl connections
    no_vps = ssh_vps_swanctl("198.51.100.X")

    nl_tunnels = parse_tunnel_interfaces(nl_output)
    gr_tunnels = parse_tunnel_interfaces(gr_output)

    # Check xs4all BGP state for standby classification
    xs4all_bgp = "idle"
    for line in nl_output.splitlines():
        if "BGP state" in line and "Established" in line:
            xs4all_bgp = "established"
            break

    # Map to labeled tunnel status
    arch = [
        ("NL \u2194 GR", "xs4all", "nl", 1),
        ("NL \u2194 NO", "xs4all", "nl", 2),
        ("NL \u2194 CH", "xs4all", "nl", 3),
    ...
    result = {}
    for label, wan, src, key in arch:
        if src == "nl":
            status = nl_tunnels.get(key, "down")
        elif src == "gr":
            status = gr_tunnels.get(key, "down")
        elif src == "no-vps":
            status = no_vps.get(key, "down")
        else:
            status = "down"

        # xs4all standby classification
        if wan == "xs4all" and status == "up" and xs4all_bgp != "established":
            status = "standby"

        result[(label, wan)] = status
