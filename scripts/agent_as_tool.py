...
class AgentSpec:
    ...
    def from_path(cls, p: Path) -> Optional["AgentSpec"]:
        ...
        body = parts[2].lstrip()
        meta: dict[str, Any] = {}
        # Block-scalar-aware frontmatter parse (bench IFRNLLEI01PRD-1423, 2026-06-26):
        # the agent `description:` fields are YAML folded/literal block scalars
        # (`description: >` with text on the following indented lines). The old flat
        # loop captured only the ">" indicator, so every description collapsed to the
        # bare agent name. Now: when a value is a block indicator (> | with chomping
        # >- |- >+ |+), consume the deeper-indented continuation lines and join them.
            ...
            if val in (">", "|", ">-", "|-", ">+", "|+"):
                ...
                while i < len(front_lines):
                    ...
                    if nxt.strip() == "":
                        ...
                        i += 1
                        continue
                    if nxt[:1].isspace():  # indented deeper than the col-0 key
...
def invoke(
    ...
    *,
    ...
    # Honor the sub-agent's declared model (.claude/agents/*.md `model:`, default "haiku"). Without this
    # the spawned `claude -p` fell back to the host-default (Opus), so every "haiku" sub-agent silently ran
    # Opus. `claude --model` accepts the short aliases (haiku/sonnet/opus) and full model ids.
    if spec.model:
