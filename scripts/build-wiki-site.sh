...
if [ -d "$REPO_ROOT/docs" ]; then
  ...
  # Exclude llm-engineers-handbook: a 7k-line IMPORTED reference book whose raw Python
  # regex lookbehinds (e.g. `(?<![A-Z][a-z]\.)`) reach mkdocs's HTMLParser as a `<![`
  # marked section and hard-crash the static render with AssertionError (dark-component
  # audit 2026-06-25: this crashed the site build silently, masked by the embed running
  # first). It stays in the RAG embed (wiki-compile.py) — only the browsable site skips it.
  find "$REPO_ROOT/docs" -type f \
    \( -name '*.md' -o -name '*.yml' -o -name '*.yaml' -o -name '*.txt' \) \
    -not -path '*/node_modules/*' \
    -not -path '*/llm-engineers-handbook/*' \
    | while read -r f; do
...
# 4b. Sanitize raw `<![` marked-section triggers that crash mkdocs's HTMLParser with
# `AssertionError: unknown status keyword ... in marked section`. These come from regex
# examples in prose/inline-code across docs + memory (e.g. `(?<![A-Za-z0-9_-])`). Insert a
# zero-width space (U+200B) between `<!` and `[` in the COPIED files only — it breaks the
# `<![` token for the parser while rendering invisibly, with no edit to the originals.
# (Dark-component audit 2026-06-25: this silently crashed the site build, masked by the
# RAG embed running on a separate path.)
ZWSP=$(printf '\xe2\x80\x8b')
find "$SITE_SRC" -type f -name '*.md' -exec sed -i "s/<!\[/<!${ZWSP}[/g" {} + 2>/dev/null || true
