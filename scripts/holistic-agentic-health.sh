...
# Migrated from OpenAI probe 2026-04-28 (IFRNLLEI01PRD-746).
# Reachability: any HTTP response (200/400/401/405/etc) proves the API is up;
# only network errors / timeouts mean unreachable. We use curl http_code to
# avoid Python urllib's HTTPError vs URLError split.
if $QUICK; then skip "openclaw-llm" "skipped (--quick)"
else
  OC_LLM=$(ssh -o ConnectTimeout=5 nl-openclaw01 'docker exec openclaw-openclaw-gateway-1 bash -lc "
[ -x /usr/local/bin/claude ] && [ -f /home/node/.claude/.credentials.json ] || { echo NO; exit 0; }
HTTP=\$(curl -s --max-time 5 -o /dev/null -w \"%{http_code}\" https://api.anthropic.com/v1/messages 2>/dev/null)
if [ -n \"\$HTTP\" ] && [ \"\$HTTP\" != \"000\" ]; then echo OK; else echo NO; fi
"' 2>/dev/null || echo "NO")
  if [ "$OC_LLM" = "OK" ]; then pass "openclaw-llm" "claude CLI present + Anthropic API reachable from container"
  else warn "openclaw-llm" "claude CLI missing OR OAuth creds absent OR Anthropic API unreachable"; fi
fi
...
if [ -f "$IG_DB" ] && [ -f "$IG_QUERY" ]; then
  ...
  # Cron wiring
  if crontab -l 2>/dev/null | grep -q "infragraph-seed.py --all"; then
    pass "infragraph-seed-cron" "daily seed cron present"
  else
    fail "infragraph-seed-cron" "no infragraph-seed.py cron in crontab"
  fi
  if crontab -l 2>/dev/null | grep -q "infragraph-learn.py"; then
    pass "infragraph-learn-cron" "hourly learn cron present"
  else
    fail "infragraph-learn-cron" "no infragraph-learn.py cron in crontab"
  fi
