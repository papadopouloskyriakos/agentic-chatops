...
MATRIX_BOT_TOKEN="REDACTED_f926083b"
YT_TOKEN="REDACTED_YT_TOKEN"
NB_TOKEN="REDACTED_4bd0c65f"
...
if $QUICK; then skip "librenms-nl" "skipped"; skip "librenms-gr" "skipped"
  ...
  NL_DEV=$(curl -sk -H "X-Auth-Token: REDACTED_20ee4f7c" "https://nl-nms01.example.net/api/v0/devices" 2>/dev/null | python3 -c "import sys,json; d=json.load(sys.stdin); print(d.get('count',len(d.get('devices',[]))))" 2>/dev/null || echo 0)
  if (( NL_DEV >= 100 )); then pass "librenms-nl" "$NL_DEV NL devices"
  else warn "librenms-nl" "only $NL_DEV"; fi

  GR_RULES=$(curl -sk -H "X-Auth-Token: REDACTED_c7cb035f" "https://gr-nms01.example.net/api/v0/rules" 2>/dev/null | python3 -c "import sys,json; print(len(json.load(sys.stdin).get('rules',[])))" 2>/dev/null || echo 0)
...
if $QUICK; then skip "vps-tunnels" "skipped (--quick)"
  ...
  VPS_SAS=$(ssh -o ConnectTimeout=5 -i ~/.ssh/one_key operator@185.44.82.32 "echo '$ASA_PW' | sudo -S swanctl --list-sas 2>/dev/null | grep -c 'ESTABLISHED'" 2>/dev/null || echo 0)
  VPS_SAS=$((VPS_SAS + 0))
