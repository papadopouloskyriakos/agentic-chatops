...
- Add GR kubeconfig context to app-user's `~/.kube/config` (if not already there)
...
| gr-sw01, gr-sw02 | Core switches | Local site |
| gr2sw01 | Remote switch | Different building |
...
| PVE hosts | 3 (nl-pve01/02/03) | 2 (gr-pve01/02) |
...
1. **n8n runs on NL only** (nl-n8n01 on nl-pve01). GR alerts must traverse the IPsec VPN to reach n8n webhooks. If VPN is down, GR alerts won't reach the pipeline.

2. **Matrix runs on NL only** (nl-matrix01 on nl-pve01). Same VPN dependency.
