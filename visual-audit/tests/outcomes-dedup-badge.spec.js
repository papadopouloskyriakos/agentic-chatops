...
test.describe('Outcomes dedup badge', () => {
  ...
  test('5. Dedup is NOT folded into auto-resolve rate', () => {
    // Sanity: the rate is per-incident resolved/(resolved+escalated) and must stay a
    // valid probability. The old 0.5 ceiling was written when the rate sat at ~6% and
    // is now stale — per-incident autonomy has legitimately grown past 50% (0.5667 as
    // of 2026-07-09), so assert a valid [0,1] rate rather than a frozen historical band.
    expect(state.autoResolveRate).toBeGreaterThanOrEqual(0);
    expect(state.autoResolveRate).toBeLessThanOrEqual(1);
  });
});
