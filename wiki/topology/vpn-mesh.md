...
> Compiled from 17 memory files + 16 CLAUDE.md files. 2026-07-03 04:30 UTC.
...
- Freedom DOWN → SMS to +3VMID_REDACTED0: "Freedom ISP DOWN... Physical fix: power-cycle ONT on nl-sw01 Gi1/0/36"
...
## freedom-ont-drill-trigger installer — manual, one-shot

Resolved the "trace the one-shot installer" deferred item from `memory/chaos_cron_collision_20260423.md`.

**Answer: the operator installs it by hand via `crontab -e` when a drill is desired.** There is no automatic scheduler — the script's own header states "Invoked by a cron entry (one-off; self-removes after firing to keep the crontab clean)."

## Evidence

- `scripts/freedom-ont-drill-trigger.sh:2-13` header explicitly calls it "one-shot" and "Operator still runs the `shutdown` manually on nl-sw01 Gi1/0/36 per the scenario's `manual_execute: true` flag."
- `scripts/freedom-ont-drill-trigger.sh:68, 88, 160` — three code paths that execute `(crontab -l | grep -v freedom-ont-drill-trigger.sh) | crontab -` to self-clean the crontab entry after the drill dispatches. This enforces the one-shot contract.
- Full-repo grep (2026-04-24) for `freedom-ont-drill-trigger` finds **no automation** that installs the cron line — no AWX template, no n8n workflow, no CI step, no setup script. Only the script itself (its self-removal calls), QA tests, and `chaos-port-shutdown.py` header comment.
- `crontab -l | grep freedom-ont` → 0 matches at the time of the trace.

## How a drill actually gets scheduled

Per the script header + commit `f0a4500` (IFRNLLEI01PRD-695 acceleration request):

1. Operator decides to run a Freedom-ONT drill.
2. Operator runs `crontab -e` on `nl-claude01` as `app-user`.
3. Operator adds a one-shot entry, e.g. `0 8 15 * * /app/claude-gateway/scripts/freedom-ont-drill-trigger.sh` (fires once at 08:00 UTC on the 15th).
4. Cron fires the script at the scheduled time.
5. Script posts Matrix notice, installs chaos-active marker, dispatches `chaos-port-shutdown.py` via nohup (post-2026-04-22 autonomous path), and self-removes the crontab entry.

**Why manual-by-design:** the drill deliberately puts Freedom ISP in a degraded state for ~15-30 min. Operator wants to be awake for it, not surprised by an auto-scheduler. The "one-shot" pattern means zero accumulated debt in crontab if an operator forgets to remove it.

## Implication for chaos cadence decision

The 04-23 collision with the intensive cron happened because an operator installed the trigger during an already-scheduled `chaos-calendar.sh` run window. The -721/-708/-709/-710 safeguards now prevent that collision cleanly (preflight blocks + ABORT with Matrix-clarity naming the other drill, shipped 2026-04-24). No automation change needed.

**How to apply:** when someone asks "who scheduled the drill I just saw," the answer is "an operator, via `crontab -e`". If a drill appears unexpectedly, check `sudo -u app-user crontab -l` on nl-claude01 before chasing automation that doesn't exist.

*Source: `memory/freedom_ont_drill_installer.md`*
...
## freedom-pppoe-outage-resolved-20260513

## Resolved 2026-05-13

| Phase | Timestamp (UTC) | Source |
|---|---|---|
| **Down** | 2026-05-08 09:46:36 | ASA Track 1 changed → Down |
| Recovery requested (NOC phone) | 2026-05-11 ~13:20 | Operator call, English; field engineer dispatched |
| **Line-protocol up** | 2026-05-13 00:00:05 | First outbound on `outside_freedom` (ASA self-originated DNS to 8.8.8.8) |
| **First NAT through assigned WAN IP** | 2026-05-13 00:56:09 | First syslog match for `outside_freedom:203.0.113.X` |
| Full healthy day | 2026-05-14 (957k NAT events) | nl-fw01 syslog |
| YT IFRNLLEI01PRD-891 closed | 2026-05-15 | This post-mortem |

**Duration: ~4 d 15 h 14 m.** Zero user-visible downtime — failover via `outside_budget` carried all traffic the whole time.

## Day-by-day count of NAT translations through the assigned WAN IP `203.0.113.X`

```
2026-05-08: 968,270  (healthy before outage)
2026-05-09:       0
2026-05-10:       0
2026-05-11:       0
2026-05-12:       0
2026-05-13: 862,815  (recovered)
2026-05-14: 957,230  (full healthy day)
```

The gap is unambiguous in `nl-fw01-2026-05-0{9..12}.log` syslog files.

## What was broken

`nl-fw01` `Port-channel1.6` / `outside_freedom` PPPoE Discovery — line protocol up, ASA sent PADI continuously, **no PADO from BRAS `198.51.100.X`**. `show ip address` had Po1.6 absent from the **Current** table; only **System** had it pinned to `203.0.113.X`. All 6 VTI tunnels sourced from `outside_freedom` (Tunnel4–9 → GR / NO / CH / NO-DMZ01 / NO-DMZ02 / TXHOU) were down with source IP `0.0.0.0`.

**Failover was clean:** `S* 0.0.0.0/0 [5/0] via 10.0.X.X`. All inter-site VTIs re-converged through Budget. BRAS itself ICMP-reachable from `outside_budget` (4/4 @ 10 ms) — the problem was specifically PPPoE session-state for our line, not BRAS reachability.

## Why (likely)

Freedom-side stale half-open PPPoE session for our circuit (line bound at OLT to our XGS-PON SN). Customer-side recovery attempts (PoE-cycle Gi1/0/36, shut/no-shut Po1.6) failed for 3 days.

## What worked

Operator (English-speaking) phoned **Freedom NOC at `088-0115666`** on 2026-05-11 ~15:20 CEST. Freedom decided to **dispatch a field engineer to physically replace the ONT**. Mechanism of fix expected to work even if the ONT itself is healthy: the swap forces OLT-side de-registration of the old PON serial + registration of the new one, which incidentally refreshes the BRAS-side service binding for circuit F0381391 — the re-bind we'd been asking for, wrapped in a hardware swap they were more comfortable dispatching than a NOC-side session-clear.

Recovery happened between May 12 23:59 and May 13 00:00 UTC. The exact mechanism (ONT swap vs back-office BRAS service-binding refresh) isn't recorded in the syslog — but the timing matches a Freedom service-window slot.

## Customer detail (for any future Freedom ticket)

- Customer / line: Example Corp BV / Leiden NL site
- Postcode + nr: 2315 HP, 17
- Customer number: **F0381391**
- Hoofdaccount: kyriakos@freedom.nl
- PPPoE: VLAN 6 (802.1Q), PAP, MTU 1492 — matches `helpdesk.freedom.nl/algemene-instellingen-eigen-modem` Glasvezel template
- ONT: Genexis XGS-PON on `nl-sw01` Gi1/0/36 (trunk vlan 4,6, PoE-fed, 15.4 W class 3)
- Negotiated IPv4 (when up): **203.0.113.X/32**
- BRAS: `198.51.100.X`
- Upstream hop: `185.85.211.117`

## Reusable diagnostic playbook (for the next PPPoE outage)

### Quick triage commands (ASA)

| Command | What it tells you | What "healthy" looks like |
|---|---|---|
| `show track 1` | PPPoE/SLA state | Reachability: Up, low change count |
| `show ip address` | Po1.6 has WAN IP? | Current table includes Po1.6 with `203.0.113.X` |
| `show interface Port-channel1.6` | Layer-2 state | up/up, MTU 1492, packets-in incrementing |
| `ping outside_budget 198.51.100.X` | Is BRAS itself reachable via failover? | 4/4 ≈ 10 ms — yes |
| (NOT `show pppoe client` / `show vpdn session` / `show logging | tail`) | These commands DON'T exist on ASA 9.16 | See `feedback_asa_9_16_pppoe_show_commands.md` |

### Customer-side surfaces to verify ARE healthy (rule them out first)

1. **PoE on Gi1/0/36**: `show power inline Gi1/0/36` → Class 3, 15.4 W, On
2. **Switch port up + L2 traffic**: `show interfaces Gi1/0/36` line-protocol up, `show mac address-table interface Gi1/0/36` populated
3. **PADI egress**: live `shut/no shut` Po1.6 + `wr` → Po1.6 output counters increment → our frames ARE leaving
4. **PADO ingress**: ASA `capture WAN interface outside_freedom ethernet-type 0x8863` → 0 received frames means BRAS isn't responding

If all 4 are good and PPPoE is still stuck, **it's Freedom-side**. Move to phone escalation.

### Don't do these

- **Don't** ask Freedom to look up the PPPoE username — the literal string on the ASA is `fake@freedom.nl` (Freedom's PAP is line-bound to the XGS-PON SN, so the username string is decorative). Identify the line by `F0381391` / public IP `203.0.113.X` / circuit-ID. ([[feedback_freedom_pppoe_username_is_decorative]] — flagged as a future feedback memory)
- **Don't** touch MTU/MSS on outside_freedom — Po1.6 MTU 1492 is correct PPPoE-on-1500-underlay; MSS clamping is irrelevant pre-Discovery. See `feedback_pppoe_discovery_failure_not_mss_clamping.md`.
- **Don't** confuse the BRAS reachability test source — ping the BRAS from `outside_budget`, NOT from `outside_freedom` (which has no IP during the outage). Pinging from outside_freedom returns 0/4 because there's no L3 — that's expected, not signal.

### Phone escalation path

- **Freedom NOC: `088-0115666`** (Mon–Fri 10:00–17:30 CET). English supported.
- Preempt first-line "have you tried turning it off and on" — explicitly demand escalation to NOC for a BRAS-side line reset / clear of half-open PPPoE session on line F0381391.
- Cite any existing email thread (Davide handled this case in English).

### Recovery signal (post-fix)

- `show track 1` → Reachability **Up**, change counter increments by 1
- `show ip address` → `Port-channel1.6` present in **Current** table with `203.0.113.X`
- `show interface Tunnel{4..9}` → line protocol up, source IP `203.0.113.X` (not `0.0.0.0`)
- `ping outside_freedom 198.51.100.X` → 4/4
- ASA syslog NAT translations through `outside_freedom:203.0.113.X` resume immediately

## Lessons / feedback worth keeping

- **Failover via outside_budget worked transparently for ~4.6 days** — the dual-WAN design held up. Bandwidth was reduced (single-FIFO cap on Budget) but no service-level downtime. SLA-tracked failover is the right architecture.
- **Customer-side recovery attempts are limited.** 3 days of daily `shut/no shut` + PoE cycles produced visible PADI egress on Po1.6 output counters but zero PADO frames returned — when the BRAS-side session state is stuck, no amount of customer-side action will fix it.
- **Phone is faster than email** for ISP NOC escalation. The Davide email thread was open since 2026-05-08; the phone call on 2026-05-11 got a field-engineer dispatch.

## Cross-references

- [[feedback_asa_9_16_pppoe_show_commands]] — which `show` commands DON'T exist on ASA 9.16
- [[feedback_pppoe_discovery_failure_not_mss_clamping]] — common diagnostic mistake
- [[feedback_asa_syslog_timezone_cest_not_utc]] — ASA syslog uses CEST, not UTC; subtract 2h in summer when copy-pasting timestamps into Freedom support emails

## Doc drift to fix later (not blocking)

- `network/CLAUDE.md` WAN table lists "XS4ALL on Po1.2 VLAN 2" but live nameif is `outside_budget` (Po1.2, VLAN 2, 10.0.X.X/30 → rtr01). Cosmetic; address in a future MR.

*Source: `memory/freedom_pppoe_outage_resolved_20260513.md`*
...
- Added 7 NAT exemptions for VPS tunnels on xs4all:
  - mgmt/k8s/grdmz02 → `S2S_NO_TUNNEL` (3 rules)
  - mgmt/k8s/grdmz02 → `S2S_CH_TUNNEL` (3 rules)
  - grdmz02 → `ALL_VPS_OVERLAY` (1 rule)
- `write memory` saved
...
4. GR `dmz_servers02_access_in` ACL — added permit rules for grdmz02→NL_mgmt and grdmz02→NL_k8s before the deny-all (required for IKEv2 TS narrowing, not just data plane)
...
| **gr-fw01** (GR ASA) | xs4all peer added to seq 60-73. Entries 37/38 deleted and re-created with xs4all as primary peer. Duplicate seq 72/73 removed. `dmz_servers02_access_in` ACL: added permit for grdmz02→NL_mgmt and grdmz02→NL_k8s before deny rules. | `write memory` ✓ |
...
## VPS BGP VTI update-source fix

GR Prometheus TargetDown (IFRGRSKG01PRD-185) for frr-edge-nodes and ipsec-edge-nodes on VPS overlay IPs (10.255.X.X, 10.255.X.X) -- resolved 2026-04-14.

**Root cause (two-layer):**

1. VPS nodes peered with GR RRs using loopback IPs (10.255.X.X, 10.255.X.X) as `update-source`, while NL RRs correctly used VTI point-to-point addresses. The GR ASA received BGP routes for 10.255.X.X/24 and 10.255.X.X/24 with unresolvable next-hops -- routes never installed.

2. Both VPS had FRR multipath enabled by default, installing ECMP across xfrm-gr AND xfrm-nl tunnels. Return traffic hashed to a different tunnel than inbound, breaking stateful ASA flow tracking.

**Fix:**
- CH VPS: `neighbor 10.0.X.X update-source 10.255.200.X` (was 10.255.X.X), same for .15.4
- NO VPS: `neighbor 10.0.X.X update-source 10.255.200.X` (was 10.255.X.X), same for .15.4
- GR RRs (grk8s-frr01+grk8s-frr02): `neighbor 10.255.200.X` and `neighbor 10.255.200.X` replacing loopback peers
- Both VPS: `maximum-paths ibgp 1` to eliminate cross-tunnel ECMP. Best-path (shortest cluster-list) selects direct tunnel. Failover preserved via BGP reconvergence.

**Why:** FRR route-map `set ip next-hop` does NOT override iBGP reflected route next-hops (tested, confirmed). The only reliable way to control BGP next-hop for reflected routes is at the source (update-source on the originating router).

**How to apply:** When adding new VPN tunnel peers to the iBGP mesh, always use VTI point-to-point addresses as update-source, never loopback IPs. The NL config is the reference pattern. Avoid ECMP across tunnels that traverse different stateful firewalls.

**Full FRR topology (8 instances):**
- nlk8s-frr01 (NL RR, 10.0.X.X, cluster-id 1.1.1.1, nl-pve01 CT VMID_REDACTED)
- nlk8s-frr02 (NL RR, 10.0.X.X, cluster-id 1.1.1.1, nl-pve03 CT VMID_REDACTED)
- grk8s-frr01 (GR RR, 10.0.X.X, cluster-id 2.2.2.2, gr-pve01 CT 201580201)
- grk8s-frr02 (GR RR, 10.0.X.X, cluster-id 2.2.2.2, gr-pve02 CT 202580201)
- CH VPS (10.255.X.X, chzrh01vps01, AS 65000 + AS 214304 IPv6)
- NO VPS (10.255.X.X, notrf01vps01, AS 65000 + AS 214304 IPv6)
- NL ASA (10.0.X.X, RR client)
- GR ASA (10.0.X.X, RR client)

**Pre-existing:** NL-FRR02 (10.0.X.X) in Connect state on both VPS for 13h+ (xs4all VTI path, unrelated).

*Source: `memory/vps_bgp_vti_update_source_fix_20260414.md`*

## VPS ipsec-health cron enabled on both VPSs — chaos engineering implication

**Active cron (both VPSs, 2026-04-21)**

`/etc/cron.d/ipsec-health`:
```
*/2 * * * * root /usr/local/bin/ipsec-health-check.sh
```

`/usr/local/bin/ipsec-health-check.sh` — v2 behavior (per script header):
- Lists all strongSwan connections (`swanctl --list-conns`)
- For each connection NOT in ESTABLISHED state, runs `swanctl --initiate --child <conn>`
- Never restarts charon globally (avoids cascading SA flushes)
- Exits quietly if charon vici socket unresponsive (avoids thrashing)
- Complements (does not replace) strongSwan's built-in `start_action = start` + DPD recovery

**Hosts**: notrf01vps01, chzrh01vps01 (each has its own independent cron).

**Chaos engineering implications**

1. **Any chaos experiment that tears down an IPsec SA on a VPS will auto-recover within 0–2 minutes.** If your experiment requires the tunnel to STAY down for longer (to measure user-visible impact, to test failover to another path), disable the cron before the experiment:
   ```
   mv /etc/cron.d/ipsec-health /etc/cron.d/ipsec-health.disabled-<date>
   ```
   and re-enable after:
   ```
   mv /etc/cron.d/ipsec-health.disabled-<date> /etc/cron.d/ipsec-health
   ```
2. **Chaos scripts at `scripts/chaos-*.sh` and the weekly chaos cron (daily 10:00 UTC self-selecting)** should account for this auto-healing when computing "recovery_seconds" — the measured time is effectively bounded below by strongSwan's native retry (typically faster) OR by the 2-min cron interval, whichever fires first.
3. **Failure-mode to watch**: if the VPS can't reach the remote peer (e.g., Freedom ONT shut), `swanctl --initiate` will fail every 2 min. Check `journalctl -t ipsec-health` on the VPS to see rate of failed initiates — non-zero means upstream peer is unreachable.
4. **Interaction with 2026-04-21 BGP-driven DMZ route change** ([vps_bgp_driven_dmz_route_20260421.md](vps_bgp_driven_dmz_route_20260421.md)): the ipsec-health cron is about re-establishing an SA after it dies; the BGP-driven route change is about rerouting traffic away from a dead SA. They're complementary — BGP moves traffic off immediately (sub-second via BFD), and when the underlying ISP returns, ipsec-health re-establishes the SA (within 2 min), restoring primary path.

**Why it was disabled before**: the `.disabled-20260421` suffix indicates today's re-enable; the timestamp on the script shows it's been present since Dec 2025 (`-rw-r----- 1 root root 54 Dec 27 16:25`). Operator re-enabled 2026-04-21 as part of the budget migration readiness pass, explicitly to speed up recovery during chaos-class events.

**Not applied anywhere else**: nl-claude01 has no equivalent, NL FRR LXCs don't run strongSwan (they run FRR only), GR FRRs likewise. VPS-only script.

*Source: `memory/vps_ipsec_health_cron_enabled_20260421.md`*
...
**FRR (4 RRs + 2 VPS + 2 ASAs) — updated 2026-04-14:**
...
- GR RR1 (grk8s-frr01): VPS peers at GR VTI IPs (10.255.200.X CH, 10.255.200.X NO) — changed from loopback 2026-04-14
- GR RR2 (grk8s-frr02): VPS peers at GR VTI IPs (10.255.200.X CH, 10.255.200.X NO) — changed from loopback 2026-04-14
- VPS FRR: `update-source` for ALL site RR peers uses VTI /31 IPs (NL: 2026-04-10, GR: 2026-04-14)
- VPS FRR: `maximum-paths ibgp 1` to prevent cross-tunnel ECMP through stateful ASAs (2026-04-14)
- **Why:** ASA can't resolve BGP next-hop when it's the loopback IP. VTI /31 IPs resolve via ASA's connected Tunnel interface routes.
- Inter-VPS peer: unchanged (still use loopback update-source)
