...
> Compiled from 26 CLAUDE.md files + 0 memory files. 2026-04-09 06:19 UTC.
...
# Custom Docker Images — Claude Code Instructions
## Purpose
Build contexts for Docker images pushed to the private registry. Most are **CI runner images** used by GitLab pipelines. One (`protonmail-bridge`) is a **service image** deployed to a host via docker-compose. Never place these under `docker/` (that directory triggers the deployment pipeline).
## Registry
