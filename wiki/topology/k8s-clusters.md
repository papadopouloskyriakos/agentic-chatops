...
> Compiled from 2 CLAUDE.md files + 2 memories. 2026-04-09 06:19 UTC.
...
│   └── pod-disruption-budgets/ # CoreDNS + Metrics Server PDBs
- K8s worker nodes have iscsid `noop_out_interval=30` / `noop_out_timeout=30` (not default 5s) to tolerate brief network hiccups without dropping iSCSI sessions. This is set in `/etc/iscsi/iscsid.conf` on each node — must be reapplied after node reprovisioning.
- ClusterSecretStore name is `openbao` — reference it in all ExternalSecret specs
