> **DEPRECATED (2026-06-23, IFRNLLEI01PRD-1260):** This free-prose spec is superseded by the EARS + Gherkin spec under `spec/` governed by `constitution.md`. Retained for historical context only; do not treat as the source of truth.
...
There is no way to:
1. Tell the system "I'm about to reboot nl-pve01" and have it suppress alerts intelligently
...
```
User:    @claude I'm updating nl-pve01, expect reboot in 5 min
Claude:  Setting 2h maintenance window on nl-pve01 + 83 guests (75 LXC, 8 QEMU).
         Alerts suppressed. I'll monitor recovery and report back.

[nl-pve01 reboots]

Claude:  nl-pve01 is back (uptime: 2min). Monitoring guest recovery...
...
User:    ignore it
Claude:  OK. Clearing maintenance window on nl-pve01 + 83 guests.
...
- [ ] iSCSI targets (nl-nas01): check sessions active
...
|------|------|-----------|------------------|
| nl-pve01 | Primary PVE (i9-12900H, 96GB) | 75 LXC + 8 QEMU (incl. K8s nlk8s-ctrl01, nlk8s-node01, nlk8s-node02) | K8s loses 1 control plane + 2 workers |
| nl-pve02 | Secondary PVE (**VM on nl-nas01**, Ryzen, 16GB) | 7 LXC (incl. K8s nlk8s-ctrl02) | K8s loses 1 control plane. **nl-nas01 reboot kills nl-pve02 too.** |
| nl-pve03 | Tertiary PVE (i9-14900K, 128GB) | 34 LXC + 14 QEMU (incl. K8s nlk8s-ctrl03, nlk8s-node03, nlk8s-node04) | K8s loses 1 control plane + 2 workers |
...
| nl-nas01 | Primary NAS (DS1621+) | **nl-pve02 VM** + K8s iSCSI LUNs | K8s iSCSI PVCs go read-only, NFS hangs, **nl-pve02 + 7 guests die** |
...
- **Never reboot nl-pve01 and nl-pve03 simultaneously** — would lose 2/3 control plane nodes (etcd quorum lost)
- **Safe order:** nl-pve02 first (only nlk8s-ctrl02, 4GB, least critical) → nl-pve01 → nl-pve03
...
- [ ] Tested with a real PVE node reboot (start with nl-pve02 — least guests, least risk)
