...
def check_openapi_valid(root: Path) -> CheckResult:
    """C06: All contracts/openapi.yaml validate via swagger-cli or spectral.
    ...
    errors: list[str] = []
    for f in files:
        # TODO: prefer `npx @stoplight/spectral-cli lint --ruleset spectral:oas` for richer linting
        result = subprocess.run(
            ["npx", "--yes", "@apidevtools/swagger-cli", "validate", str(f)],
            capture_output=True, text=True,
        )
        if result.returncode != 0:
            errors.append(f"{f.relative_to(root)}: {result.stderr.strip()[:200]}")
    if errors:
...
def check_asyncapi_valid(root: Path) -> CheckResult:
    ...
    for f in files:
        result = subprocess.run(
            ["npx", "--yes", "@asyncapi/cli", "validate", str(f)],
            capture_output=True, text=True,
        )
        if result.returncode != 0:
            errors.append(f"{f.relative_to(root)}: {result.stderr.strip()[:200]}")
...
def check_gherkin_parseable(root: Path) -> CheckResult:
    """C14: Every .feature file in acceptance/ parses and references at least one REQ-NNN."""
    ...
    errors: list[str] = []
    for f in files:
        # TODO: prefer real Gherkin parser, e.g. `npx -p @cucumber/cucumber cucumber-js --dry-run`
        text = f.read_text()
        if "Feature:" not in text or "Scenario:" not in text:
            errors.append(f"{f.relative_to(root)}: missing Feature: or Scenario:")
        if not REQ_ID_RE.search(text) and "REQ-" not in text:
            errors.append(f"{f.relative_to(root)}: no REQ-NNN reference")
    if errors:
        return CheckResult("gherkin_parseable", False, f"{len(errors)}/{len(files)} invalid", errors)
    return CheckResult("gherkin_parseable", True, f"{len(files)} .feature files OK")
...
def check_slot_config_entry_valid(root: Path) -> CheckResult:
    ...
    # TODO: also verify keys 'cwd' + 'room' present, room is Matrix room-id shape,
    #       cwd is absolute path, slot key not already in live slot-config.json.
    return CheckResult("slot_config_entry_valid", True,
                       f"{len(entry)} slot(s) declared (deep validation TODO)")
    ...
    for f in files:
        ...
        for task in data.get("tasks", []):
            ...
            if not isinstance(score, (int, float)) or not 0.0 <= score <= 1.0:
                errors.append(f"{f.relative_to(root)}: task {task.get('id')} risk_score={score!r} not in [0,1]")
            elif score > 0.7:
                high_risk.append(f"{task.get('id')} (risk_score={score})")
    if errors:
