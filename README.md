...
| Component | Role |
|-----------|------|
| **[n8n](https://n8n.io/)** | 26 workflows — alert intake, session management, knowledge population, teacher-agent runner |
...
| QA suite | **411/0 PASS (99.52%)**, 2 benign skips, across **44 suite files** — ~3–5 min run, JSON scorecard, per-suite timeout guard |
...
| SQLite tables | **42** (39 + teacher-agent `learning_progress`/`learning_sessions`/`teacher_operator_dm` [-651/-653]) |
