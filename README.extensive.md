...
Production agentic ChatOps/ChatSecOps/ChatDevOps platform implementing all 21 design patterns from Antonio Gulli's *Agentic Design Patterns* (Springer, 2025). Tri-source audited against [Anthropic's official documentation](https://docs.anthropic.com/) (17 sources), the [Anthropic Academy](https://academy.anthropic.com/) sub-agent design course, and [6 industry references](docs/industry-agentic-references.md). Includes a [Karpathy-style compiled knowledge base](wiki/index.md) — 45 articles auto-compiled from 7+ sources with 4-signal RAG integration (semantic + keyword + wiki + [MemPalace](https://github.com/milla-jovovich/mempalace) session transcripts).
...
10. [n8n Workflows](#10-n8n-workflows-25-475-nodes)
...
- **Self-improving prompts** -- An eval flywheel (98 test scenarios, LLM-as-a-Judge, monthly cycle) detects weak dimensions, generates prompt patches, and applies them automatically. The system literally rewrites its own instructions based on measured performance.
...
- **Karpathy-style compiled wiki** -- 45 articles auto-compiled daily from 7 fragmented knowledge stores (memories, CLAUDE.md files, incidents, lessons, OpenClaw, docs, 03_Lab). Answers "what do we know about host X?" in one lookup instead of five.
...
LibreNMS/Prometheus/CrowdSec Alert
   ...
   |  ~475 nodes   |     |  15 skills    |     |  10 sub-agents    |
   +---------+-----+     +---------------+     +---------+---------+
             |                                           |
             v                                           v
   +--------------+                              +---------------+
...
| 8 | **Memory** | **A+** | All 3 memory types (semantic, episodic, procedural) active across both tiers. 55 CLAUDE.md files auto-routed by hostname to triage. 74 feedback memory files synced across both agent hosts. Procedural rules ("NEVER do X") injected into Tier 1 triage output and Tier 2 Build Prompt. SQLite (23 tables, 148K+ rows incl. `session_transcripts`, `agent_diary`, `otel_spans`, `tool_call_log`). Vector embeddings (nomic-embed-text, 768 dims). Lessons-to-prompt pipeline. **[Karpathy-style compiled wiki](wiki/index.md)** — 45 articles synthesizing all memory types into organized, browsable knowledge with health checks. | Ch8 |
...
| 14 | **RAG** | **A+** | **4-signal hybrid RRF:** (1) semantic — vector embeddings (nomic-embed-text, 768 dims) with cosine similarity; (2) keyword — SQL LIKE on hostname/alert/resolution; (3) **wiki articles** — 45 compiled knowledge base articles; (4) **session transcripts** — verbatim exchange-pair chunks ([MemPalace](https://github.com/milla-jovovich/mempalace), weight 0.3). All fused via Reciprocal Rank Fusion. Plus deterministic channel: hostname-routed CLAUDE.md extraction (55 files, category-aware grep). Triage RAG at Step 1.5 (semantic) + Step 2-kb (CLAUDE.md + memory). 3-tier injection for Tier 2. Backfill cron every 30min. Wiki recompiled daily. | Ch14 |
...
| 18 | **Guardrails** | **A+** | 7-layer defense: Claude Code hooks (PreToolUse) + safe-exec.sh + exec-approvals.json (36 patterns) + input sanitization (42 injection patterns incl. encoding obfuscation, role confusion, delimiter injection, social engineering) + credential/PII scanning (16 regex) + output fact-checking + **Evaluator-Optimizer** (Haiku screening for high-stakes responses, 3 nodes). Per-source token caps on injected knowledge. Tool call limit (75). Zero hardcoded passwords. | Ch18 |
| 19 | **Evaluation** | **A+** | 19-surface [Prompt Scorecard](scripts/grade-prompts.sh) (6 dimensions, daily). [Agent Trajectory](scripts/score-trajectory.sh) (8 infra / 4 dev steps). [LLM-as-a-Judge](scripts/llm-judge.sh) (Haiku/Opus, 5 dimensions, [calibrated](scripts/judge-calibrate.sh)). **98 test scenarios** across [3 eval sets](docs/evaluation-process.md) (regression/discovery/holdout) + 18 node-level tests + 12 negative controls. [CI eval gate](.gitlab-ci.yml). [Eval flywheel](scripts/eval-flywheel.sh) (monthly). Reproducibility (temp=0, seed=42). | Ch19 |
...
[`scripts/holistic-agentic-health.sh`](scripts/holistic-agentic-health.sh) — **138 automated checks** across 37 sections that verify every feature claimed in this README actually works in production. Not just "does the file exist?" — functional tests, cross-site verification, and e2e smoke tests.

**Latest score: 99%** (136 pass, 1 fail, 0 warn) in 18 seconds.

```bash
./scripts/holistic-agentic-health.sh            # Full run (138 checks, ~18s)
./scripts/holistic-agentic-health.sh --quick     # Skip SSH/kubectl (~8s)
...
| SQLite Tables | 12 | 25 tables, 148K+ rows, 8 staleness thresholds (per-table freshness) |
...
| Eval Pipeline | 12 | 98 scenarios (4 sets), 5 scripts, judgments, **functional trajectory test** |
...
| Crons | 1 | 32 active cron entries |
| Self-Improving Prompts | 2 | 5 active patches, prompt-improver executable |
| Predictive Alerting | 2 | Script + daily cron configured |
| Compiled Wiki | 3 | 41 articles, daily cron, compile freshness (<48h) |
...
| Knowledge Injection | 2 | 55 CLAUDE.md files, 97 memory files |
...
| Hook | Event | Purpose |
|------|-------|---------|
| [`audit-bash.sh`](scripts/hooks/audit-bash.sh) | PreToolUse (Bash) | Logs all commands + blocks 30+ destructive patterns + reverse shells |
| [`protect-files.sh`](scripts/hooks/protect-files.sh) | PreToolUse (Edit\|Write) | Blocks edits to .env, *.key, *.pem, credentials, passwords |
...
| `session_trajectory` | 52 | Per-session agent trajectory scores (8 infra / 4 dev step markers) |
| `session_judgment` | 36 | LLM-as-a-Judge results (5-dimension rubric, Haiku/Opus + dev rubric) |
| `session_feedback` | 1 | Thumbs up/down reactions linked to issues |
| `session_transcripts` | 834 | Verbatim JSONL exchange-pair chunks with embeddings (4th RAG signal, [MemPalace](https://github.com/milla-jovovich/mempalace)) |
| `agent_diary` | 55 | Persistent per-agent knowledge across sessions, 10 agent archetypes |
| `incident_knowledge` | 33 | Alert resolutions with vector embeddings (nomic-embed-text, 768 dims) |
...
| `wiki_articles` | 45 | Compiled wiki articles with vector embeddings (3rd RAG signal) |
...
| `graph_entities` | 263 | GraphRAG entities (host, alert_rule, incident, lesson) |
| `graph_relationships` | 127 | GraphRAG relationships (triggers, affects, resolves, depends_on) |
...
### Prometheus Metrics (10 exporters, 31 cron jobs)
...
Additional: AUTHORIZED_SENDERS filter, **$10/session cost warning + $25/day plan-only** budget ceiling, zero hardcoded passwords (all env vars sourced from `.env`), confidence gating (< 0.5 = STOP).
...
Four RAG channels fused via 4-signal Reciprocal Rank Fusion:
...
| Component | Detail |
|-----------|--------|
| CLAUDE.md files | 51 across all repos (IaC, products, gateway, RFCs, websites) |
...
| Memory files | 74 feedback memories with operational rules ("NEVER do X") |
...
| Component | Detail |
|-----------|--------|
| Compiler | [`wiki-compile.py`](scripts/wiki-compile.py) — reads 7+ sources, compiles 45 articles |
...
| RRF weight | 0.3 (lower than summarized incident_knowledge at 1.0 to avoid noise from raw exchanges) |
...
- 73 memory files (grep by keyword)
...
[`scripts/wiki-compile.py`](scripts/wiki-compile.py) reads all sources, compiles them into 45 markdown articles organized by category:
...
**RAG integration:** All 45 articles are chunked by heading and embedded via nomic-embed-text. They appear as a 3rd ranking signal in the existing Reciprocal Rank Fusion search — alongside semantic (incident_knowledge) and keyword matches.
...
| RAG signals | 2 (semantic + keyword) | **4 (+ wiki + session transcripts)** |
| Knowledge articles | 0 | **45** |
...
├── grafana/                        # Dashboard JSON exports (9 dashboards, 127 panels)
├── openclaw/
...
│   ├── kb-semantic-search.py       # 4-signal RRF search (semantic + keyword + wiki + transcripts)
│   ├── wiki-compile.py             # Karpathy-style wiki compiler (7+ sources → 45 articles)
...
│   └── ... (50 scripts total)
├── wiki/                           # Compiled knowledge base (45 auto-generated articles)
