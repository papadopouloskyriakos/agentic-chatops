...
# Structured debug trail (IFRNLLEI01PRD pipeline observability). Fallback no-op when
# the helper isn't present (e.g. inside the OpenClaw container).
PIPELINE_DEBUG_SCRIPT="escalate-to-claude.sh"
# shellcheck source=/dev/null
source /app/claude-gateway/scripts/lib/pipeline-debug.sh 2>/dev/null || pdbg() { :; }
pdbg "escalate_start" "$ISSUE_ID" "fire n8n youtrack-webhook (FORCE_ESCALATE=${FORCE_ESCALATE:-})"
...
pdbg "escalate_result" "$ISSUE_ID" "http=$HTTP_CODE resp=$(printf '%s' "$RESPONSE" | head -c 120)"
