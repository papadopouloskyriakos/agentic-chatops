...
# Load credentials
for d in /root/.openclaw/workspace /home/node/.openclaw/workspace; do [ -r "$d/.env" ] && . "$d/.env" && break; done
...
if [ -z "$ISSUE_ID" ]; then
  # Search YouTrack for open issues with same Alert Rule (created within 24h)
  EXISTING=$(python3 -c "
import urllib.request, json, ssl, urllib.parse, time
ctx = ssl.create_default_context()
ctx.check_hostname = False
ctx.verify_mode = ssl.CERT_NONE
query = 'project: $YT_PROJECT Alert Rule: $ALERTNAME State: -Done,-Cancelled,-Duplicate sort by: created desc'
