...
# Load credentials. Searches openclaw-container path first then claude01 repo path.
for d in /root/.openclaw/workspace /home/app-user/.openclaw/workspace /app/claude-gateway; do [ -r "$d/.env" ] && . "$d/.env" && break; done
# Normalise env var names. app-user's .env uses YOUTRACK_API_TOKEN; the
# openclaw .env uses YOUTRACK_TOKEN. Same for LIBRENMS variants.
...
run_tier1_suppression "${HOSTNAME:-${NODE:-k8s}}" "$ALERTNAME" "$SEVERITY"
...
if [ -f "$KB_SEARCH" ] && [ -f "$_TRIAGE_GATEWAY_DB" ]; then
  PRIOR_KNOWLEDGE=$(GATEWAY_DB="$_TRIAGE_GATEWAY_DB" python3 "$KB_SEARCH" search "${KB_QUERY//\'/\'}" --limit 3 --days 90 2>/dev/null) || true
else
...
# ─── Step 2-kb: CLAUDE.md + Memory Knowledge ───
echo "--- Step 2-kb: CLAUDE.md + Memory Knowledge ---"
KB_CONTEXT=""
KB_HOST="${NODE:-$ALERTNAME}"
if [ -f "$SCRIPT_DIR/claude-knowledge-lookup.sh" ]; then
  KB_CONTEXT=$("$SCRIPT_DIR/claude-knowledge-lookup.sh" "$KB_HOST" "kubernetes" --site "$SITE_ID" 2>/dev/null) || true
...
# Step 2 is best-effort exploratory data collection. Many pipelines end in
# `grep | tail` where a zero-match grep exits non-zero — under pipefail +
# the ERR trap that would abort the whole triage. Observed failures on GR
# (IFRGRSKG01PRD-188/-197/-201) all came from this class of false-negative:
# e.g. line 550 `grep -oE 'pve[0-9]+'` on NetBox-returned "gr-pve"
# (no digit suffix); line 635 `kctl logs --previous` when pod never
# restarted; lines 716/769 `grep -iE 'error|...'` when logs are clean.
#
# Disable pipefail + ERR trap for Step 2; re-enable at Step 3.
set +o pipefail
trap - ERR
...
# Initialize PVE lookup vars (used in Step 2e outside the NODE block)
K8S_VMID=""
K8S_PVE_HOST=""
K8S_HOST_TYPE=""
...
# Re-enable strict error handling for action-bearing steps (YT posts,
# escalation). Step 2's pipefail-off guard ends here.
set -o pipefail
trap error_handler ERR
...
PIPELINE_DEBUG_SCRIPT="k8s-triage.sh"
...
if [ "$SHOULD_ESCALATE" = true ]; then
  if [ "${SKIP_ESCALATION:-}" != "true" ]; then
    pdbg "escalate_decision" "${ISSUE_ID:-unknown}" "escalate=yes alert=$ALERTNAME sev=$SEVERITY reusing=${REUSING_ISSUE:-} ctrlplane=${CONTROL_PLANE_ALERT:-} conf=${TRIAGE_CONFIDENCE:-}"
    ESCALATION_REASON=""
    ...
    pdbg "escalate_decision" "${ISSUE_ID:-unknown}" "escalate=skipped reason=SKIP_ESCALATION alert=$ALERTNAME sev=$SEVERITY"
  fi
else
  echo "--- Step 5: No escalation criteria met (monitoring only) ---"
  pdbg "escalate_decision" "${ISSUE_ID:-unknown}" "escalate=no reason=no-criteria-met alert=$ALERTNAME sev=$SEVERITY reusing=${REUSING_ISSUE:-} ctrlplane=${CONTROL_PLANE_ALERT:-} conf=${TRIAGE_CONFIDENCE:-}"
fi
