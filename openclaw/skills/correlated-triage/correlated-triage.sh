...
# If gateway.maintenance is active for a scheduled ASA reboot, suppress burst triage entirely.
MAINT_FILE="/home/app-user/gateway.maintenance"
if [ -f "$MAINT_FILE" ]; then
  MAINT_EVENT=$(python3 -c "import json; print(json.load(open('$MAINT_FILE')).get('event_id',''))" 2>/dev/null || echo "")
  if echo "$MAINT_EVENT" | grep -q "asa-weekly-reboot"; then
    MAINT_REASON=$(python3 -c "import json; print(json.load(open('$MAINT_FILE')).get('reason','unknown'))" 2>/dev/null || echo "unknown")
    echo ""
    echo "*** SCHEDULED EVENT — BURST SUPPRESSED ***"
    echo "Reason: $MAINT_REASON"
    echo "This correlated alert burst is expected during the ASA reboot."
    echo "All affected hosts will recover automatically within 10 minutes."
    echo ""
    echo "CONFIDENCE: 0.1 — Correlated burst during scheduled ASA reboot"
    exit 0
  fi
fi
...
# ─── Freedom ISP PPPoE Outage Pattern Detection ───
# Pattern: NL "Service up/down" + GR "Devices up/down" firing simultaneously
# = Freedom WAN down, S2S VPN collapsed, xs4all takes over automatically
HAS_NL_SVC_DOWN=false
HAS_GR_DEV_DOWN=false
for i in "${!HOSTS[@]}"; do
  [[ "${HOSTS[$i]}" == nl* ]] && [[ "${RULES[$i]}" == *"Service up/down"* ]] && HAS_NL_SVC_DOWN=true
  [[ "${HOSTS[$i]}" == gr* ]] && [[ "${RULES[$i]}" == *"up/down"* ]] && HAS_GR_DEV_DOWN=true
done

if $HAS_NL_SVC_DOWN && $HAS_GR_DEV_DOWN; then
  echo ""
  echo "*** FREEDOM ISP OUTAGE PATTERN DETECTED ***"
  echo "NL service alerts + GR device-down alerts firing together = Freedom PPPoE failure."
  echo ""
  echo "Evidence to gather:"
  echo "  1. show vpdn pppinterface — PPP id=1 'deleted and pending reuse' = confirmed"
  echo "  2. show interface outside_freedom — 'IP address unassigned' = confirmed"
  echo "  3. show vpn-sessiondb l2l — check if xs4all tunnels auto-established"
  echo ""
  echo "Expected behavior: xs4all (145.53.163.13) carries all S2S tunnels automatically."
  echo "Tenant QoS auto-applied via freedom-qos-toggle.sh cron (5/2 Mbps per room)."
  echo "GR alerts are FALSE POSITIVES — GR devices are fine, just unreachable during"
  echo "the tunnel renegotiation period (~1-5 minutes)."
  echo ""
  echo "Physical fix: power-cycle Genexis XGS-PON ONT on nl-sw01 Gi1/0/36 (PoE-powered)"
  echo "or contact Freedom ISP."
  echo ""
  echo "CONFIDENCE: 0.95 — Freedom ISP PPPoE outage pattern (NL svc + GR dev-down burst)"
  echo ""
  echo "Continuing with per-host triage for detailed evidence collection..."
  echo ""
fi
...
# ─── Step 1b: Shared CLAUDE.md context for correlated burst ───
echo "--- Step 1b: Shared CLAUDE.md context ---"
if [ -f "$SCRIPT_DIR/../claude-knowledge-lookup.sh" ]; then
  SHARED_KB=$("$SCRIPT_DIR/../claude-knowledge-lookup.sh" "${HOSTS[0]}" "correlated" --site "$SITE_ID" 2>/dev/null) || true
  if [ -n "$SHARED_KB" ] && ! echo "$SHARED_KB" | grep -q "^No relevant"; then
    echo "$SHARED_KB" | head -15
    ./skills/yt-post-comment.sh "$MASTER_ID" "Procedural knowledge context (from CLAUDE.md + Memory):
$SHARED_KB" 2>&1 || echo "WARN: Failed to post knowledge comment"
  else
    echo "No shared CLAUDE.md knowledge found"
  fi
else
