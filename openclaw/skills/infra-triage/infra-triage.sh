...
MAINT_FILE="/home/claude-runner/gateway.maintenance"
MAINT_ENDED_FILE="/home/claude-runner/gateway.maintenance-ended"
...
if [ -z "$ISSUE_ID" ]; then
  # Search YouTrack for open issues with same hostname (created within 24h)
  EXISTING=$(python3 -c "
import urllib.request, json, ssl, urllib.parse, time
ctx = ssl.create_default_context()
ctx.check_hostname = False
ctx.verify_mode = ssl.CERT_NONE
query = 'project: $YT_PROJECT Hostname: $HOSTNAME State: -Done,-Cancelled,-Duplicate sort by: created desc'
...
PRIOR_KNOWLEDGE=$(ssh -o StrictHostKeyChecking=no -o ConnectTimeout=5 -o BatchMode=yes \
  -i ~/.ssh/one_key claude-runner@nl-claude01 \
...
echo "--- Step 2: Classifying device via LibreNMS ---"
...
SYSLOG_ERRORS=$(fetch_syslog "$HOSTNAME" 30 "error|fail|down|restart|critical|warn|denied|unreachable" 2>/dev/null || true)
SYSLOG_RECENT=$(fetch_syslog "$HOSTNAME" 20 2>/dev/null || true)
...
SSH_OPTS="-i /home/node/.ssh/one_key -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10"
...
COMMENT="Level 2 Automated Investigation Results:${RECURRENCE_NOTE}
$(echo -e "$FINDINGS")
${RELATED_NOTE}$(echo -e "$PRIOR_NOTE")
Alert: $RULE_NAME ($SEVERITY)
Recommended action: Manual review required. Container/VM may need restart or configuration fix."
...
# Store episodic memory for OpenClaw (via SSH to claude-runner where DB lives)
