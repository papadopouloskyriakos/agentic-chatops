...
" 2>/dev/null)
  if [ "$FREEDOM_STATUS" = "DOWN" ]; then
    ...
    echo "xs4all WAN (145.53.163.13) has full S2S tunnel coverage — GR connectivity"
...
if [ -f "$KB_SEARCH" ] && [ -f "/home/node/.claude-data/gateway.db" ]; then
  PRIOR_KNOWLEDGE=$(GATEWAY_DB=/home/node/.claude-data/gateway.db python3 "$KB_SEARCH" search "${KB_QUERY//\'/\'}" --limit 3 --days 90 --threshold 0.5 --mode hybrid --rewrite 2>/dev/null) || true
else
