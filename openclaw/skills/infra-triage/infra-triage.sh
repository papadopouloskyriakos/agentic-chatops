...
# ─── Step 0.5: Tier 1 suppression decision (shared 3-phase flow) ──────────
# Phase 1 dedup / Phase 2 knownpattern / Phase 3 active-memory.
# Fails OPEN on any error. Disable via TIER1_SUPPRESSION_DISABLED=1.
CURRENT_STEP="Step 0.5 (suppression check)"
. ./skills/lib/tier1-suppression-flow.sh
run_tier1_suppression "$HOSTNAME" "$RULE_NAME" "$SEVERITY"
...
# ─── Self-learning (scheduled-reboot): if a reboot alert reached investigation
# (not suppressed by phase SR) and the feature is enabled, RCAs the reboot and
# registers a deterministic schedule as 'observing' so the promoter can confirm
# + promote it. Non-blocking; registration NEVER suppresses the current alert.
# Idempotent (upsert dedupes); only fires on reboot-class rules.
if [ "${TIER1_SCHED_REBOOT_ENABLED:-0}" = "1" ] || [ -f "$HOME/gateway.sched_reboot" ]; then
  case "$(echo "$RULE_NAME" | tr '[:upper:]' '[:lower:]')" in
    *reboot*|*uptime*|*reloaded*|*sysuptime*)
      nohup python3 /app/claude-gateway/scripts/classify-reboot-alert.py \
        "$HOSTNAME" "$RULE_NAME" --register >/dev/null 2>&1 & ;;
  esac
fi
...
# Step 2-graph: Infragraph dependency context (IFRNLLEI01PRD-1036)
# ADVISORY ONLY — fails open. Any failure (graph empty, host unknown, timeout,
# INFRAGRAPH_DISABLED=1) leaves triage exactly as before this step existed.
echo "--- Step 2-graph: Infragraph Dependency Context ---"
INFRAGRAPH_SUMMARY=""
IG_QUERY="$HOME/gitlab/n8n/claude-gateway/scripts/infragraph-query.py"
if [ -n "${INFRAGRAPH_DISABLED:-}" ] && [ "${INFRAGRAPH_DISABLED}" != "0" ]; then
  echo "Infragraph disabled via INFRAGRAPH_DISABLED — skipping"
elif [ -f "$IG_QUERY" ]; then
  IG_BLAST=$(timeout 5 python3 "$IG_QUERY" blast-radius --host "$HOSTNAME" 2>/dev/null) || IG_BLAST=""
  # Phase B (IFRNLLEI01PRD-1039): --record logs every cascade prediction +
  # its shuffled-graph control as a shadow row for the hourly eval. Append-
  # only, zero authority — the eval scorecard is the -1040 gate evidence.
  IG_CASCADE=$(timeout 5 python3 "$IG_QUERY" cascade --host "$HOSTNAME" --rule "$RULE_NAME" --record --issue "${ISSUE_ID:-}" 2>/dev/null) || IG_CASCADE=""
  if [ -n "$IG_BLAST" ]; then
    INFRAGRAPH_SUMMARY=$(IG_B="$IG_BLAST" IG_C="$IG_CASCADE" python3 - <<'PYEOF' 2>/dev/null || true
import json, os
try:
    b = json.loads(os.environ.get("IG_B") or "{}")
    c = json.loads(os.environ.get("IG_C") or "{}")
    parts = []
    counts = b.get("counts") or {}
    if counts.get("total"):
        by_t = ", ".join(f"{v} {k}" for k, v in sorted((counts.get("by_type") or {}).items()))
        parts.append(f"blast radius: {counts['total']} downstream ({by_t})")
        top = [n["name"] for n in (b.get("nodes") or [])[:6]]
        if top:
            parts.append("affected: " + ", ".join(top) + (" ..." if counts["total"] > 6 else ""))
    preds = c.get("predictions") or []
    if preds:
        ptxt = "; ".join(
            f"{p['host']}:{p['rule']} (conf {p['confidence']}, src {p.get('source','?')})"
            for p in preds[:5])
        parts.append(f"expected cascade within {c.get('window_seconds')}s: {ptxt}"
                     + (" ..." if len(preds) > 5 else ""))
    print(" | ".join(parts))
except Exception:
    pass
PYEOF
  ...
  if [ -n "$INFRAGRAPH_SUMMARY" ]; then
    echo "INFRAGRAPH: $INFRAGRAPH_SUMMARY"
    FINDINGS="$FINDINGS\nInfragraph context: $INFRAGRAPH_SUMMARY"
  else
    echo "No infragraph context for $HOSTNAME (host not in graph or graph unavailable) — continuing"
  fi
else
  echo "infragraph-query.py not deployed — skipping"
fi
COMPLETED_STEPS="$COMPLETED_STEPS, Step 2-graph (Infragraph)"
echo ""
...
PIPELINE_DEBUG_SCRIPT="infra-triage.sh"
...
if [ "${SKIP_ESCALATION:-}" = "true" ]; then
  ...
  pdbg "escalate_decision" "${ISSUE_ID:-unknown}" "escalate=skipped reason=SKIP_ESCALATION host=$HOSTNAME rule=$RULE_NAME sev=$SEVERITY"
else
  ...
  pdbg "escalate_decision" "${ISSUE_ID:-unknown}" "escalate=yes host=$HOSTNAME rule=$RULE_NAME sev=$SEVERITY reason=$ESCALATION_REASON conf=${TRIAGE_CONFIDENCE:-}"
