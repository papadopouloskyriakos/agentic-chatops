...
case "$SITE" in
  ...
  gr|GR|gr)
    ...
    export LIBRENMS_API_KEY="${LIBRENMS_GR_API_KEY:-REDACTED_c7cb035f}"
    ;;
  *)
...
fetch_syslog() {
  ...
  local lines="${2:-50}"
  local pattern="${3:-}"
  local ssh_opts="-i /home/app-user/.ssh/one_key -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o ConnectTimeout=10"
